...
Axiom concat_fun_bin :
  forall (f:Z -> Z) (g:Z -> Z) (i:Z) (k:Z),
  (forall (j:Z), (0%Z <= (f j))%Z /\ ((f j) <= 1%Z)%Z) ->
  (forall (j:Z), (0%Z <= (g j))%Z /\ ((g j) <= 1%Z)%Z) ->
  (0%Z <= (concat_fun f g i k))%Z /\ ((concat_fun f g i k) <= 1%Z)%Z.

Axiom shift_bin :
  forall (f:Z -> Z) (i:Z) (k:Z),
  (forall (j:Z), (0%Z <= (f j))%Z /\ ((f j) <= 1%Z)%Z) ->
  (0%Z <= (shift f i k))%Z /\ ((shift f i k) <= 1%Z)%Z.
...
Axiom ket_part_def :
  ...
  (forall (j:Z), (0%Z <= (bvx j))%Z /\ ((bvx j) <= 1%Z)%Z) ->
  (forall (j:Z), (0%Z <= (bvy j))%Z /\ ((bvy j) <= 1%Z)%Z) ->
  match to_pre c with
...
Axiom ket_part_spec :
  ...
  (forall (j:Z), (0%Z <= (bvx j))%Z /\ ((bvx j) <= 1%Z)%Z) ->
  (forall (j:Z), (0%Z <= (bvy j))%Z /\ ((bvy j) <= 1%Z)%Z) ->
  (0%Z <= (ket_part c bvx bvy i))%Z /\ ((ket_part c bvx bvy i) <= 1%Z)%Z.
...
Axiom H : ((sum_range (rz (o, o1))) = 0%Z).

Axiom H1 : ((depth (rz (o, o1))) = 1%Z).
...
(* Why3 goal *)
Theorem VC_rz_spec : ((ket_part (rz (o, o1)) bvx bvy i) = (bvx i)).
Proof.
