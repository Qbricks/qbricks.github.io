...
Axiom get_fir :
  forall {a:Type} {a_WT:WhyType a} {b:Type} {b_WT:WhyType b},
  forall (o:(a* b)%type), match o with
                          | (a1, b1) => ((fir (a1, b1)) = a1)
                          end.
...
Axiom reverse_spec :
  ...
  ((forall (e:Z), (mem e s) -> (((reverse s f) (f e)) = e)) /\
   forall (e:Z), (mem e s) -> ((f ((reverse s f) e)) = e)).
...
Axiom get_invol_permut :
  forall (s:set Z) (f:Z -> Z), (is_a_permutation s f) ->
  (invol_permut s f) ->
  (forall (e:Z), (mem e s) -> ((f (f e)) = e)) /\ ((reverse s f) = f).
...
Axiom self_subterms_pre :
  ...
  match c with
  | Sequence d e =>
      ~ (mem c' (subterms d)) -> ~ (mem c' (subterms e)) -> (c' = c)
  | Parallel d e =>
      ~ (mem c' (subterms d)) -> ~ (mem c' (subterms e)) -> (c' = c)
  | _ => (c' = c)
  end.
...
Axiom immediate_subterms :
  forall (c:quantum_circuit),
  match c with
  | Sequence d e => (mem d (subterms c)) /\ (mem e (subterms c))
  | Parallel d e => (mem d (subterms c)) /\ (mem e (subterms c))
  | _ => True
  end.
...
Axiom ket_part_def :
  ...
  | Rx _ => ((ket_part c bvx bvy) = (make_bv (result20 bvx bvy) 1%Z))
  | Ry _ => ((ket_part c bvx bvy) = (make_bv (result21 bvx bvy) 1%Z))
  | Rz _ =>
      ((ket_part c bvx bvy) =
       (make_bv ((fun (y0:bitvec) (y1:Z) => (getbv y0 y1)) bvx) 1%Z))
  | Cnot => ((ket_part c bvx bvy) = (make_bv (result22 bvx) 2%Z))
  | Sequence d e =>
      ((ket_part c bvx bvy) =
       (ket_part e (ket_part d bvx (hpart bvy (sum_range d)))
        (tpart bvy (sum_range d))))
  | Parallel d e =>
      ((ket_part c bvx bvy) =
       (concat (ket_part d (hpart bvx (depth d)) (hpart bvy (sum_range d)))
        (ket_part e (tpart bvx (depth d)) (tpart bvy (sum_range d)))))
  end.
...
Axiom phase_part_def :
  ...
  | Sequence d e =>
      ((phase_part c bvx bvy) =
       (infix_asdt (phase_part d bvx (hpart bvy (sum_range d)))
        (phase_part e (ket_part d bvx (hpart bvy (sum_range d)))
         (tpart bvy (sum_range d)))))
  | Parallel d e =>
      ((phase_part c bvx bvy) =
       (infix_asdt
        (phase_part d (hpart bvx (depth d)) (hpart bvy (sum_range d)))
        (phase_part e (tpart bvx (depth d)) (tpart bvy (sum_range d)))))
  end.
     ...
     ((forall (bvx:bitvec) (bvy:bitvec), forall (i:Z),
      ...
      ((forall (bvx:bitvec) (bvy:bitvec),
        ((phase_part (Rz o) bvx bvy) =
         (d_omega (phase_inv (getbv bvx 1%Z) (div_two o))))) /\
       forall (x:matrix t), (is_a_ket x) -> ((ket_length x) = 1%Z) ->
...
Axiom ry_spec :
  forall (o:(Z* Z)%type),
  (sequence_correct (Ry o)) /\
  (((sum_range (Ry o)) = 1%Z) /\
   (((depth (Ry o)) = 1%Z) /\
    ((forall (bvx:bitvec) (bvy:bitvec),
      ((ket_part (Ry o) bvx bvy) = (make_bv (fc21 bvx bvy) 1%Z))) /\
     ((forall (bvx:bitvec) (bvy:bitvec), forall (i:Z),
       ((1%Z <= i)%Z /\ (i <= 1%Z)%Z) ->
       ((getbv (ket_part (Ry o) bvx bvy) i) = (getbv bvy i))) /\
      ((forall (bvx:bitvec) (bvy:bitvec),
         ...
         (fun (j:Z) =>
          (infix_asdtdt (get x j 0%Z)
           (mat_sum (to_fset 0%Z 2%Z)
            (fun (i:Z) =>
             (infix_asdtdt
              (infix_pldt (infix_asdt (indic i j) (cos_d (div_two o)))
               (infix_asdt
                (infix_asdt (indic i (1%Z - j)%Z)
                 (cpower (prefix_mndt tone) j))
                (sin_d (div_two o))))
              (ket 1%Z i))))))))))))).
...
Axiom H : sequence_correct (Rx (o, o1)).

Axiom H1 : ((sum_range (Rx (o, o1))) = 1%Z).

Axiom H2 : ((depth (Rx (o, o1))) = 1%Z).

Parameter fc22: bitvec -> bitvec -> Z -> Z.

Axiom fc_def22 :
  forall (bvx:bitvec) (bvy:bitvec) (i:Z),
  ((i = 1%Z) -> (((fc22 bvx bvy) i) = (getbv bvy i))) /\
  (~ (i = 1%Z) -> (((fc22 bvx bvy) i) = (getbv bvx i))).

Axiom H3 :
  forall (bvx:bitvec) (bvy:bitvec),
  ((ket_part (Rx (o, o1)) bvx bvy) = (make_bv (fc22 bvx bvy) 1%Z)).

Axiom H4 :
  forall (bvx:bitvec) (bvy:bitvec), forall (i:Z),
  ((1%Z <= i)%Z /\ (i <= 1%Z)%Z) ->
  ((getbv (ket_part (Rx (o, o1)) bvx bvy) i) = (getbv bvy i)).

Axiom H5 :
  forall (bvx:bitvec) (bvy:bitvec),
  ((phase_part (Rx (o, o1)) bvx bvy) =
   (infix_asdt squarert_two
    (infix_mndt
     (infix_asdt (indic (getbv bvx 1%Z) (getbv bvy 1%Z))
      (cos_d (div_two (o, o1))))
...
Axiom fc_def23 :
  forall (i1:Z),
  ((i1 = 1%Z) ->
   ((fc23 i1) = (getbv (int_to_bv a1 (sum_range (Rx (o, o1)))) i1))) /\
  (~ (i1 = 1%Z) ->
   ((fc23 i1) = (getbv (int_to_bv a (depth (Rx (o, o1)))) i1))).

Axiom H10 : valid_index (bv_to_ket (make_bv fc23 1%Z)) i j.
...
Axiom fc_def24 :
  forall (i11:Z),
  ((i11 = 1%Z) -> ((fc24 i11) = (getbv (int_to_bv a1 1%Z) i11))) /\
  (~ (i11 = 1%Z) -> ((fc24 i11) = (getbv (int_to_bv a 1%Z) i11))).

Axiom H12 : (i1 <= (length (make_bv fc24 1%Z)))%Z.

Parameter fc25: Z -> Z.

Axiom fc_def25 :
  forall (i11:Z),
  ((i11 = 1%Z) -> ((fc25 i11) = (getbv (int_to_bv a1 1%Z) i11))) /\
  (~ (i11 = 1%Z) -> ((fc25 i11) = (getbv (int_to_bv a 1%Z) i11))).

(* Why3 goal *)
Theorem G :
  (((1%Z <= i1)%Z /\ (i1 <= 1%Z)%Z) ->
   ((fc25 i1) = (getbv (ket_to_bv (ket 1%Z a1)) i1))) /\
  (~ ((1%Z <= i1)%Z /\ (i1 <= 1%Z)%Z) ->
   (0%Z = (getbv (ket_to_bv (ket 1%Z a1)) i1))).
Proof.
