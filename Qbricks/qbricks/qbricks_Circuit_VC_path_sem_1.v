...
Axiom hops : Type.
Parameter hops_WhyType : WhyType hops.
Existing Instance hops_WhyType.

Parameter h_width: hops -> Z.

Parameter h_range: hops -> Z.

Parameter h_basis_ket: hops -> (Z -> Z) -> (Z -> Z) -> Z -> Z.

Parameter h_angle: hops -> (Z -> Z) -> (Z -> Z) -> angle.

Axiom hops'invariant :
  forall (self:hops),
  (0%Z <= (h_width self))%Z /\
  ((0%Z <= (h_range self))%Z /\
...
Axiom hops_ket_bound :
  forall (h:hops) (x:Z -> Z) (y:Z -> Z) (i:Z),
  (0%Z <= ((((h_basis_ket h) x) y) i))%Z /\
  (((((h_basis_ket h) x) y) i) < 2%Z)%Z.

Axiom low_hops_ket_bound :
  forall (h:hops) (x:Z -> Z) (y:Z -> Z) (i:Z),
  (0%Z <= ((((h_basis_ket h) x) y) i))%Z.

Axiom high_hops_ket_bound :
  forall (h:hops) (x:Z -> Z) (y:Z -> Z) (i:Z),
  (((((h_basis_ket h) x) y) i) < 2%Z)%Z.

Parameter hops_apply_basis: hops -> bitvec -> matrix complex.

Axiom hops_apply_basis_def :
  forall (h:hops) (x:bitvec), ((length x) = (h_width h)) ->
  ((hops_apply_basis h x) =
...
Axiom hops_apply_basis_spec :
  forall (h:hops) (x:bitvec), ((length x) = (h_width h)) ->
  (is_a_ket_l (hops_apply_basis h x) (h_width h)) /\
  forall (i:Z), ((0%Z <= i)%Z /\ (i < (power_ 2%Z (h_width h)))%Z) ->
  ((get (hops_apply_basis h x) i 0%Z) =
...
Parameter hops_apply: hops -> (matrix complex) -> matrix complex.

Parameter result18: hops -> (matrix complex) -> bitvec -> matrix complex.

Axiom result_def18 :
  forall (h:hops) (x:matrix complex) (y:bitvec),
  (((length y) = (h_width h)) ->
   (((result18 h x) y) =
    (infix_asdtdt (get x (bv_to_int y) 0%Z) (hops_apply_basis h y)))) /\
  (~ ((length y) = (h_width h)) -> (((result18 h x) y) = x)).

Axiom hops_apply_def :
  forall (h:hops) (x:matrix complex), (is_a_ket_l x (h_width h)) ->
  ((hops_apply h x) =
   (ket_sum_l (n_bvs (h_width h)) (result18 h x) (h_width h))).

Axiom hops_apply_spec :
  forall (h:hops) (x:matrix complex), (is_a_ket_l x (h_width h)) ->
  is_a_ket_l (hops_apply h x) (h_width h).

Axiom get_hops_apply :
  forall (h:hops) (x:matrix complex), (is_a_ket_l x (h_width h)) ->
  ((hops_apply h x) =
...
Axiom rewrite_hops_apply_basis :
  forall (h:hops) (x:matrix complex), (is_a_ket_l x (h_width h)) ->
  (is_a_ket_basis_elt x) ->
  ((hops_apply h x) = (hops_apply_basis h (ket_to_bv x))).

Axiom hops_apply_decomp :
  forall (h:hops) (x:matrix complex), (is_a_ket_l x (h_width h)) ->
  ((hops_apply h x) =
   (ket_sum_l (n_bvs (h_width h))
    (fun (z:bitvec) =>
     (infix_asdtdt (get x (bv_to_int z) 0%Z) (hops_apply h (bv_to_ket z))))
    (h_width h))).
...
Parameter circ_to_hops: circuit -> hops.

Axiom circ_to_hops_spec :
  forall (c:circuit),
  ((h_width (circ_to_hops c)) = (width c)) /\
  (((h_range (circ_to_hops c)) = (range c)) /\
   (((h_basis_ket (circ_to_hops c)) =
     ((fun (y0:circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
       (basis_ket y0 y1 y2 y3))
      c)) /\
    ((h_angle (circ_to_hops c)) =
     ((fun (y0:circuit) (y1:Z -> Z) (y2:Z -> Z) => (ang_ind y0 y1 y2)) c)))).
...
Axiom path_sem_basis_spec :
  ...
  ((path_sem_basis c x) = (hops_apply_basis (circ_to_hops c) (ket_to_bv x))).

Parameter c: circuit.

Parameter x: matrix complex.

Axiom H : is_a_ket_l x (width c).

Axiom H1 : (0%Z <= (width c))%Z.

Parameter y: bitvec.

Axiom H2 : is_a_ket_basis_elt (bv_to_ket y).

Axiom H3 : ((ket_to_int (bv_to_ket y)) = (bv_to_int y)).

Axiom H4 : is_a_ket_l (bv_to_ket y) (length y).

Axiom H5 : ((rows (bv_to_ket y)) = (power 2%Z (length y))).

Axiom H6 : ((columns (bv_to_ket y)) = 1%Z).

Axiom H7 : ((bv_to_ket y) = (bin_to_ket (length y) (value y))).

(* Why3 goal *)
Theorem VC_path_sem : is_a_ket_l (bv_to_ket y) (width c).
Proof.
