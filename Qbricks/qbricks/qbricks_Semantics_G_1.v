...
Parameter value: d_frac -> t.

Axiom d_frac'invariant : forall (self:d_frac), ~ ((value self) = tzero).

Axiom equal_d_frac :
  forall (d:d_frac) (d':d_frac), (d = d') <-> ((value d) = (value d')).

Parameter dyadic: Z -> Z -> d_frac.

Axiom dyadic_spec :
  forall (k:Z) (n:Z), (0%Z <= n)%Z -> ((value (dyadic k n)) = (p_omega n k)).

Parameter add1: d_frac -> d_frac -> d_frac.

Axiom add_spec1 :
  forall (d:d_frac) (d':d_frac),
  ((value (add1 d d')) = (infix_asdt (value d) (value d'))).

Parameter inv1: d_frac -> d_frac.

Axiom inv_spec :
  forall (d:d_frac), ((value (inv1 d)) = (infix_sldt tone (value d))).

Parameter div_two: d_frac -> d_frac.

Axiom div_two_spec :
  forall (d:d_frac),
  ((cpower (value (div_two d)) 2%Z) = (value d)) /\
  forall (k:Z) (n:Z), ((0%Z <= k)%Z /\ (k < (power 2%Z n))%Z) ->
  (0%Z <= n)%Z -> (d = (dyadic k n)) ->
  ((div_two d) = (dyadic k (n + 1%Z)%Z)).

Axiom value_zero : ((value (dyadic 0%Z 0%Z)) = tone).

Parameter d_mult_int: d_frac -> Z -> d_frac.

Axiom d_mult_int_spec :
  forall (d:d_frac) (i:Z), ((value (d_mult_int d i)) = (cpower (value d) i)).
...
(* Why3 assumption *)
Definition d_cos (o:d_frac) : t := t_real_part (value o).

Axiom d_cos_spec : forall (o:d_frac), real_ (d_cos o).

(* Why3 assumption *)
Definition d_sin (o:d_frac) : t := t_im_part (value o).
...
Axiom omega_decomp :
  forall (o:d_frac),
  ((value o) = (infix_pldt (d_cos o) (infix_asdt im (d_sin o)))).

Axiom d_cos_eq :
  forall (d1:d_frac) (d2:d_frac), ((value d1) = (value d2)) ->
  ((d_cos d1) = (d_cos d2)).
...
Inductive quantum_circuit_pre :=
  | Phase_ : d_frac -> quantum_circuit_pre
  | Rx : d_frac -> quantum_circuit_pre
  | Ry : d_frac -> quantum_circuit_pre
  | Rz_ : d_frac -> quantum_circuit_pre
  | Cnot_ : quantum_circuit_pre
  | Sequence_ : quantum_circuit_pre -> quantum_circuit_pre ->
      quantum_circuit_pre
  | Parallel_ : quantum_circuit_pre -> quantum_circuit_pre ->
      quantum_circuit_pre.
...
Fixpoint depth_pre (c:quantum_circuit_pre) {struct c}: Z :=
  match c with
  | Cnot_ => 2%Z
  | Sequence_ d _ => depth_pre d
  | Parallel_ d e => ((depth_pre d) + (depth_pre e))%Z
  | _ => 1%Z
  end.
...
Fixpoint build_correct (c:quantum_circuit_pre) {struct c}: Prop :=
  ...
  | Parallel_ d e => (build_correct d) /\ (build_correct e)
  | _ => True
  end.
...
Axiom quantum_circuit'invariant :
  forall (self:quantum_circuit), build_correct (to_pre self).
...
(* Why3 assumption *)
Definition depth_ (c:quantum_circuit) : Z := depth_pre (to_pre c).

Axiom depth__spec : forall (c:quantum_circuit), (0%Z < (depth_ c))%Z.

Parameter sum_range_: quantum_circuit -> Z.

Axiom sum_range__def :
  forall (c:quantum_circuit),
  match to_pre c with
  | Phase_ _ => ((sum_range_ c) = 0%Z)
  | Rx _ => ((sum_range_ c) = 1%Z)
  | Ry _ => ((sum_range_ c) = 1%Z)
  | Rz_ _ => ((sum_range_ c) = 0%Z)
  | Cnot_ => ((sum_range_ c) = 0%Z)
  | Sequence_ d e =>
      ((sum_range_ c) = ((sum_range_ (to_qc d)) + (sum_range_ (to_qc e)))%Z)
  | Parallel_ d e =>
      ((sum_range_ c) = ((sum_range_ (to_qc d)) + (sum_range_ (to_qc e)))%Z)
  end.

Axiom sum_range__spec :
  forall (c:quantum_circuit), (0%Z <= (sum_range_ c))%Z.

Parameter ket_part_: quantum_circuit -> (Z -> Z) -> (Z -> Z) -> Z -> Z.

Axiom ket_part__def :
  forall (c:quantum_circuit) (bvx:Z -> Z) (bvy:Z -> Z) (i:Z),
  match to_pre c with
  | Phase_ _ =>
      ((i = 0%Z) -> ((ket_part_ c bvx bvy i) = (bvx i))) /\
      (~ (i = 0%Z) -> ((ket_part_ c bvx bvy i) = 0%Z))
  | Rx _ =>
      ((i = 0%Z) -> ((ket_part_ c bvx bvy i) = (bvy i))) /\
      (~ (i = 0%Z) -> ((ket_part_ c bvx bvy i) = 0%Z))
  | Ry _ =>
      ((i = 0%Z) -> ((ket_part_ c bvx bvy i) = (bvy i))) /\
      (~ (i = 0%Z) -> ((ket_part_ c bvx bvy i) = 0%Z))
...
Axiom ket_part__spec :
  forall (c:quantum_circuit) (bvx:Z -> Z) (bvy:Z -> Z) (i:Z),
  ((binary bvx) -> (binary bvy) ->
   (0%Z <= (ket_part_ c bvx bvy i))%Z /\ ((ket_part_ c bvx bvy i) < 2%Z)%Z) /\
  (((i < 0%Z)%Z -> ((ket_part_ c bvx bvy i) = 0%Z)) /\
   (((depth_ c) <= i)%Z -> ((ket_part_ c bvx bvy i) = 0%Z))).
...
Parameter phase_part_: quantum_circuit -> (Z -> Z) -> (Z -> Z) -> t.

Axiom phase_part__def :
  forall (c:quantum_circuit) (bvx:Z -> Z) (bvy:Z -> Z),
  match to_pre c with
  | Phase_ o => ((phase_part_ c bvx bvy) = (value o))
  | Rx o =>
      (((bvx 0%Z) = (bvy 0%Z)) ->
       ((phase_part_ c bvx bvy) = (infix_asdt squarert_two (d_cos o)))) /\
      (~ ((bvx 0%Z) = (bvy 0%Z)) ->
       ((phase_part_ c bvx bvy) =
  ...
  | Rz_ o =>
      ((phase_part_ c bvx bvy) = (value (phase_inv (1%Z - (bvx 0%Z))%Z o)))
  | Cnot_ => ((phase_part_ c bvx bvy) = tone)
  | Sequence_ d e =>
      ((phase_part_ c bvx bvy) =
       (infix_asdt (phase_part_ (to_qc d) bvx bvy)
        (phase_part_ (to_qc e)
         ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
             (ket_part_ y0 y1 y2 y3))
            (to_qc d))
...
Parameter phase_: d_frac -> quantum_circuit.

Axiom phase__spec :
  forall (o:d_frac),
  ((sum_range_ (phase_ o)) = 0%Z) /\
  (((depth_ (phase_ o)) = 1%Z) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
     ((0%Z <= i)%Z /\ (i < (depth_ (phase_ o)))%Z) ->
     ((ket_part_ (phase_ o) bvx bvy i) = (bvx i))) /\
    ((forall (bvx:Z -> Z) (bvy:Z -> Z),
      ((phase_part_ (phase_ o) bvx bvy) = (value o))) /\
     ((to_pre (phase_ o)) = (Phase_ o))))).

Parameter rz_: d_frac -> quantum_circuit.

Axiom rz__spec :
  forall (o:d_frac),
  ((sum_range_ (rz_ o)) = 0%Z) /\
  (((depth_ (rz_ o)) = 1%Z) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
     ((0%Z <= i)%Z /\ (i < (depth_ (rz_ o)))%Z) ->
     ((ket_part_ (rz_ o) bvx bvy i) = (bvx i))) /\
    ((forall (bvx:Z -> Z) (bvy:Z -> Z),
      ((phase_part_ (rz_ o) bvx bvy) =
       (value (phase_inv (1%Z - (bvx 0%Z))%Z o)))) /\
     ((to_pre (rz_ o)) = (Rz_ o))))).
...
Axiom ry_spec :
  ...
  (((depth_ (ry o)) = 1%Z) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
     ((0%Z <= i)%Z /\ (i < (depth_ (ry o)))%Z) ->
     ((ket_part_ (ry o) bvx bvy i) = (bvy i))) /\
    ((forall (bvx:Z -> Z) (bvy:Z -> Z),
      (((bvx 0%Z) = (bvy 0%Z)) ->
       ((phase_part_ (ry o) bvx bvy) = (infix_asdt squarert_two (d_cos o)))) /\
      (~ ((bvx 0%Z) = (bvy 0%Z)) ->
       ((phase_part_ (ry o) bvx bvy) =
        (infix_asdt squarert_two
         (infix_asdt (cpower (prefix_mndt tone) (bvx 0%Z)) (d_sin o)))))) /\
     ((to_pre (ry o)) = (Ry o))))).
...
Axiom rx_spec :
  ...
  (((depth_ (rx o)) = 1%Z) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
     ((0%Z <= i)%Z /\ (i < (depth_ (rx o)))%Z) ->
     ((ket_part_ (rx o) bvx bvy i) = (bvy i))) /\
    ((forall (bvx:Z -> Z) (bvy:Z -> Z),
      (((bvx 0%Z) = (bvy 0%Z)) ->
       ((phase_part_ (rx o) bvx bvy) = (infix_asdt squarert_two (d_cos o)))) /\
      (~ ((bvx 0%Z) = (bvy 0%Z)) ->
       ((phase_part_ (rx o) bvx bvy) =
        (infix_asdt squarert_two (infix_asdt (prefix_mndt im) (d_sin o)))))) /\
     ((to_pre (rx o)) = (Rx o))))).

Parameter cnot_: unit -> quantum_circuit.

Axiom cnot__spec :
  forall (us:unit),
  ((sum_range_ (cnot_ us)) = 0%Z) /\
  (((depth_ (cnot_ us)) = 2%Z) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
     ((0%Z <= i)%Z /\ (i < (depth_ (cnot_ us)))%Z) ->
     ((i = 1%Z) ->
      ((ket_part_ (cnot_ us) bvx bvy i) =
       (int.EuclideanDivision.mod1 ((bvx 0%Z) + (bvx 1%Z))%Z 2%Z))) /\
     (~ (i = 1%Z) -> ((ket_part_ (cnot_ us) bvx bvy i) = (bvx i)))) /\
    ((forall (bvx:Z -> Z) (bvy:Z -> Z),
      ((phase_part_ (cnot_ us) bvx bvy) = tone)) /\
     ((to_pre (cnot_ us)) = Cnot_)))).

Parameter parallel_: quantum_circuit -> quantum_circuit -> quantum_circuit.

Axiom parallel__spec :
  forall (d:quantum_circuit) (e:quantum_circuit),
  ((depth_ (parallel_ d e)) = ((depth_ d) + (depth_ e))%Z) /\
  (((sum_range_ (parallel_ d e)) = ((sum_range_ d) + (sum_range_ e))%Z) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
     ((0%Z <= i)%Z /\ (i < (depth_ d))%Z) ->
     ((ket_part_ (parallel_ d e) bvx bvy i) = (ket_part_ d bvx bvy i))) /\
    ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
      (((depth_ d) <= i)%Z /\ (i < ((depth_ d) + (depth_ e))%Z)%Z) ->
      ((ket_part_ (parallel_ d e) bvx bvy i) =
       (ket_part_ e
        (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvx)
         (depth_ d))
        (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvy)
        ...
        (i - (depth_ d))%Z))) /\
     ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
       ((0%Z <= i)%Z /\ (i < (depth_ (parallel_ d e)))%Z) ->
       ((ket_part_ (parallel_ d e) bvx bvy i) =
        (concat_fun
         ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
             (ket_part_ y0 y1 y2 y3))
         ...
         (depth_ d) i))) /\
      ((forall (bvx:Z -> Z) (bvy:Z -> Z),
        ((phase_part_ (parallel_ d e) bvx bvy) =
         (infix_asdt (phase_part_ d bvx bvy)
          (phase_part_ e
           (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvx)
            (depth_ d))
           (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvy)
            (sum_range_ d)))))) /\
       ((to_pre (parallel_ d e)) = (Parallel_ (to_pre d) (to_pre e)))))))).

Parameter sequence_: quantum_circuit -> quantum_circuit -> quantum_circuit.

Axiom sequence__spec :
  forall (d:quantum_circuit) (e:quantum_circuit),
  ((depth_ d) = (depth_ e)) ->
  ((sum_range_ (sequence_ d e)) = ((sum_range_ d) + (sum_range_ e))%Z) /\
  (((depth_ (sequence_ d e)) = (depth_ d)) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
     ((0%Z <= i)%Z /\ (i < (depth_ (sequence_ d e)))%Z) ->
     ((ket_part_ (sequence_ d e) bvx bvy i) =
      (ket_part_ e
       ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
           (ket_part_ y0 y1 y2 y3))
          d)
      ...
      (((0%Z <= i)%Z /\ (i < (depth_ (sequence_ d e)))%Z) ->
       ((ket_part_ (sequence_ d e) bvx bvy i) =
        (ket_part_ e
         ...
         i))) /\
      (~ ((0%Z <= i)%Z /\ (i < (depth_ (sequence_ d e)))%Z) ->
       ((ket_part_ (sequence_ d e) bvx bvy i) = 0%Z))) /\
     ((forall (bvx:Z -> Z) (bvy:Z -> Z),
       ((phase_part_ (sequence_ d e) bvx bvy) =
        (infix_asdt (phase_part_ d bvx bvy)
         (phase_part_ e
          ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
              (ket_part_ y0 y1 y2 y3))
          ...
          (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvy)
           (sum_range_ d)))))) /\
      (((to_pre (sequence_ d e)) = (Sequence_ (to_pre d) (to_pre e))) /\
       ((sequence_ d e) = (to_qc (Sequence_ (to_pre d) (to_pre e))))))))).

Parameter rzp_: Z -> quantum_circuit.

Axiom rzp__def :
  forall (n:Z), (0%Z <= n)%Z ->
  ((rzp_ n) =
   (sequence_ (phase_ (dyadic 1%Z (n + 1%Z)%Z))
    (rz_ (dyadic 1%Z (n + 1%Z)%Z)))).

Axiom rzp__spec :
  forall (n:Z), (0%Z <= n)%Z ->
  ((depth_ (rzp_ n)) = 1%Z) /\
  (((sum_range_ (rzp_ n)) = 0%Z) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
     ((0%Z <= i)%Z /\ (i < (depth_ (rzp_ n)))%Z) ->
     ((ket_part_ (rzp_ n) bvx bvy i) = (bvx i))) /\
    forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) ->
    ((phase_part_ (rzp_ n) bvx bvy) = (value (dyadic (bvx 0%Z) n))))).

Axiom omega_d13 :
  ((value (dyadic 1%Z 3%Z)) =
   (infix_pldt (infix_sldt squarert_two (infix_pldt tone tone))
    (infix_asdt im (infix_sldt squarert_two (infix_pldt tone tone))))).
...
Axiom omega_d0n :
  forall (n:Z), (0%Z <= n)%Z -> ((value (dyadic 0%Z n)) = tone).

Axiom hadamard__spec :
  ((depth_ (sequence_ (rzp_ 1%Z) (ry (dyadic 1%Z 3%Z)))) = 1%Z) /\
  (((sum_range_ (sequence_ (rzp_ 1%Z) (ry (dyadic 1%Z 3%Z)))) = 1%Z) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
     ((0%Z <= i)%Z /\
      (i < (depth_ (sequence_ (rzp_ 1%Z) (ry (dyadic 1%Z 3%Z)))))%Z) ->
     ((ket_part_ (sequence_ (rzp_ 1%Z) (ry (dyadic 1%Z 3%Z))) bvx bvy i) =
      (bvy i))) /\
    forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
    ((phase_part_ (sequence_ (rzp_ 1%Z) (ry (dyadic 1%Z 3%Z))) bvx bvy) =
     (value (dyadic ((bvx 0%Z) * (bvy 0%Z))%Z 1%Z))))).

Axiom to_pre_to_qc : forall (c:quantum_circuit), ((to_qc (to_pre c)) = c).

Axiom ket_part_equiv_ :
  forall (c:quantum_circuit) (bvx:Z -> Z) (bvx':Z -> Z) (bvy:Z -> Z)
    (bvy':Z -> Z),
  (forall (i:Z), ((0%Z <= i)%Z /\ (i < (depth_ c))%Z) ->
   ((bvx i) = (bvx' i))) ->
  (forall (j:Z), ((0%Z <= j)%Z /\ (j < (sum_range_ c))%Z) ->
   ((bvy j) = (bvy' j))) ->
  (((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
       (ket_part_ y0 y1 y2 y3))
      c)
...
Axiom phase_part_equiv_ :
  ...
  (forall (j:Z), ((0%Z <= j)%Z /\ (j < (sum_range_ c))%Z) ->
   ((bvy j) = (bvy' j))) ->
  ((phase_part_ c bvx bvy) = (phase_part_ c bvx' bvy')).

Axiom ket_part_equiv_i_ :
  forall (c:quantum_circuit) (bvx:Z -> Z) (bvx':Z -> Z) (bvy:Z -> Z)
    (bvy':Z -> Z) (i:Z),
  (forall (i1:Z), ((0%Z <= i1)%Z /\ (i1 < (depth_ c))%Z) ->
   ((bvx i1) = (bvx' i1))) ->
  (forall (j:Z), ((0%Z <= j)%Z /\ (j < (sum_range_ c))%Z) ->
   ((bvy j) = (bvy' j))) ->
  ((ket_part_ c bvx bvy i) = (ket_part_ c bvx' bvy' i)).

Parameter flat_: quantum_circuit -> Prop.
...
Axiom flat__def :
  ...
  | Rz_ _ => flat_ c
  | Cnot_ => flat_ c
  | Sequence_ d e => (flat_ c) <-> ((flat_ (to_qc d)) /\ (flat_ (to_qc e)))
  | Parallel_ d e => (flat_ c) <-> ((flat_ (to_qc d)) /\ (flat_ (to_qc e)))
  | _ => ~ (flat_ c)
  end.

Axiom flat__spec :
  forall (c:quantum_circuit), (flat_ c) <-> ((sum_range_ c) = 0%Z).

Parameter flat_ket_: quantum_circuit -> (Z -> Z) -> Z -> Z.

Axiom flat_ket__def :
  forall (c:quantum_circuit) (bvx:Z -> Z) (i:Z), (flat_ c) ->
  match to_pre c with
  | Phase_ _ =>
      ((i = 0%Z) -> ((flat_ket_ c bvx i) = (bvx i))) /\
      (~ (i = 0%Z) -> ((flat_ket_ c bvx i) = 0%Z))
  | Rz_ _ =>
      ((i = 0%Z) -> ((flat_ket_ c bvx i) = (bvx i))) /\
      (~ (i = 0%Z) -> ((flat_ket_ c bvx i) = 0%Z))
  | Cnot_ =>
  ...
  | Sequence_ d e =>
      ((flat_ket_ c bvx i) =
       (flat_ket_ (to_qc e)
        (((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z) =>
           (flat_ket_ y0 y1 y2))
          (to_qc d))
         bvx)
        i))
       ...
       (concat_fun
        ...
        (((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z) =>
         ...
         (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvx)
          (depth_ (to_qc d))))
        (depth_ (to_qc d)) i))
  | _ => ((flat_ket_ c bvx i) = 0%Z)
  end.

Axiom flat_ket__spec :
  forall (c:quantum_circuit) (bvx:Z -> Z) (i:Z), (flat_ c) ->
  forall (bvy:Z -> Z), ((flat_ket_ c bvx i) = (ket_part_ c bvx bvy i)).

Parameter flat_phase_: quantum_circuit -> (Z -> Z) -> t.

Axiom flat_phase__def :
  forall (c:quantum_circuit) (bvx:Z -> Z), (flat_ c) ->
  match to_pre c with
  | Phase_ o => ((flat_phase_ c bvx) = (value o))
  | Rz_ o =>
      ((flat_phase_ c bvx) = (value (phase_inv (1%Z - (bvx 0%Z))%Z o)))
  | Cnot_ => ((flat_phase_ c bvx) = (value (dyadic 0%Z 0%Z)))
  | Parallel_ d e =>
      ((flat_phase_ c bvx) =
       (infix_asdt (flat_phase_ (to_qc d) bvx)
        (flat_phase_ (to_qc e)
         (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvx)
          (depth_ (to_qc d))))))
  | Sequence_ d e =>
       ...
       (infix_asdt (flat_phase_ (to_qc d) bvx)
        (flat_phase_ (to_qc e)
         (((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z) =>
           ...
           (to_qc d))
          bvx))))
  | _ => ((flat_phase_ c bvx) = (value (dyadic 0%Z 0%Z)))
  end.

Axiom flat_phase__spec :
  forall (c:quantum_circuit) (bvx:Z -> Z), (flat_ c) -> forall (bvy:Z -> Z),
  ((phase_part_ c bvx bvy) = (flat_phase_ c bvx)).

Axiom Positive_a_depth_ : forall (c:quantum_circuit), (0%Z < (depth_ c))%Z.

Axiom a_binary_ket_part_ :
  forall (c:quantum_circuit), forall (bvx:Z -> Z) (bvy:Z -> Z),
  (binary bvx) -> (binary bvy) ->
  binary
  ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
      (ket_part_ y0 y1 y2 y3))
...
Axiom seq_pres_a_flat_ :
  forall (d:quantum_circuit) (e:quantum_circuit), (flat_ d) -> (flat_ e) ->
  ((depth_ d) = (depth_ e)) -> flat_ (sequence_ d e).
...
Axiom A_Flat_ket_equiv_ :
  forall (c:quantum_circuit), forall (bvx:Z -> Z) (bvx':Z -> Z), (flat_ c) ->
  (forall (i:Z), ((0%Z <= i)%Z /\ (i < (depth_ c))%Z) ->
   ((bvx i) = (bvx' i))) ->
  ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z) => (flat_ket_ y0 y1 y2)) c)
    bvx)
   =
   (((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z) => (flat_ket_ y0 y1 y2)) c)
    bvx')).
...
Axiom A_Flat_a_phase_part_equiv_ :
  ...
  (forall (i:Z), ((0%Z <= i)%Z /\ (i < (depth_ c))%Z) ->
   ((bvx i) = (bvx' i))) ->
  ((flat_phase_ c bvx) = (flat_phase_ c bvx')).
...
Axiom fc_def6 :
  ...
  (((p j) = true) -> (((fc6 f p m) j) = (f j))) /\
  (~ ((p j) = true) -> (((fc6 f p m) j) = m)).
...
Axiom bitvec'invariant :
  ...
  ((forall (i:Z), ((0%Z <= i)%Z /\ (i < (length self))%Z) ->
    (0%Z <= ((value1 self) i))%Z /\ (((value1 self) i) < 2%Z)%Z) /\
   forall (i:Z), ~ ((0%Z <= i)%Z /\ (i < (length self))%Z) ->
   (((value1 self) i) = 0%Z)).
...
Axiom getbv_spec : forall (a:bitvec), binary (value1 a).

Axiom getbv_bound :
  forall (bv:bitvec) (i:Z),
  (0%Z <= ((value1 bv) i))%Z /\ (((value1 bv) i) < 2%Z)%Z.

Axiom getbv_eq :
  forall (bvx:bitvec) (bvy:bitvec) (i:Z), (bvx = bvy) ->
  (((value1 bvx) i) = ((value1 bvy) i)).

Axiom getbv_eq_gen :
  forall (bvx:bitvec) (bvy:bitvec) (i:Z) (j:Z), (bvx = bvy) -> (i = j) ->
  (((value1 bvx) i) = ((value1 bvy) j)).
...
Axiom set_bvlength :
  ...
  (forall (k:Z), ((1%Z <= k)%Z /\ (k < i)%Z) ->
   (0%Z <= ((value1 bv) k))%Z /\ (((value1 bv) k) < 2%Z)%Z) ->
  bvlength (value1 bv) i.
...
Axiom binary_bv :
  ...
  (0%Z <= ((value1 bv) i))%Z /\ (((value1 bv) i) <= 1%Z)%Z.

Axiom equal_bv :
  forall (m:bitvec) (n:bitvec),
  (m = n) <->
  (((length m) = (length n)) /\
   forall (i:Z), (in_range m i) -> (((value1 m) i) = ((value1 n) i))).

Axiom set_equal_bv :
  forall (bv:bitvec) (bv':bitvec), ((length bv) = (length bv')) ->
  (forall (i:Z), ((0%Z <= i)%Z /\ (i < (length bv))%Z) ->
   (((value1 bv) i) = ((value1 bv') i))) ->
  (bv = bv').
...
Axiom make_bv_spec :
  ...
  ((forall (i:Z), ((0%Z <= i)%Z /\ (i < s)%Z) ->
    (((value1 (make_bv f s)) i) = (f i))) /\
   forall (i:Z), ~ ((0%Z <= i)%Z /\ (i < s)%Z) ->
   (((value1 (make_bv f s)) i) = 0%Z)).
...
Axiom make_bv_m_spec :
  ...
  ((forall (i:Z), bvlength (value1 (make_bv_m f s)) i) /\
   ((forall (k:Z), ((0%Z <= k)%Z /\ (k < s)%Z) ->
     (((value1 (make_bv_m f s)) k) = (int.EuclideanDivision.mod1 (f k) 2%Z))) /\
    forall (k:Z), ~ ((0%Z <= k)%Z /\ (k < s)%Z) ->
    (((value1 (make_bv_m f s)) k) = 0%Z))).

Axiom assert_make_bv_no_bound :
  forall (f:Z -> Z) (s:Z) (i:Z),
  (forall (i1:Z), ((0%Z <= i1)%Z /\ (i1 < s)%Z) ->
   (0%Z <= (f i1))%Z /\ ((f i1) < 2%Z)%Z) ->
  ((1%Z <= i)%Z /\ (i <= s)%Z) -> (((value1 (make_bv f 0%Z)) i) = 0%Z).
...
Axiom assert_make_bv :
  ...
  (forall (i1:Z), ((0%Z <= i1)%Z /\ (i1 < s)%Z) ->
   (0%Z <= (f i1))%Z /\ ((f i1) < 2%Z)%Z) ->
  (0%Z <= s)%Z ->
  (((0%Z <= i)%Z /\ (i < s)%Z) -> (((value1 (make_bv f s)) i) = (f i))) /\
  (~ ((0%Z <= i)%Z /\ (i < s)%Z) -> (((value1 (make_bv f s)) i) = 0%Z)).

Axiom assert_make_m :
  forall (f:Z -> Z) (s:Z) (i:Z), (0%Z <= s)%Z ->
  (((0%Z <= i)%Z /\ (i < s)%Z) ->
   (((value1 (make_bv_m f s)) i) = (int.EuclideanDivision.mod1 (f i) 2%Z))) /\
  (~ ((0%Z <= i)%Z /\ (i < s)%Z) -> (((value1 (make_bv_m f s)) i) = 0%Z)).
...
Axiom make_m_bv_length :
  forall (f:Z -> Z) (s:Z) (i:Z), (0%Z <= s)%Z ->
  bvlength (value1 (make_bv_m f s)) i.
...
Axiom tail_def :
  forall (bv:bitvec), (1%Z <= (length bv))%Z ->
  ((tail bv) =
   (make_bv (fun (i:Z) => ((value1 bv) (i + 1%Z)%Z)) ((length bv) - 1%Z)%Z)).

Axiom tail_spec :
  forall (bv:bitvec), (1%Z <= (length bv))%Z ->
  ((length (tail bv)) = ((length bv) - 1%Z)%Z) /\
  forall (i:Z), (in_range (tail bv) i) ->
  (((value1 (tail bv)) i) = ((value1 bv) (i + 1%Z)%Z)).
...
Axiom result_def8 :
  ...
  (~ (k = 0%Z) -> (((result8 bv i) k) = ((value1 bv) (k - 1%Z)%Z))).
...
Axiom concat_l_spec :
  ...
  ((((value1 (concat_l bv i)) 0%Z) = i) /\
   forall (j:Z), (in_range bv j) ->
   (((value1 (concat_l bv i)) (j + 1%Z)%Z) = ((value1 bv) j))).

Axiom concat_l_value :
  forall (bv:bitvec) (i:Z) (k:Z), ((0%Z <= i)%Z /\ (i < 2%Z)%Z) ->
  ((k = 0%Z) -> (((value1 (concat_l bv i)) k) = i)) /\
  (~ (k = 0%Z) ->
   (((0%Z < k)%Z /\ (k <= ((length bv) + 1%Z)%Z)%Z) ->
    (((value1 (concat_l bv i)) k) = ((value1 bv) (k - 1%Z)%Z))) /\
   (~ ((0%Z < k)%Z /\ (k <= ((length bv) + 1%Z)%Z)%Z) ->
    (((value1 (concat_l bv i)) k) = 0%Z))).
...
Axiom concat_l_value_b :
  ...
  ((0%Z <= i)%Z /\ (i < 2%Z)%Z) ->
  ((k = 0%Z) -> (((value1 (concat_l bv i)) k) = i)) /\
  (~ (k = 0%Z) -> (((value1 (concat_l bv i)) k) = ((value1 bv) (k - 1%Z)%Z))).
...
Axiom result_def9 :
  ...
  ((in_range bv k) ->
   (((result9 bv) k) =
    (((value1 bv) k) * (power 2%Z (((length bv) - 1%Z)%Z - k)%Z))%Z)) /\
...
Axiom bv_to_int_onebit :
  ...
  ((bv_to_int bv) = ((value1 bv) 0%Z)).

Axiom ind_isum_bv_rev :
  forall (bv:bitvec) (i:Z), ((0%Z <= i)%Z /\ (i <= (length bv))%Z) ->
  ((ind_isum
    (fun (l:Z) =>
     (((value1 bv) l) * (power 2%Z (((length bv) - 1%Z)%Z - l)%Z))%Z)
    i (length bv))
   =
   (ind_isum
    (fun (l:Z) =>
     (((value1 bv) (((length bv) - 1%Z)%Z - l)%Z) * (power 2%Z l))%Z)
    0%Z ((length bv) - i)%Z)).
...
Axiom bv_to_int_sum_rev :
  ...
  ((ind_isum
    (fun (k:Z) =>
     (((value1 bv) (((length bv) - 1%Z)%Z - k)%Z) * (power 2%Z k))%Z)
    0%Z (length bv))
   = (bv_to_int bv)).

Axiom ind_isum_bv_bound_growing :
  forall (bv:bitvec) (i:Z), ((0%Z < i)%Z /\ (i < (length bv))%Z) ->
  ((ind_isum (fun (l:Z) => (((value1 bv) l) * (power 2%Z l))%Z) 0%Z i) <
   (power 2%Z i))%Z.
...
Axiom ind_isum_mod_div :
  ...
  ((int.EuclideanDivision.div
   ...
   =
   (ind_isum
    (fun (k:Z) => (((value1 bv) k) * (power 2%Z ((i - 1%Z)%Z - k)%Z))%Z) 0%Z
    i)).
...
Axiom ind_isum_mod :
  forall (bv:bitvec) (i:Z), ((0%Z <= i)%Z /\ (i < (length bv))%Z) ->
  ((int.EuclideanDivision.mod1
    (ind_isum (fun (l:Z) => (((value1 bv) l) * (power 2%Z (i - l)%Z))%Z) 0%Z
     (i + 1%Z)%Z)
    2%Z)
   = ((value1 bv) i)).
...
Axiom int_to_bv_sum_pre :
  ...
  ((int.EuclideanDivision.mod1 i (power 2%Z k)) =
   (ind_isum
    (fun (l:Z) =>
     (((value1 (int_to_bv i n)) l) * (power 2%Z ((n - l)%Z - 1%Z)%Z))%Z)
    (n - k)%Z n)).
...
Axiom int_to_bv_sum :
  ...
  (i =
   (ind_isum
    (fun (k:Z) =>
     (((value1 (int_to_bv i n)) k) * (power 2%Z ((n - 1%Z)%Z - k)%Z))%Z)
    0%Z n)).
...
Axiom mod_isum :
  ...
  ((int.EuclideanDivision.mod1 i (power 2%Z k)) =
   (ind_isum
    (fun (l:Z) =>
     (((value1 (int_to_bv i n)) l) * (power 2%Z ((n - 1%Z)%Z - l)%Z))%Z)
    (n - k)%Z n)).
...
Axiom mod_isum_z :
  ...
  ((int.EuclideanDivision.mod1 i (power 2%Z k)) =
   (ind_isum
    (fun (l:Z) =>
     (((value1 (int_to_bv i n)) (l + (n - k)%Z)%Z) *
...
Axiom int_to_sum_spec :
  ...
  (i =
   (ind_isum
    (fun (k:Z) =>
     (((value1 (int_to_bv i n)) k) * (power 2%Z ((n - k)%Z - 1%Z)%Z))%Z)
    0%Z n)).
...
Axiom int_to_bin_def :
  forall (i:Z) (n:Z), (0%Z <= n)%Z ->
  ((0%Z <= i)%Z /\ (i < (power 2%Z n))%Z) ->
  ((int_to_bin i n) = (value1 (int_to_bv i n))).
...
Axiom bound_sum_dec :
  ...
  ((ind_isum
    (fun (l:Z) => (((value1 bv) l) * (power 2%Z ((length bv) - l)%Z))%Z) i
    ((length bv) + 1%Z)%Z)
...
Axiom int_to_bv_to_int_value :
  ...
  (((0%Z <= i)%Z /\ (i < (length bv))%Z) ->
   (((value1 (int_to_bv (bv_to_int bv) (length bv))) i) = ((value1 bv) i))) /\
  (~ ((0%Z <= i)%Z /\ (i < (length bv))%Z) ->
   (((value1 (int_to_bv (bv_to_int bv) (length bv))) i) = 0%Z)).
...
Axiom set_n_bvs :
  ...
  ~ ((0%Z <= i)%Z /\ (i < (length bv))%Z) -> (((value1 bv) i) = 0%Z).
...
Axiom first_div_spec :
  ...
  (forall (j:Z), ((0%Z <= j)%Z /\ (j < (first_div bv1 bv2))%Z) ->
   (((value1 bv1) j) = ((value1 bv2) j))) /\
  ((((first_div bv1 bv2) < (length bv1))%Z ->
    ~ (((value1 bv1) (first_div bv1 bv2)) =
       ((value1 bv2) (first_div bv1 bv2)))) /\
...
Axiom inf_first_div :
  ...
  ~ (((value1 bv1) j) = ((value1 bv2) j)) -> ((first_div bv1 bv2) <= j)%Z.

Axiom first_div_diff :
  forall (bv1:bitvec) (bv2:bitvec), ((length bv1) = (length bv2)) ->
  ~ (bv1 = bv2) ->
  ~ (((value1 bv1) (first_div bv1 bv2)) = ((value1 bv2) (first_div bv1 bv2))).
...
Axiom set_diff_val :
  ...
  ~ (((value1 bv1) i) = ((value1 bv2) i)) -> ~ (bv1 = bv2).
...
Axiom kth_decomp :
  ...
  ((bv_to_int bv) =
   (((ind_isum (fc7 bv) 0%Z k) +
     (((value1 bv) k) * (power 2%Z (((length bv) - 1%Z)%Z - k)%Z))%Z)%Z
...
Axiom int_to_bv_prod :
  ...
  ((ind_product
    (fun (k:Z) =>
     (indic ((value1 (int_to_bv i n)) k) ((value1 (int_to_bv j n)) k)))
    0%Z n)
   = (indic i j)).
...
Axiom kth_decomp_bound_one :
  ...
  ((bv_to_int bv) <
   ((ind_isum
     (fun (l:Z) =>
      (((value1 bv) l) * (power 2%Z (((length bv) - l)%Z - 1%Z)%Z))%Z)
     0%Z k)
...
Axiom result_def11 :
  ...
  ((((length bv1) <= i)%Z /\ (i < ((length bv1) + (length bv2))%Z)%Z) ->
   (((result11 bv1 bv2) i) = ((value1 bv2) (i - (length bv1))%Z))) /\
  (~ (((length bv1) <= i)%Z /\ (i < ((length bv1) + (length bv2))%Z)%Z) ->
   (((result11 bv1 bv2) i) = ((value1 bv1) i))).
...
Axiom concat_spec :
  ...
  ((forall (i:Z), ((0%Z <= i)%Z /\ (i < (length bv1))%Z) ->
    (((value1 (concat bv1 bv2)) i) = ((value1 bv1) i))) /\
   ((forall (i:Z), ((length (concat bv1 bv2)) <= i)%Z ->
     (((value1 (concat bv1 bv2)) i) = ((value1 bv1) i))) /\
    forall (i:Z),
    (((length bv1) <= i)%Z /\ (i < (length (concat bv1 bv2)))%Z) ->
    (((value1 (concat bv1 bv2)) i) = ((value1 bv2) (i - (length bv1))%Z)))).
...
Axiom hpart_def :
  ...
  ((hpart bv m) = (make_bv (value1 bv) m)).

Axiom hpart_spec :
  forall (bv:bitvec) (m:Z), (0%Z <= m)%Z ->
  ((length (hpart bv m)) = m) /\
  ((forall (k:Z), ((0%Z <= k)%Z /\ (k < m)%Z) ->
    (((value1 (hpart bv m)) k) = ((value1 bv) k))) /\
   forall (k:Z), ~ ((0%Z <= k)%Z /\ (k < m)%Z) ->
   (((value1 (hpart bv m)) k) = 0%Z)).

Axiom hpart_value :
  forall (bv:bitvec) (m:Z) (i:Z), (0%Z <= m)%Z ->
  (((0%Z <= i)%Z /\ (i < m)%Z) ->
   (((value1 (hpart bv m)) i) = ((value1 bv) i))) /\
  (~ ((0%Z <= i)%Z /\ (i < m)%Z) -> (((value1 (hpart bv m)) i) = 0%Z)).
...
Axiom tpart_def :
  ...
  ((m <= (length bv))%Z ->
   ((tpart bv m) =
    (make_bv (fun (k:Z) => ((value1 bv) (k + m)%Z)) ((length bv) - m)%Z))) /\
  (~ (m <= (length bv))%Z ->
   ((tpart bv m) = (make_bv (fun (k:Z) => ((value1 bv) (k + m)%Z)) 0%Z))).
...
Axiom tpart_spec :
  ...
  ((((length bv) < m)%Z -> ((length (tpart bv m)) = 0%Z)) /\
   ...
   (((value1 (tpart bv m)) k) = ((value1 bv) (k + m)%Z))).

Axiom tpart_value :
  forall (bv:bitvec) (m:Z) (i:Z), (0%Z <= m)%Z ->
  (((0%Z <= i)%Z /\ (i < ((length bv) - m)%Z)%Z) ->
   (((value1 (tpart bv m)) i) = ((value1 bv) (i + m)%Z))) /\
  (~ ((0%Z <= i)%Z /\ (i < ((length bv) - m)%Z)%Z) ->
   (((value1 (tpart bv m)) i) = 0%Z)).
...
Axiom concat_value_tpart :
  ...
  (((value1 (concat bv1 (tpart bv2 (length bv1)))) i) = ((value1 bv2) i)).

Axiom concat_value_ht :
  forall (bv1:bitvec) (bv2:bitvec) (bv3:bitvec) (bv4:bitvec) (i:Z),
  ((0%Z <= i)%Z /\ (i < (length bv3))%Z) ->
  (((0%Z <= i)%Z /\ (i < (length bv1))%Z) ->
   (((value1 bv1) i) = ((value1 bv4) i))) ->
  (((length bv1) <= i)%Z -> (((value1 bv3) i) = ((value1 bv4) i))) ->
  (bv2 = (tpart bv3 (length bv1))) ->
  (((value1 (concat bv1 bv2)) i) = ((value1 bv4) i)).
...
Axiom set_concat :
  ...
  (forall (i:Z), ((0%Z <= i)%Z /\ (i < (length bv1))%Z) ->
   (((value1 bv1) i) = ((value1 bv4) i))) ->
  (forall (i:Z), ((length bv1) <= i)%Z ->
   (((value1 bv2) (i - (length bv1))%Z) = ((value1 bv4) i))) ->
  ((length bv4) = ((length bv1) + (length bv2))%Z) ->
  ((concat bv1 bv2) = bv4).
...
Axiom concat_m :
  ...
  ((concat (make_bv_m (value1 bv1) i1) (make_bv_m (value1 bv2) i2)) =
   (make_bv_m (value1 (concat bv1 bv2)) (i1 + i2)%Z)).
...
Axiom concat_int_to_bv_value :
  ...
  (j < (length bv))%Z ->
  (((value1 (concat_int_bv bv i n)) j) = ((value1 bv) j)).
   ...
   (((ket_length (bv_to_ket bv)) = (length bv)) /\
    (((rows (bv_to_ket bv)) = (power 2%Z (length bv))) /\
     (((columns (bv_to_ket bv)) = 1%Z) /\
      ((bv_to_ket bv) = (bin_to_ket (length bv) (value1 bv))))))).
...
Parameter unit_path_semantics: quantum_circuit -> (matrix t) -> matrix t.

Axiom unit_path_semantics_def :
  forall (c:quantum_circuit) (x:matrix t), (is_a_ket_basis_elt x) ->
  ((ket_length x) = (depth_ c)) -> (is_a_ket_basis_elt x) ->
  ((ket_length x) = (depth_ c)) ->
  ((unit_path_semantics c x) =
   (infix_asdtdt (pow_inv_sqrt_2 (sum_range_ c))
    (ket_sum_l (n_bvs (sum_range_ c))
     (fun (bvy:bitvec) =>
      (infix_asdtdt (phase_part_ c (value1 (ket_to_bv x)) (value1 bvy))
       (bin_to_ket (depth_ c)
        ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
            (ket_part_ y0 y1 y2 y3))
           c)
...
Axiom unit_path_semantics_spec :
  forall (c:quantum_circuit) (x:matrix t), (is_a_ket_basis_elt x) ->
  ((ket_length x) = (depth_ c)) -> (is_a_ket_basis_elt x) ->
  ((ket_length x) = (depth_ c)) ->
  is_a_ket_l (unit_path_semantics c x) (depth_ c).

Axiom unit_path_rows :
  forall (c:quantum_circuit) (x:matrix t), (is_a_ket_basis_elt x) ->
  ((ket_length x) = (depth_ c)) -> (is_a_ket_basis_elt x) ->
  ((ket_length x) = (depth_ c)) ->
  ((rows (unit_path_semantics c x)) = (power 2%Z (ket_length x))).

Parameter path_semantics: quantum_circuit -> (matrix t) -> matrix t.

Axiom path_semantics_def :
  forall (c:quantum_circuit) (x:matrix t), (is_a_ket_l x (depth_ c)) ->
  ((path_semantics c x) =
   (ket_sum_l (n_bvs (depth_ c))
    (fun (bvx:bitvec) =>
     (infix_asdtdt (get x (bv_to_int bvx) 0%Z)
      (unit_path_semantics c (bv_to_ket (make_bv (value1 bvx) (depth_ c))))))
    (depth_ c))).

Axiom path_semantics_spec :
  forall (c:quantum_circuit) (x:matrix t), (is_a_ket_l x (depth_ c)) ->
  (is_a_ket_l (path_semantics c x) (depth_ c)) /\
  ((path_semantics c x) =
   (ket_sum_l (n_bvs (depth_ c))
    (fun (bvx:bitvec) =>
     (infix_asdtdt (get x (bv_to_int bvx) 0%Z)
      (unit_path_semantics c (bv_to_ket bvx))))
    (depth_ c))).

Axiom path_semantics_kets :
  forall (c:quantum_circuit) (x:matrix t), (is_a_ket_l x (depth_ c)) ->
  ((path_semantics c x) =
   (ket_sum_l (n_bvs (depth_ c))
    (fun (bvx:bitvec) =>
     (infix_asdtdt (get x (bv_to_int bvx) 0%Z)
      (unit_path_semantics c (ket (ket_length x) (bv_to_int bvx)))))
    (depth_ c))).

Axiom path_semantics_unit :
  forall (c:quantum_circuit) (x:matrix t), (is_a_ket_basis_elt x) ->
  ((ket_length x) = (depth_ c)) ->
  ((path_semantics c x) = (unit_path_semantics c x)).
...
Axiom path_semantics_decomp :
  ...
  ((path_semantics c x) =
   (ket_sum_l (n_bvs (depth_ c))
    (fun (bvx:bitvec) =>
     (infix_asdtdt (get x (bv_to_int bvx) 0%Z)
      (path_semantics c (bv_to_ket bvx))))
    (depth_ c))).

Axiom add_path_semantics :
  forall (c:quantum_circuit) (x:matrix t) (y:matrix t),
  (is_a_ket_l x (depth_ c)) -> (is_a_ket_l y (depth_ c)) ->
  ((path_semantics c (add_ket_l x y (depth_ c))) =
   (add_ket_l (path_semantics c x) (path_semantics c y) (depth_ c))).

Axiom scal_path_semantics :
  forall (c:quantum_circuit) (x:matrix t) (a:t), (is_a_ket_l x (depth_ c)) ->
  ((path_semantics c (infix_asdtdt a x)) =
   (infix_asdtdt a (path_semantics c x))).

Axiom mat_sum_path_semantics :
  forall {a:Type} {a_WT:WhyType a},
  forall (c:quantum_circuit) (s:set a) (f:a -> matrix t),
  (0%Z < (cardinal s))%Z ->
  (forall (e:a), (mem e s) -> is_a_ket_l (f e) (depth_ c)) ->
  ((path_semantics c (ket_sum_l s f (depth_ c))) =
   (ket_sum_l s (fun (e:a) => (path_semantics c (f e))) (depth_ c))).

Axiom mat_sum_scal_path_semantics :
  forall {a:Type} {a_WT:WhyType a},
  forall (c:quantum_circuit) (s:set a) (scal:a -> t) (mat:a -> matrix t),
  (0%Z < (cardinal s))%Z ->
  (forall (e:a), (mem e s) -> is_a_ket_l (mat e) (depth_ c)) ->
  ((path_semantics c
    (ket_sum_l s (fun (e:a) => (infix_asdtdt (scal e) (mat e))) (depth_ c)))
   =
   (ket_sum_l s
    (fun (e:a) => (infix_asdtdt (scal e) (path_semantics c (mat e))))
    (depth_ c))).
...
Parameter d: quantum_circuit.

Parameter e: quantum_circuit.

Parameter x: matrix t.

Axiom H : is_a_ket_l x (depth_ d).

Axiom H1 : is_a_ket_basis_elt x.

Axiom H2 : ((depth_ d) = (depth_ e)).

Axiom H3 : ((path_semantics d x) = (unit_path_semantics d x)).

Parameter a: bitvec.

Axiom H4 : mem a (n_bvs (sum_range_ d)).

Parameter a1: bitvec.

Axiom H5 : mem a1 (n_bvs (sum_range_ e)).

Parameter i: Z.

Axiom H6 : (0%Z <= i)%Z.

Axiom H7 : (i < (depth_ e))%Z.

(* Why3 goal *)
Theorem G :
  (((value1
     (make_bv
      (fun (y3:Z) => (ket_part_ d (value1 (ket_to_bv x)) (value1 a) y3))
      (depth_ e)))
    i)
   = ((fun (y3:Z) => (ket_part_ d (value1 (ket_to_bv x)) (value1 a) y3)) i)).
Proof.
