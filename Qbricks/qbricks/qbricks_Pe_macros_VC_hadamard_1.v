...
Inductive quantum_circuit_pre :=
  | Phase : (Z* Z)%type -> quantum_circuit_pre
  | Rx : (Z* Z)%type -> quantum_circuit_pre
  | Ry : (Z* Z)%type -> quantum_circuit_pre
  | Rz : (Z* Z)%type -> quantum_circuit_pre
...
(* Why3 assumption *)
Fixpoint sequence_correct (c:quantum_circuit_pre) {struct c}: Prop :=
  match c with
  | Sequence d e =>
...
Axiom quantum_circuit'invariant :
  ...
  (sequence_correct (to_pre self)) /\
  ((depth self) = (depth_pre (to_pre self))).
...
Parameter phase: (Z* Z)%type -> quantum_circuit.

Axiom phase_spec : forall (o:(Z* Z)%type), ((to_pre (phase o)) = (Phase o)).

Parameter rx: (Z* Z)%type -> quantum_circuit.

Axiom rx_spec : forall (o:(Z* Z)%type), ((to_pre (rx o)) = (Rx o)).

Parameter ry: (Z* Z)%type -> quantum_circuit.

Axiom ry_spec : forall (o:(Z* Z)%type), ((to_pre (ry o)) = (Ry o)).

Parameter rz: (Z* Z)%type -> quantum_circuit.

Axiom rz_spec : forall (o:(Z* Z)%type), ((to_pre (rz o)) = (Rz o)).
...
Axiom to_qc_def :
  forall (c:quantum_circuit_pre), (sequence_correct c) ->
  match c with
  | Rx o => ((to_qc c) = (rx o))
  | Ry o => ((to_qc c) = (ry o))
  | Rz o => ((to_qc c) = (rz o))
...
Axiom reverse_pre_spec :
  forall (c:quantum_circuit_pre), (sequence_correct c) ->
  sequence_correct (reverse_pre c).
...
Axiom identity_def :
  forall (k:Z), (1%Z <= k)%Z ->
  ((identity k) = (repeat (phase (0%Z, 1%Z)) k)).
  ...
  | Rx o =>
      ((phase_part c bvx bvy) =
       (infix_asdt squarert_two
        (infix_mndt
         (infix_asdt (indic (bvx 1%Z) (bvy 1%Z)) (cos_d (div_two o)))
         (infix_asdt (infix_asdt (indic (bvx 1%Z) (1%Z - (bvy 1%Z))%Z) im)
          (sin_d (div_two o))))))
  | Ry o =>
      ((phase_part c bvx bvy) =
       (infix_asdt squarert_two
        (infix_pldt
         (infix_asdt (indic (bvx 1%Z) (bvy 1%Z)) (cos_d (div_two o)))
         (infix_asdt
          (infix_asdt (indic (bvx 1%Z) (1%Z - (bvy 1%Z))%Z)
           (cpower (prefix_mndt tone) (bvx 1%Z)))
          (sin_d (div_two o))))))
  | Rz o =>
      ((phase_part c bvx bvy) = (d_omega (phase_inv (bvx 1%Z) (div_two o))))
  ...
  (((depth (rz o)) = 1%Z) /\
   ((forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
    ...
    ((phase_part (rz o) bvx bvy) =
     (d_omega (phase_inv (bvx 1%Z) (div_two o)))))).
...
Axiom H : ((sum_range (rz (1%Z, 0%Z))) = 0%Z).

Axiom H1 : ((depth (rz (1%Z, 0%Z))) = 1%Z).

Axiom H2 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  ((ket_part (rz (1%Z, 0%Z)) bvx bvy 1%Z) = (bvx 1%Z)).

Axiom H3 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  ((phase_part (rz (1%Z, 0%Z)) bvx bvy) =
   (d_omega (phase_inv (bvx 1%Z) (div_two (1%Z, 0%Z))))).

Axiom H4 : ((sum_range (phase (1%Z, 1%Z))) = 0%Z).

Axiom H5 : ((depth (phase (1%Z, 1%Z))) = 1%Z).

Axiom H6 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  ((ket_part (phase (1%Z, 1%Z)) bvx bvy 1%Z) = (bvx 1%Z)).

Axiom H7 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  ((phase_part (phase (1%Z, 1%Z)) bvx bvy) = (d_omega (1%Z, 1%Z))).

Axiom H8 : ((sum_range (ry (1%Z, 1%Z))) = 1%Z).

Axiom H9 : ((depth (ry (1%Z, 1%Z))) = 1%Z).

Axiom H10 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  ((ket_part (ry (1%Z, 1%Z)) bvx bvy 1%Z) = (bvy 1%Z)).

Axiom H11 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  ((phase_part (ry (1%Z, 1%Z)) bvx bvy) =
   (infix_asdt squarert_two
    (infix_pldt
     (infix_asdt (indic (bvx 1%Z) (bvy 1%Z)) (cos_d (div_two (1%Z, 1%Z))))
     (infix_asdt
      (infix_asdt (indic (bvx 1%Z) (1%Z - (bvy 1%Z))%Z)
       (cpower (prefix_mndt tone) (bvx 1%Z)))
      (sin_d (div_two (1%Z, 1%Z))))))).

Axiom H12 : ((to_pre (phase (1%Z, 1%Z))) = (Phase (1%Z, 1%Z))).

Axiom H13 : ((to_pre (rz (1%Z, 0%Z))) = (Rz (1%Z, 0%Z))).

Axiom H14 :
  ((sum_range (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))) =
   ((sum_range (rz (1%Z, 0%Z))) + (sum_range (phase (1%Z, 1%Z))))%Z).

Axiom H15 :
  ((depth (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))) =
   (depth (rz (1%Z, 0%Z)))).

Axiom H16 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  forall (i:Z), ((1%Z <= i)%Z /\ (i <= (depth (rz (1%Z, 0%Z))))%Z) ->
  ((ket_part (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z))) bvx bvy i) =
   (ket_part (phase (1%Z, 1%Z))
    ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
        (ket_part y0 y1 y2 y3))
       (rz (1%Z, 0%Z)))
      bvx)
     bvy)
    (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvy)
     (sum_range (rz (1%Z, 0%Z))))
    i)).

Axiom H17 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  ((phase_part (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z))) bvx bvy) =
   (infix_asdt
    (phase_part (rz (1%Z, 0%Z)) bvx
     (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvy)
      (sum_range (rz (1%Z, 0%Z)))))
    (phase_part (phase (1%Z, 1%Z))
     ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
         (ket_part y0 y1 y2 y3))
        (rz (1%Z, 0%Z)))
       bvx)
...
Axiom H18 : ((to_pre (phase (1%Z, 1%Z))) = (Phase (1%Z, 1%Z))).

Axiom H19 : ((to_pre (rz (1%Z, 0%Z))) = (Rz (1%Z, 0%Z))).

Axiom H20 :
  ((to_pre (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))) =
   (Sequence (to_pre (rz (1%Z, 0%Z))) (to_pre (phase (1%Z, 1%Z))))).

Axiom H21 : ((to_pre (ry (1%Z, 1%Z))) = (Ry (1%Z, 1%Z))).

Axiom H22 :
  ((sum_range
    (sequence (ry (1%Z, 1%Z)) (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))))
   =
   ((sum_range (ry (1%Z, 1%Z))) +
    (sum_range (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))))%Z).
...
Axiom H24 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  forall (i:Z), ((1%Z <= i)%Z /\ (i <= (depth (ry (1%Z, 1%Z))))%Z) ->
  ((ket_part
    (sequence (ry (1%Z, 1%Z)) (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z))))
    bvx bvy i)
   =
   (ket_part (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))
    ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
        (ket_part y0 y1 y2 y3))
       (ry (1%Z, 1%Z)))
      bvx)
...
Axiom H25 :
  ...
  ((phase_part
   ...
   =
   (infix_asdt
    (phase_part (ry (1%Z, 1%Z)) bvx
     (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvy)
      (sum_range (ry (1%Z, 1%Z)))))
    (phase_part (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))
     ((((fun (y0:quantum_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
         (ket_part y0 y1 y2 y3))
        (ry (1%Z, 1%Z)))
       bvx)
...
Axiom H26 : ((to_pre (phase (1%Z, 1%Z))) = (Phase (1%Z, 1%Z))).

Axiom H27 : ((to_pre (rz (1%Z, 0%Z))) = (Rz (1%Z, 0%Z))).

Axiom H28 :
  ((to_pre (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))) =
   (Sequence (to_pre (rz (1%Z, 0%Z))) (to_pre (phase (1%Z, 1%Z))))).

Axiom H29 : ((to_pre (ry (1%Z, 1%Z))) = (Ry (1%Z, 1%Z))).

Axiom H30 :
  ((to_pre
    (sequence (ry (1%Z, 1%Z)) (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))))
   =
   (Sequence (to_pre (ry (1%Z, 1%Z)))
    (to_pre (sequence (rz (1%Z, 0%Z)) (phase (1%Z, 1%Z)))))).
...
Axiom H34 : binary bvx.

Axiom H35 : binary bvy.

(* Why3 goal *)
Theorem VC_hadamard :
  ((infix_asdt
    (phase_part (ry (1%Z, 1%Z)) bvx
     (fun (y2:Z) => (shift bvy (sum_range (ry (1%Z, 1%Z))) y2)))
    (infix_asdt
     (phase_part (rz (1%Z, 0%Z))
      (fun (y3:Z) =>
       (ket_part (ry (1%Z, 1%Z)) bvx
        (fun (y2:Z) => (shift bvy (sum_range (ry (1%Z, 1%Z))) y2)) y3))
      (fun (y2:Z) => (shift bvy (sum_range (rz (1%Z, 0%Z))) y2)))
     (phase_part (phase (1%Z, 1%Z))
      (fun (y3:Z) =>
       (ket_part (rz (1%Z, 0%Z))
        (fun (y31:Z) =>
         (ket_part (ry (1%Z, 1%Z)) bvx
          (fun (y2:Z) => (shift bvy (sum_range (ry (1%Z, 1%Z))) y2)) y31))
        (fun (y2:Z) => (shift bvy (sum_range (rz (1%Z, 0%Z))) y2)) y3))
      bvy)))
   = (p_omega 1%Z ((bvx 1%Z) * (bvy 1%Z))%Z)).
Proof.
