...
Axiom cpower_ttwo_two : ((cpower ttwo 2%Z) = (i_to_t 4%Z)).
...
Axiom min_set_spec :
  forall (s:set Z), ((cardinal s) > 0%Z)%Z ->
  (mem (min_set s) s) /\ forall (e:Z), (mem e s) -> (e >= (min_set s))%Z.
...
Axiom set_mem_divisors :
  forall (i:Z) (j:Z), (0%Z < i)%Z -> ((1%Z <= j)%Z /\ (j <= i)%Z) ->
  ((int.EuclideanDivision.mod1 i j) = 0%Z) -> mem j (divisors i).
...
Axiom prime_def :
  forall (a:Z),
  (prime a) <-> ((0%Z < a)%Z /\ ((divisors a) = (add a (to_set 1%Z)))).

Axiom card_prime_divisors :
  forall (a:Z), (1%Z < a)%Z -> (prime a) -> ((cardinal (divisors a)) = 2%Z).

Parameter co_prime: Z -> Z -> Prop.

Axiom co_prime_def :
  forall (a:Z) (b:Z),
  (co_prime a b) <->
  ((0%Z < a)%Z /\
   ((0%Z < b)%Z /\ ((inter (divisors a) (divisors b)) = (to_set 1%Z)))).
...
Axiom H4 : ((int.EuclideanDivision.mod1 (a * b)%Z c) = 0%Z).

Axiom H5 : ~ ((int.EuclideanDivision.mod1 b c) = 0%Z).

Axiom H6 : ~ ((int.EuclideanDivision.mod1 a c) = 0%Z).

Axiom H7 : (1%Z < c)%Z -> ((cardinal (to_fset 1%Z c)) = (c - 1%Z)%Z).

Axiom H8 : (c <= 1%Z)%Z -> is_empty (to_fset 1%Z c).

Axiom H9 :
  forall (k:Z), (mem k (to_fset 1%Z c)) <-> ((1%Z <= k)%Z /\ (k < c)%Z).
...
Axiom o_def :
  ...
  ((o d) = true) <->
  (((1%Z <= d)%Z /\ (d <= c)%Z) /\
   (~ ((int.EuclideanDivision.mod1 d c) = 0%Z) /\
    ((int.EuclideanDivision.mod1 (a * d)%Z c) = 0%Z))).

Axiom b'_def : (b' = (min_filter (to_fset 1%Z c) o)).
...
Axiom H12 :
  forall (e:Z), (mem e (to_fset 1%Z c)) -> ((o2 e) = true) -> (b' <= e)%Z.
...
Axiom H13 :
  forall (e:Z), (mem e (to_fset 1%Z c)) -> (e < b')%Z -> ~ ((o3 e) = true).

Axiom H14 : (0%Z < b')%Z.

Axiom H15 : (b' < c)%Z.

Axiom H16 : (0%Z < (int.EuclideanDivision.mod1 b' c))%Z.

Parameter r: Z.

Axiom r_def : (r = (int.EuclideanDivision.mod1 c b')).

Axiom h : (0%Z = r).

Axiom h1 : mem b' (divisors c).

(* Why3 goal *)
Theorem h2 : ~ ((0%Z < c)%Z /\ ((divisors c) = (add c (to_set 1%Z)))).
Proof.
