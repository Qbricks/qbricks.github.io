...
Axiom H1 : co_prime a b.

Axiom H2 : (0%Z < c)%Z.

Axiom H3 : co_prime a c.

Axiom H4 : co_prime b c.
...
Axiom H8 : (0%Z < (a * b)%Z)%Z.

Axiom H9 : (1%Z < c)%Z.

Axiom h : ((a * b)%Z = c).

Parameter result7: Z -> bool.

Axiom result_def7 :
  forall (j:Z),
  ((result7 j) = true) <->
  (((1%Z <= j)%Z /\ (j <= c)%Z) /\ ((int.EuclideanDivision.mod1 c j) = 0%Z)).

(* Why3 goal *)
Theorem h1 : mem a (my_filter (to_fset 1%Z (c + 1%Z)%Z) result7).
Proof.
