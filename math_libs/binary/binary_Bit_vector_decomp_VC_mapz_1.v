...
Axiom n_bvs_spec :
  ...
  ((1%Z <= n)%Z -> forall (s:set bitvec),
   (forall (f:bitvec), (mem f s) <-> ((length f) = (n - 1%Z)%Z)) ->
   ((n_bvs n) = (union (mapz_pre (n - 1%Z)%Z s) (mapo_pre (n - 1%Z)%Z s)))) /\
...
Axiom H1 :
  (1%Z <= (n - 1%Z)%Z)%Z -> forall (s:set bitvec),
  (forall (f:bitvec), (mem f s) <-> ((length f) = ((n - 1%Z)%Z - 1%Z)%Z)) ->
  ((n_bvs (n - 1%Z)%Z) =
   (union (mapz_pre ((n - 1%Z)%Z - 1%Z)%Z s)
    (mapo_pre ((n - 1%Z)%Z - 1%Z)%Z s))).

Axiom H2 :
  forall (f:bitvec), (mem f (n_bvs (n - 1%Z)%Z)) ->
  ((length f) = (n - 1%Z)%Z).

Axiom H3 :
  forall (f:bitvec),
  (mem f (n_bvs (n - 1%Z)%Z)) <-> ((length f) = (n - 1%Z)%Z).

Axiom H4 : ((cardinal (n_bvs (n - 1%Z)%Z)) > 0%Z)%Z.

Parameter result12: set bitvec.

Axiom result_def12 : (result12 = (mapz_pre n (n_bvs (n - 1%Z)%Z))).

Axiom H5 : mem (make_bv (fun (us:Z) => 0%Z) n) result12.

Axiom H6 : forall (e:bitvec), (mem e result12) -> (((getbv e) 0%Z) = 0%Z).

Axiom H7 :
  forall (e:bitvec), (mem e result12) ->
  exists bv:bitvec, (mem bv (n_bvs (n - 1%Z)%Z)) /\ (e = (concat_l bv 0%Z)).

Axiom H8 : forall (e:bitvec), (mem e result12) -> ((length e) = n).
