...
Axiom Hinst :
  ((ind_isum
    (fun (k1:Z) =>
     (((value bv) k1) * (power 2%Z (((length bv) - 1%Z)%Z - k1)%Z))%Z)
    k (length bv))
   =
   (ind_isum
    (fun (b:Z) =>
...
Axiom Hinst1 :
  ((0%Z <= 1%Z)%Z /\ (1%Z <= (((length bv) - k)%Z - 1%Z)%Z)%Z) ->
  (binary (fun (b:Z) => (((value bv) (b + k)%Z) * (power 2%Z b))%Z)) ->
  ((int.EuclideanDivision.div
    (ind_isum
     (fun (k1:Z) =>
      (((fun (b:Z) => (((value bv) (b + k)%Z) * (power 2%Z b))%Z) k1) *
       (power 2%Z (((((length bv) - k)%Z - 1%Z)%Z - 1%Z)%Z - k1)%Z))%Z)
     0%Z (((length bv) - k)%Z - 1%Z)%Z)
    (power 2%Z 1%Z))
   =
   (ind_isum
    (fun (k1:Z) =>
     (((fun (b:Z) => (((value bv) (b + k)%Z) * (power 2%Z b))%Z) k1) *
      (power 2%Z ((((((length bv) - k)%Z - 1%Z)%Z - 1%Z)%Z - k1)%Z - 1%Z)%Z))%Z)
    0%Z ((((length bv) - k)%Z - 1%Z)%Z - 1%Z)%Z)).

(* Why3 goal *)
Theorem VC_bv_to_int_kth_pre :
  (((value bv) k) =
   (int.EuclideanDivision.div
    (ind_isum
     (fun (b:Z) =>
      (((value bv) (b + k)%Z) *
       (power 2%Z (((length bv) - 1%Z)%Z - (b + k)%Z)%Z))%Z)
     (k - k)%Z ((length bv) - k)%Z)
    (power 2%Z (((length bv) - k)%Z - 1%Z)%Z))).
Proof.
