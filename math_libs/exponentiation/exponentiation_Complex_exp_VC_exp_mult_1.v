...
(* Why3 assumption *)
Definition infix_lsas (x:R) (y:R) : Prop := (x <= y)%R /\ ~ (x = y).
...
Axiom H : (teq x tzero) <-> (x = tzero).

Axiom H1 :
  ((0%Z <= y)%Z -> ((exp (infix_asdt x (i_to_t y))) = (cpower (exp x) y))) /\
  (~ (0%Z <= y)%Z ->
   ((exp (infix_asdt x (i_to_t (-y)%Z))) = (cpower (exp x) (-y)%Z))).

Axiom h : ~ (y = 0%Z).

(* Why3 goal *)
Theorem VC_exp_mult :
  ((infix_sldt tone (exp (infix_asdt x (i_to_t (-y)%Z)))) =
   (cpower (exp x) y)).
Proof.
