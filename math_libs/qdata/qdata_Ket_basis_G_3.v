...
Parameter result15: (matrix t) -> Z -> bool.

Axiom result_def15 :
  forall (m:matrix t) (x:Z),
  (((result15 m) x) = true) <-> ((get m x 0%Z) = tone).

Axiom ket_to_int_def :
  forall (m:matrix t), (is_a_ket_basis_elt m) ->
  ((ket_to_int m) =
   (element (filter (result15 m) (to_fset 0%Z (power 2%Z (ket_length m)))))).
...
Axiom H1 : ((columns m) = (rows m)).

Axiom H2 : ((rows m) = (rows x)).

Parameter i: Z.

Parameter j: Z.

Axiom H3 : valid_index (mat_mult m x) i j.

Axiom Hinst :
  ((0%Z <= (ket_to_int x))%Z /\ ((ket_to_int x) < (columns m))%Z) ->
  (forall (k:Z), ((0%Z <= k)%Z /\ (k < (columns m))%Z) ->
   ~ (k = (ket_to_int x)) ->
   (((fun (k1:Z) => (infix_asdt (get m i k1) (get x k1 j))) k) = tzero)) ->
  ((ind_sum (fun (k:Z) => (infix_asdt (get m i k) (get x k j))) 0%Z
    (columns m))
   = ((fun (k:Z) => (infix_asdt (get m i k) (get x k j))) (ket_to_int x))).

(* Why3 goal *)
Theorem G :
  ((s_rows (to_fset 0%Z (rows x))
    (fun (k:Z) =>
     (infix_asdtdt (get m k (ket_to_int x)) (ket (ket_length x) k))))
   = (power 2%Z (ket_length x))).
Proof.
