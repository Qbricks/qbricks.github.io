...
Parameter fc5: Z -> Z -> Z -> Z.

Axiom fc_def5 :
  forall (i:Z) (n:Z) (l:Z),
  ((1%Z <= l)%Z ->
   (((fc5 i n) l) = ((kth_nit_r i l n) * (power n (l - 1%Z)%Z))%Z)) /\
  (~ (1%Z <= l)%Z -> (((fc5 i n) l) = 0%Z)).

Axiom nary_decomposition_pre :
  forall (i:Z) (k:Z) (n:Z), (0%Z <= i)%Z -> (0%Z < n)%Z -> (1%Z <= k)%Z ->
  ((int.EuclideanDivision.mod1 i (power n k)) =
   (ind_isum (fc5 i n) 1%Z (k + 1%Z)%Z)) /\
  ((int.EuclideanDivision.mod1 i (power n k)) =
...
Axiom result_def7 :
  ...
  ((k = 1%Z) -> (((result7 bv i) k) = i)) /\
  (~ (k = 1%Z) -> (((result7 bv i) k) = (getbv bv (k - 1%Z)%Z))).
...
Axiom result_def8 :
  ...
  ((in_range bv k) ->
   (((result8 bv) k) = ((getbv bv k) * (power 2%Z ((length bv) - k)%Z))%Z)) /\
  (~ (in_range bv k) -> (((result8 bv) k) = 1%Z)).

(* Why3 assumption *)
Definition bv_to_int (bv:bitvec) : Z :=
  ind_isum (result8 bv) 1%Z ((length bv) + 1%Z)%Z.
...
Parameter result9: Z -> Z -> Z -> matrix t.

Axiom result_def9 :
  forall (i:Z) (n:Z) (k:Z),
  (((1%Z <= k)%Z /\ (k < (n + 1%Z)%Z)%Z) ->
   (((result9 i n) k) = (unary_ket (kth_bit_l i n k)))) /\
  (~ ((1%Z <= k)%Z /\ (k < (n + 1%Z)%Z)%Z) ->
   (((result9 i n) k) = (make 1%Z 1%Z tzero))).

Axiom init_ket_def :
  forall (i:Z) (n:Z), ((0%Z <= i)%Z /\ (i < (power 2%Z n))%Z) ->
  (0%Z < n)%Z -> ((init_ket i n) = (k_iter (result9 i n) 1%Z (n + 1%Z)%Z)).
...
Axiom result_def10 :
  forall (i:Z) (n:Z) (k:Z),
  (((1%Z <= k)%Z /\ (k <= n)%Z) -> (((result10 i n) k) = (kth_bit_l i n k))) /\
  (~ ((1%Z <= k)%Z /\ (k <= n)%Z) -> (((result10 i n) k) = 0%Z)).
...
Axiom fc_def7 :
  ...
  ((l <= (length bv))%Z ->
   (((fc7 bv) l) = ((getbv bv l) * (power 2%Z ((length bv) - l)%Z))%Z)) /\
  (~ (l <= (length bv))%Z -> (((fc7 bv) l) = 0%Z)).

Axiom fc_def8 :
  forall (bv:bitvec) (l:Z),
  ((l <= (length bv))%Z ->
   (((fc8 bv) l) = ((getbv bv l) * (power 2%Z ((length bv) - l)%Z))%Z)) /\
...
Parameter result11: Z -> Z -> (Z* Z)%type -> matrix t.
...
Axiom result_def11 :
  ...
  | (i, j) =>
      ((((0%Z <= i)%Z /\ (i < r)%Z) /\ ((0%Z <= j)%Z /\ (j < c)%Z)) ->
       (((result11 r c) o) = (ind_basis_mat i j r c))) /\
      (~ (((0%Z <= i)%Z /\ (i < r)%Z) /\ ((0%Z <= j)%Z /\ (j < c)%Z)) ->
       (((result11 r c) o) = (make r c tzero)))
  end.

Axiom basis_mat_def :
  forall (r:Z) (c:Z), (0%Z < r)%Z -> (0%Z < c)%Z ->
  ((basis_mat r c) =
   (map (result11 r c) (cartesian_product (to_fset 0%Z r) (to_fset 0%Z c)))).
...
Parameter result12: (matrix t) -> (Z* Z)%type -> bool.
...
Axiom basis_mat_indexes_def :
  ...
  ((basis_mat_indexes m) = (element (filter (result12 m) (mat_indices m)))).
...
Axiom result_def13 :
  ...
  (~ (valid_index m (fir o) (sec o)) -> (((result13 m) o) = m)).

(* Why3 assumption *)
Definition basis_projections (m:matrix t) : set (matrix t) :=
  map (result13 m) (to_indexes m).
...
Axiom result_def14 :
  ...
  (~ (mem o (to_indexes m)) -> (((result14 m) o) = m)).

(* Why3 assumption *)
Definition indexes_decomp (m:matrix t) : matrix t :=
  mat_sum (to_indexes m) (result14 m).
...
Axiom H1 :
  forall (m:matrix t),
  (mem m (basis_mat (power 2%Z n) 1%Z)) <->
  exists i:Z, exists j:Z,
  ((0%Z <= i)%Z /\ (i < (power 2%Z n))%Z) /\
  (((0%Z <= j)%Z /\ (j < 1%Z)%Z) /\
   (m = (ind_basis_mat i j (power 2%Z n) 1%Z))).

Axiom H2 :
  ((basis_mat (power 2%Z n) 1%Z) =
   (map
    (fun (o:(Z* Z)%type) =>
...
Axiom H3 :
  constant_size (basis_mat (power 2%Z n) 1%Z) (fun (y0:matrix t) => y0).

Axiom H4 :
  ((s_columns (basis_mat (power 2%Z n) 1%Z) (fun (y0:matrix t) => y0)) = 1%Z).

Axiom H5 :
  ((s_rows (basis_mat (power 2%Z n) 1%Z) (fun (y0:matrix t) => y0)) =
   (power 2%Z n)).

Parameter fc17: (matrix t) -> (Z* Z)%type -> bool.

Axiom fc_def17 :
  forall (m:matrix t) (x:(Z* Z)%type),
  (((fc17 m) x) = true) <-> ((get m (fir x) (sec x)) = tone).

Axiom H6 :
  forall (m:matrix t), (mem m (basis_mat (power 2%Z n) 1%Z)) ->
  ((cardinal (filter (fc17 m) (mat_indices m))) = 1%Z).
...
Axiom H9 :
  forall (mat:matrix t), (mem mat (basis_mat (power 2%Z n) 1%Z)) ->
...
Parameter mat: matrix t.

Axiom H11 : mem mat (basis_mat (power 2%Z n) 1%Z).

Axiom H12 : ((rows mat) = (power 2%Z n)).

Axiom H13 : ((columns mat) = 1%Z).

Parameter a: (Z* Z)%type.

Axiom Hinst :
  (mem a (cartesian_product (to_fset 0%Z (power 2%Z n)) (to_fset 0%Z 1%Z))) /\
  (mat =
   ((fun (o:(Z* Z)%type) =>
     (ind_basis_mat (fir o) (sec o) (power 2%Z n) 1%Z))
    a)).
...
(* Why3 goal *)
Theorem G :
  (valid_index mat (fir a) 0%Z) /\ ((basis_mat_indexes mat) = (fir a, 0%Z)).
Proof.
