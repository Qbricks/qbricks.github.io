...
Axiom tensor_indexes_to_left :
  forall (m:Z -> Z -> t) (n:Z -> Z -> t), (is_a_q_bra m) -> (is_a_q_bra n) ->
  forall (k:Z) (l:Z),
  (valid_index m k (int.EuclideanDivision.div l (columns n))) ->
...
Axiom H2 :
  forall (k:Z) (l:Z),
  (valid_index m k (int.EuclideanDivision.div l (columns n))) ->
  ((k = 0%Z) /\ (0%Z <= (int.EuclideanDivision.div l (columns n)))%Z) ->
  ((k = 0%Z) /\ (0%Z <= l)%Z) ->
  valid_index n k (int.EuclideanDivision.mod1 l (columns n)).
...
Axiom H5 :
  ...
  (k = 0%Z) /\
  ((0%Z <= l)%Z /\
   (l < ((power 2%Z (bra_length m)) * (power 2%Z (bra_length n)))%Z)%Z).

Axiom H6 :
  forall (l:Z), (valid_index (tensor m n) 0%Z l) ->
  (0%Z <= l)%Z /\
  (l < ((power 2%Z (bra_length m)) * (power 2%Z (bra_length n)))%Z)%Z.

Axiom H7 :
  forall (l:Z),
  (((int.EuclideanDivision.div l (columns n)) * (columns n))%Z <= l)%Z /\
  (l <
   (((int.EuclideanDivision.div l (columns n)) + 1%Z)%Z * (columns n))%Z)%Z.

Axiom H8 :
  forall (l:Z),
  (((int.EuclideanDivision.div l (columns n)) * (power 2%Z (bra_length n)))%Z
   <= l)%Z.
...
(* Why3 goal *)
Theorem VC_tensor_indexes :
  ((int.EuclideanDivision.div l (columns n)) < (power 2%Z (bra_length m)))%Z.
Proof.
