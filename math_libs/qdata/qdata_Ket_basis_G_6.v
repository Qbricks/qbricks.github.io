...
Parameter result15: bitvec -> bitvec -> Z -> Z.

Axiom result_def15 :
  forall (bv1:bitvec) (bv2:bitvec) (i:Z),
  ((((length bv1) <= i)%Z /\ (i < ((length bv1) + (length bv2))%Z)%Z) ->
   (((result15 bv1 bv2) i) = ((value bv2) (i - (length bv1))%Z))) /\
  (~ (((length bv1) <= i)%Z /\ (i < ((length bv1) + (length bv2))%Z)%Z) ->
   (((result15 bv1 bv2) i) = ((value bv1) i))).

(* Why3 assumption *)
Definition concat (bv1:bitvec) (bv2:bitvec) : bitvec :=
  make_bv (result15 bv1 bv2) ((length bv1) + (length bv2))%Z.
...
(* Why3 goal *)
Theorem G :
  ((rows (ket (ket_length x) (choose (to_fset 0%Z (rows x))))) = (rows x)).
Proof.
