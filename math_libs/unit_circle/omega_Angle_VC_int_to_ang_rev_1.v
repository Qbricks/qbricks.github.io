...
Parameter ang_meas: angle -> t.

Axiom ang_meas_spec :
  forall (o:angle),
  (infix_lseqdt tzero (ang_meas o)) /\ (infix_lsdt (ang_meas o) tone).
...
Axiom Real_To_Ang_one :
  forall (phi:t), (real_ phi) -> ((real_to_ang tone) = ang_zero).
...
Axiom meas_value :
  forall (phi:t), (real_ phi) ->
  ((infix_lseqdt tzero phi) /\ (infix_lsdt phi tone)) ->
...
Axiom real_to_ang_zero : ((real_to_ang tzero) = ang_zero).

(* Why3 assumption *)
Definition ang_exp (o:angle) : t :=
  exp
  (infix_asdt (infix_asdt (infix_asdt im pi) (infix_pldt tone tone))
   (ang_meas o)).

Axiom ang_exp_spec : forall (o:angle), ~ ((ang_exp o) = tzero).
...
Axiom meas_value_plus_one :
  ...
  ((exp
   ...
   (exp
    (infix_asdt (infix_asdt (infix_asdt im pi) (infix_pldt tone tone))
     (infix_pldt (ang_meas (real_to_ang phi)) tone)))).

Axiom ang_exp_pi_over_four :
  ((ang_exp (real_to_ang (infix_sldt tone (i_to_t 8%Z)))) =
   (infix_pldt
    (infix_sldt (square_rt (infix_pldt tone tone)) (infix_pldt tone tone))
    (infix_asdt im
     (infix_sldt (square_rt (infix_pldt tone tone)) (infix_pldt tone tone))))).
...
Axiom ang_exp_pi_over_two :
  ((ang_exp (real_to_ang (infix_sldt tone (i_to_t 4%Z)))) = im).

Axiom ang_exp_pi :
  ((ang_exp (real_to_ang (infix_sldt tone (i_to_t 2%Z)))) =
   (prefix_mndt tone)).
...
Axiom ang_exp_zero : ((ang_exp ang_zero) = tone).

Axiom ang_exp_two_pi : ((ang_exp (real_to_ang (i_to_t 1%Z))) = tone).

Axiom ang_exp_tone : ((ang_exp (real_to_ang tone)) = tone).

Axiom ang_meas_add_inf_tone :
  forall (o:angle) (o':angle),
  (infix_lsdt (infix_pldt (ang_meas o) (ang_meas o')) tone) ->
...
Axiom ang_meas_zero : ((ang_meas ang_zero) = tzero).

Axiom real_to_ang_add_one :
  forall (x:t), (real_ x) ->
  ((real_to_ang (infix_pldt x tone)) = (real_to_ang x)).
...
Parameter int_part: t -> t.

Axiom int_part_def :
  forall (x:t), (real_ x) -> ((int_part x) = (i_to_t (floor (real_part x)))).

Axiom int_part_spec :
  forall (x:t), (real_ x) ->
  (exists i:Z, ((int_part x) = (i_to_t i))) /\
  ((infix_lseqdt (int_part x) x) /\
   (infix_lsdt x (infix_pldt (int_part x) tone))).

Parameter int_part_int: t -> Z.

Axiom int_part_int_def :
  forall (x:t), (real_ x) -> ((int_part_int x) = (floor (real_part x))).

Axiom int_part_int_spec :
  forall (x:t), (real_ x) ->
  ((i_to_t (int_part_int x)) = (int_part x)) /\
  ((infix_lseqdt (i_to_t (int_part_int x)) x) /\
   (infix_lsdt x (infix_pldt (i_to_t (int_part_int x)) tone))).

Parameter dec_part: t -> t.

Axiom dec_part_def :
  forall (x:t), (real_ x) -> ((dec_part x) = (infix_mndt x (int_part x))).

Axiom dec_part_spec :
  forall (x:t), (real_ x) ->
  (exists i:Z, ((dec_part x) = (infix_mndt x (i_to_t i)))) /\
  ((infix_lseqdt tzero (dec_part x)) /\ (infix_lsdt (dec_part x) tone)).
...
Axiom real_to_ang_add :
  forall (x:t) (x':t), (real_ x) -> (real_ x') ->
...
Axiom ang_exp_add_pre :
  forall (o:angle) (o':angle),
  (infix_lseqdt tone (infix_pldt (ang_meas o) (ang_meas o'))) ->
...
Axiom real_to_ang_int_pre :
  forall (k:Z), (0%Z <= k)%Z -> ((ang_exp (real_to_ang (i_to_t k))) = tone).

Axiom real_to_ang_int :
  forall (k:Z), ((ang_exp (real_to_ang (i_to_t k))) = tone).

Axiom real_to_ang_int_ : forall (k:Z), ((real_to_ang (i_to_t k)) = ang_zero).

Axiom real_to_ang_value_add :
  forall (phi:t) (k:Z),
  ((infix_lseqdt tzero phi) /\ (infix_lsdt phi tone)) ->
  ((ang_exp (real_to_ang (infix_pldt phi (i_to_t k)))) =
   (exp
...
Axiom int_to_ang_def :
  ...
  ((int_to_ang k n) =
   (real_to_ang (infix_sldt (i_to_t k) (i_to_t (power_ 2%Z n))))).

Axiom int_to_ang_spec :
  forall (k:Z) (n:Z), (0%Z <= n)%Z ->
  ((int_to_ang k n) =
   (real_to_ang
    (infix_sldt (i_to_t (int.EuclideanDivision.mod1 k (power_ 2%Z n)))
     (i_to_t (power_ 2%Z n))))) /\
  (((0%Z <= k)%Z /\ (k < (power_ 2%Z n))%Z) ->
   ((ang_meas (int_to_ang k n)) =
    (infix_sldt (i_to_t k) (i_to_t (power_ 2%Z n))))).
...
Definition div_two (d:angle) : angle :=
  real_to_ang (infix_sldt (ang_meas d) (infix_pldt tone tone)).

Axiom div_two_spec :
  forall (d:angle),
  ((ang_meas (div_two d)) = (infix_sldt (ang_meas d) (infix_pldt tone tone))) /\
...
Axiom H3 :
  ((int_to_ang (((-k)%Z * l)%Z * (power_ 2%Z m))%Z n) =
   (int_to_ang ((-k)%Z * l)%Z 1%Z)).

Axiom H4 : ((int.EuclideanDivision.mod1 ((-k)%Z * l)%Z 2%Z) = 0%Z).

(* Why3 goal *)
Theorem VC_int_to_ang_rev :
  ((int_to_ang (((-k)%Z * l)%Z * (power_ 2%Z m))%Z n) = (int_to_ang 0%Z 1%Z)).
Proof.
