...
Parameter zero_d: d_frac.

Axiom Assoc2 :
  forall (x:d_frac) (y:d_frac) (z:d_frac),
  ((add1 (add1 x y) z) = (add1 x (add1 y z))).

Axiom Unit_def_l1 : forall (x:d_frac), ((add1 zero_d x) = x).

Axiom Unit_def_r1 : forall (x:d_frac), ((add1 x zero_d) = x).

Axiom Inv_def_l1 : forall (x:d_frac), ((add1 (inv1 x) x) = zero_d).

Axiom Inv_def_r1 : forall (x:d_frac), ((add1 x (inv1 x)) = zero_d).
...
Axiom H2 :
  let o1 := (i - 1%Z)%Z in
  ((((int.EuclideanDivision.mod1 o1 2%Z) = 0%Z) -> (o = d)) /\
   ((((int.EuclideanDivision.mod1 o1 2%Z) = 1%Z) -> (o = d)) /\
    forall (k:Z) (n:Z), (d = (dyadic k n)) ->
    (o = (dyadic ((power (-1%Z)%Z (ZArith.BinInt.Z.abs o1)) * k)%Z n)))) /\
  (result6 = (inv1 o)).

Axiom H3 : ((int.EuclideanDivision.mod1 i 2%Z) = 0%Z).

(* Why3 goal *)
Theorem VC_phase_inv : (result6 = d).
Proof.
