...
Axiom H2 : (0%Z < c)%Z -> ((cardinal (to_fset 0%Z c)) = (c - 0%Z)%Z).

Axiom H3 : (c <= 0%Z)%Z -> is_empty (to_fset 0%Z c).

Axiom H4 :
  forall (k:Z), (mem k (to_fset 0%Z c)) <-> ((0%Z <= k)%Z /\ (k < c)%Z).

Axiom H5 : (0%Z < r)%Z -> ((cardinal (to_fset 0%Z r)) = (r - 0%Z)%Z).

Axiom H6 : (r <= 0%Z)%Z -> is_empty (to_fset 0%Z r).

Axiom H7 :
  forall (k:Z), (mem k (to_fset 0%Z r)) <-> ((0%Z <= k)%Z /\ (k < r)%Z).

Axiom H8 :
  ((cardinal (cartesian_product (to_fset 0%Z r) (to_fset 0%Z c))) =
   ((cardinal (to_fset 0%Z r)) * (cardinal (to_fset 0%Z c)))%Z).

Axiom H9 :
  forall (a:Z), forall (b:Z),
  (mem (a, b) (cartesian_product (to_fset 0%Z r) (to_fset 0%Z c))) <->
  ((mem a (to_fset 0%Z r)) /\ (mem b (to_fset 0%Z c))).

Axiom H10 :
  forall (o:(Z* Z)%type),
  (mem o (cartesian_product (to_fset 0%Z r) (to_fset 0%Z c))) <->
  ((mem (fir o) (to_fset 0%Z r)) /\ (mem (sec o) (to_fset 0%Z c))).

Parameter o: (Z* Z)%type -> matrix t.
...
Axiom o_def :
  ...
  | (i, j) =>
      ((((0%Z <= i)%Z /\ (i < r)%Z) /\ ((0%Z <= j)%Z /\ (j < c)%Z)) ->
       ((o o1) = (ind_basis_mat i j r c))) /\
      (~ (((0%Z <= i)%Z /\ (i < r)%Z) /\ ((0%Z <= j)%Z /\ (j < c)%Z)) ->
       ((o o1) = (make r c tzero)))
  end.

(* Why3 assumption *)
Definition result7 : set (matrix t) :=
  map o (cartesian_product (to_fset 0%Z r) (to_fset 0%Z c)).
...
Axiom H12 :
  (result7 =
   (map (fun (o1:(Z* Z)%type) => (ind_basis_mat (fir o1) (sec o1) r c))
    (cartesian_product (to_fset 0%Z r) (to_fset 0%Z c)))).

Axiom H13 : constant_size result7 (fun (y0:matrix t) => y0).

Axiom H14 : ((s_columns result7 (fun (y0:matrix t) => y0)) = c).

Axiom H15 : ((s_rows result7 (fun (y0:matrix t) => y0)) = r).

Parameter m: matrix t.

Axiom H16 : mem m result7.

Parameter fc8: (Z* Z)%type -> bool.

Axiom fc_def8 :
  forall (x:(Z* Z)%type),
  ((fc8 x) = true) <-> ((get m (fir x) (sec x)) = tone).

(* Why3 goal *)
Theorem VC_basis_mat : ((cardinal (filter fc8 (mat_indices m))) = 1%Z).
Proof.
