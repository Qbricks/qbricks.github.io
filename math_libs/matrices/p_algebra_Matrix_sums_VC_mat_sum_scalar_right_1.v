...
Axiom teq_spec1 : forall (x:t) (y:t), (x = y) -> infix_eqeq x y.
...
Axiom H : constant_size s f.

Axiom H1 : (0%Z < (cardinal s))%Z.

Axiom H2 : (0%Z <= i)%Z.

Axiom H3 : (i < (s_rows s f))%Z.

Axiom H4 : (0%Z <= j)%Z.

Axiom H5 : (j < (s_columns s f))%Z.

Axiom H6 : ~ (is_empty s) -> mem (choose s) s.

Axiom H7 : ~ (is_empty s) -> mem (choose s) s.

Axiom H8 :
  forall (y:a),
  (mem y (remove (choose s) s)) <-> (~ (y = (choose s)) /\ (mem y s)).

Axiom H9 :
  ((mat_sum (add (choose s) (remove (choose s) s)) f) =
   (add_mat (f (choose s)) (mat_sum (remove (choose s) s) f))).

Axiom H10 : ~ (is_empty s) -> mem (choose s) s.

Axiom H11 : ~ (is_empty s) -> mem (choose s) s.
...
Axiom H13 :
  ((mat_sum (add (choose s) (remove (choose s) s))
    (fun (k:a) => (infix_asdtdt (get (f k) i j) m)))
   =
   (add_mat ((fun (k:a) => (infix_asdtdt (get (f k) i j) m)) (choose s))
    (mat_sum (remove (choose s) s)
     (fun (k:a) => (infix_asdtdt (get (f k) i j) m))))).

Axiom H14 : ~ (is_empty s) -> mem (choose s) s.
...
Axiom H16 :
  ((infix_asdtdt (get (mat_sum (remove (choose s) s) f) i j) m) =
   (mat_sum (remove (choose s) s)
    (fun (k:a) => (infix_asdtdt (get (f k) i j) m)))).

Axiom H17 : ~ (is_empty s) -> mem (choose s) s.
...
Axiom H19 :
  ((rows (mat_sum (remove (choose s) s) f)) =
   (s_rows (remove (choose s) s) f)).

Axiom H20 :
  ((columns (mat_sum (remove (choose s) s) f)) =
   (s_columns (remove (choose s) s) f)).

Axiom H21 :
  forall (e:a), (mem e (remove (choose s) s)) ->
  ((rows (f e)) = (s_rows (remove (choose s) s) f)).

Axiom H22 :
  forall (e:a), (mem e (remove (choose s) s)) ->
  ((columns (f e)) = (s_columns (remove (choose s) s) f)).

Axiom H23 :
  ((get (mat_sum (remove (choose s) s) f) i j) =
   (((elts (mat_sum (remove (choose s) s) f)) i) j)).

Axiom H24 : ~ (is_empty s) -> mem (choose s) s.

Axiom H25 : ((get (f (choose s)) i j) = (((elts (f (choose s))) i) j)).

Axiom H26 :
  ((add_mat (infix_asdtdt (get (f (choose s)) i j) m)
    (infix_asdtdt (get (mat_sum (remove (choose s) s) f) i j) m))
   =
   (infix_asdtdt
    (infix_pldt (get (f (choose s)) i j)
     (get (mat_sum (remove (choose s) s) f) i j))
    m)).

Axiom h : ~ ((cardinal s) = 1%Z).

(* Why3 goal *)
Theorem VC_mat_sum_scalar_right :
