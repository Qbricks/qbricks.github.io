...
Parameter f: b -> matrix t.

Parameter s: set a.

Parameter t1: a -> b.

Axiom H : (0%Z < (cardinal s))%Z.

Axiom H1 : constant_size s (fun (a1:a) => (f (t1 a1))).

Axiom H2 : p_injective t1 s.

Axiom H3 : constant_size (map t1 s) f.

Axiom H4 :
  ((cardinal s) = 1%Z) \/
  (((map t1 (remove (choose s) s)) = (remove (t1 (choose s)) (map t1 s))) /\
   ((~ (is_empty s) -> mem (choose s) s) /\
    ((~ (is_empty s) -> mem (choose s) s) /\
     ((forall (y:b),
       (mem y (remove (t1 (choose s)) (map t1 s))) <->
       (~ (y = (t1 (choose s))) /\ (mem y (map t1 s)))) /\
      (((mat_sum (add (t1 (choose s)) (remove (t1 (choose s)) (map t1 s))) f)
        =
        (add_mat (f (t1 (choose s)))
         (mat_sum (remove (t1 (choose s)) (map t1 s)) f))) /\
       (((mat_sum (map t1 s) f) =
         (add_mat ((fun (a1:a) => (f (t1 a1))) (choose s))
          (mat_sum (map t1 (remove (choose s) s)) f))) /\
        ((~ (is_empty s) -> mem (choose s) s) /\
         ((~ (is_empty s) -> mem (choose s) s) /\
          ((forall (y:a),
            (mem y (remove (choose s) s)) <->
            (~ (y = (choose s)) /\ (mem y s))) /\
           (((mat_sum (add (choose s) (remove (choose s) s))
              (fun (a1:a) => (f (t1 a1))))
             =
             (add_mat ((fun (a1:a) => (f (t1 a1))) (choose s))
              (mat_sum (remove (choose s) s) (fun (a1:a) => (f (t1 a1)))))) /\
            (((remove (t1 (choose s)) (map t1 s)) =
              (map t1 (remove (choose s) s))) /\
             ((~ (is_empty s) -> mem (choose s) s) /\
              ((forall (y:a),
                (mem y (remove (choose s) s)) <->
                (~ (y = (choose s)) /\ (mem y s))) /\
...
Axiom h1 : ((map t1 s) = (add (t1 (choose s)) (empty : set b))).

(* Why3 goal *)
Theorem VC_map_mat_sum : ((f (element (map t1 s))) = (f (t1 (choose s)))).
Proof.
