...
Parameter k_proj:
  (Z -> Z -> Z -> t) -> (Z -> Z) -> (Z -> Z) -> Z -> Z -> Z -> Z -> Z -> t.

Axiom k_proj_def :
  forall (f:Z -> Z -> Z -> t) (f1:Z -> Z) (f2:Z -> Z) (i:Z) (j:Z) (k:Z) 
    (l:Z) (h:Z),
  (0%Z <= i)%Z -> (0%Z <= j)%Z -> (l < h)%Z ->
  ((0%Z <= k)%Z /\ (k <= (l - h)%Z)%Z) ->
  (forall (k1:Z), (0%Z < (f1 k1))%Z) -> (forall (k1:Z), (0%Z < (f2 k1))%Z) ->
  ((k_proj f f1 f2 i j k l h) =
   (((f ((l + k)%Z - 1%Z)%Z) (kth_left i k l h f1)) (kth_left j k l h f2))).

Parameter kronecker_indexed_val:
  (Z -> Z -> Z -> t) -> Z -> Z -> Z -> Z -> Z -> t.

Axiom kronecker_indexed_val_def :
  forall (f:Z -> Z -> Z -> t) (i:Z) (j:Z) (k:Z) (l:Z) (h:Z), (0%Z <= i)%Z ->
  (l < h)%Z -> (0%Z <= j)%Z -> ((0%Z <= k)%Z /\ (k <= (l - h)%Z)%Z) ->
  ((kronecker_indexed_val f i j k l h) =
   (k_proj f ((fun (y0:Z -> Z -> Z -> t) (y1:Z) => (frows y0 y1)) f)
    ((fun (y0:Z -> Z -> Z -> t) (y1:Z) => (fcolumns y0 y1)) f) i j k l h)).
...
Axiom kronecker_indexed_val_div_mod_genk :
...
Axiom kronecker_proj_transl :
  forall (f:Z -> Z -> Z -> t) (i:Z) (j:Z) (k:Z) (l:Z) (h:Z) (t1:Z),
  (0%Z <= i)%Z -> (l < h)%Z -> (0%Z <= j)%Z -> (0%Z <= t1)%Z ->
  ((0%Z <= k)%Z /\ (k <= (l - h)%Z)%Z) ->
  ((kronecker_indexed_val f i j k l h) =
   (kronecker_indexed_val f i j k l (h + t1)%Z)).
  ...
  ((kronecker_indexed_val f i j k l h) =
   (((f ((l + k)%Z - 1%Z)%Z)
    ...
    (int.EuclideanDivision.mod1
     (int.EuclideanDivision.div i
      (ind_iproduct
       ((fun (y0:Z -> Z -> Z -> t) (y1:Z) => (fcolumns y0 y1)) f) (l + k)%Z
       h))
     (fcolumns f ((l + k)%Z - 1%Z)%Z)))).
...
Axiom H3 :
  ((k_iter f l h) = (kronecker (k_iter f l (h - 1%Z)%Z) (f (h - 1%Z)%Z))).

(* Why3 assumption *)
Definition o : Z -> Z :=
  (fun (y0:Z -> Z -> Z -> t) (y1:Z) => (fcolumns y0 y1)) f.

Axiom H4 : forall (k:Z), (0%Z < (o k))%Z.

(* Why3 assumption *)
Definition o1 : Z -> Z :=
  (fun (y0:Z -> Z -> Z -> t) (y1:Z) => (frows y0 y1)) f.

Axiom H5 : forall (k:Z), (0%Z < (o1 k))%Z.

Axiom H6 :
  ((rows (k_iter f l (h - 1%Z)%Z)) =
   (ind_iproduct ((fun (y0:Z -> Z -> Z -> t) (y1:Z) => (frows y0 y1)) f) l
    (h - 1%Z)%Z)).

Axiom H7 :
  ((columns (k_iter f l (h - 1%Z)%Z)) =
   (ind_iproduct ((fun (y0:Z -> Z -> Z -> t) (y1:Z) => (fcolumns y0 y1)) f) l
    (h - 1%Z)%Z)).

Axiom H8 :
  ((((kronecker (k_iter f l (h - 1%Z)%Z) (f (h - 1%Z)%Z))
     (int.EuclideanDivision.mod1 i (ind_iproduct o1 l h)))
    (int.EuclideanDivision.mod1 j (ind_iproduct o l h)))
   =
   (infix_asdt
    (((k_iter f l (h - 1%Z)%Z)
      (int.EuclideanDivision.div
...
Axiom H9 : forall (k:Z), (0%Z < (o2 k))%Z.

Axiom H10 :
  ((ind_iproduct o2 l h) =
   ((ind_iproduct o2 l (h - 1%Z)%Z) * (o2 (h - 1%Z)%Z))%Z).

(* Why3 assumption *)
Definition o3 : Z -> Z :=
  (fun (y0:Z -> Z -> Z -> t) (y1:Z) => (fcolumns y0 y1)) f.

Axiom H11 : forall (k:Z), (0%Z < (o3 k))%Z.

Axiom H12 :
  ((ind_iproduct o3 l h) =
   ((ind_iproduct o3 l (h - 1%Z)%Z) * (o3 (h - 1%Z)%Z))%Z).

Axiom H13 : (0%Z < (frows f (h - 1%Z)%Z))%Z.

(* Why3 assumption *)
Definition o4 : Z -> Z :=
  (fun (y0:Z -> Z -> Z -> t) (y1:Z) => (frows y0 y1)) f.

Axiom H14 : forall (k:Z), (0%Z < (o4 k))%Z.

Axiom H15 :
  ((int.EuclideanDivision.mod1
    (int.EuclideanDivision.mod1 i
     ((frows f (h - 1%Z)%Z) * (ind_iproduct o4 l (h - 1%Z)%Z))%Z)
    (ind_iproduct o4 l (h - 1%Z)%Z))
   = (int.EuclideanDivision.mod1 i (ind_iproduct o4 l (h - 1%Z)%Z))).

Axiom H16 : (0%Z < (fcolumns f (h - 1%Z)%Z))%Z.

(* Why3 assumption *)
Definition o5 : Z -> Z :=
  (fun (y0:Z -> Z -> Z -> t) (y1:Z) => (fcolumns y0 y1)) f.

Axiom H17 : forall (k:Z), (0%Z < (o5 k))%Z.

Axiom H18 :
  ((int.EuclideanDivision.mod1
    (int.EuclideanDivision.mod1 i
     ((fcolumns f (h - 1%Z)%Z) * (ind_iproduct o5 l (h - 1%Z)%Z))%Z)
    (ind_iproduct o5 l (h - 1%Z)%Z))
   = (int.EuclideanDivision.mod1 i (ind_iproduct o5 l (h - 1%Z)%Z))).
