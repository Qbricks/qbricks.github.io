...
Parameter fc7: Z -> Z -> Z -> Z -> (Z* Z)%type -> bool.
...
Axiom ind_basis_mat_spec :
  ...
  (((columns (ind_basis_mat i j r c)) = c) /\
   ((forall (k:Z) (l:Z), (valid_index (ind_basis_mat i j r c) k l) ->
    ...
    ((cardinal (filter (fc7 i j r c) (mat_indices (ind_basis_mat i j r c))))
     = 1%Z))).
...
Parameter result7: Z -> Z -> (Z* Z)%type -> matrix t.
...
Parameter result8: (matrix t) -> (Z* Z)%type -> bool.
...
Axiom basis_mat_indexes_def :
  ...
  ((basis_mat_indexes m) = (element (filter (result8 m) (mat_indices m)))).
...
Parameter result10: (matrix t) -> (Z* Z)%type -> matrix t.

Axiom result_def10 :
  forall (m:matrix t) (o:(Z* Z)%type),
  ((mem o (to_indexes m)) ->
   (((result10 m) o) = (basis_projection m (fir o) (sec o)))) /\
  (~ (mem o (to_indexes m)) -> (((result10 m) o) = m)).

(* Why3 assumption *)
Definition indexes_decomp (m:matrix t) : matrix t :=
  mat_sum (to_indexes m) (result10 m).
...
Axiom H1 : ((columns m) = (s_rows s f)).

Axiom H2 : subset s' s.

Axiom H3 : (0%Z < (cardinal s'))%Z.

Axiom H4 : ~ ((cardinal s') = 1%Z).

Parameter o: a -> matrix t.

Axiom o_def :
  forall (a1:a),
  ((mem a1 s) -> ((o a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o a1) = m)).

Axiom H5 : constant_size s' o.

Parameter o1: a -> matrix t.

Parameter o2: a -> matrix t.

Axiom o_def1 :
  forall (a1:a),
  ((mem a1 s) -> ((o1 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o1 a1) = m)).

Axiom o_def2 :
  forall (a1:a),
  ((mem a1 s) -> ((o2 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o2 a1) = m)).

Axiom H6 : ((s_rows s' o1) = (s_rows s o2)).

Parameter o3: a -> matrix t.

Parameter o4: a -> matrix t.

Axiom o_def3 :
  forall (a1:a),
  ((mem a1 s) -> ((o3 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o3 a1) = m)).

Axiom o_def4 :
  forall (a1:a),
  ((mem a1 s) -> ((o4 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o4 a1) = m)).

Axiom H7 : ((s_columns s' o3) = (s_columns s o4)).

Axiom H8 : ~ ((cardinal s') = 1%Z).

Axiom H9 : ~ (is_empty s') -> mem (choose s') s'.

Axiom H10 :
  forall (y:a),
  (mem y (remove (choose s') s')) <-> (~ (y = (choose s')) /\ (mem y s')).

Parameter o5: a -> matrix t.

Axiom o_def5 :
  forall (a1:a),
  ((mem a1 s) -> ((o5 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o5 a1) = m)).

Axiom H11 : constant_size (remove (choose s') s') o5.

Parameter o6: a -> matrix t.

Parameter o7: a -> matrix t.

Axiom o_def6 :
  forall (a1:a),
  ((mem a1 s) -> ((o6 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o6 a1) = m)).

Axiom o_def7 :
  forall (a1:a),
  ((mem a1 s) -> ((o7 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o7 a1) = m)).

Axiom H12 : ((s_rows (remove (choose s') s') o6) = (s_rows s' o7)).

Parameter o8: a -> matrix t.

Parameter o9: a -> matrix t.

Axiom o_def8 :
  forall (a1:a),
  ((mem a1 s) -> ((o8 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o8 a1) = m)).

Axiom o_def9 :
  forall (a1:a),
  ((mem a1 s) -> ((o9 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o9 a1) = m)).

Axiom H13 : ((s_columns (remove (choose s') s') o8) = (s_columns s' o9)).

Axiom H14 : ~ (is_empty s') -> mem (choose s') s'.

Axiom H15 : ~ (is_empty s') -> mem (choose s') s'.

Axiom H16 :
  forall (y:a),
  (mem y (remove (choose s') s')) <-> (~ (y = (choose s')) /\ (mem y s')).

Axiom H17 : (0%Z < (cardinal (remove (choose s') s')))%Z.

Parameter o10: a -> matrix t.

Axiom o_def10 :
  forall (a1:a),
  ((mem a1 s) -> ((o10 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o10 a1) = m)).

Axiom H18 : constant_size (remove (choose s') s') o10.

Parameter o11: a -> matrix t.

Parameter o12: a -> matrix t.

Axiom o_def11 :
  forall (a1:a),
  ((mem a1 s) -> ((o11 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o11 a1) = m)).

Axiom o_def12 :
  forall (a1:a),
  ((mem a1 s) -> ((o12 a1) = (mat_mult m (f a1)))) /\
  (~ (mem a1 s) -> ((o12 a1) = m)).

Axiom H19 :
  ((rows (o11 (choose s'))) = (s_rows (remove (choose s') s') o12)).

(* Why3 goal *)
Theorem G :
  ((mem (choose (remove (choose s') s')) s) ->
   ((mem (choose s') s) ->
    ((columns (mat_mult m (f (choose (remove (choose s') s'))))) =
     (columns (mat_mult m (f (choose s')))))) /\
   (~ (mem (choose s') s) ->
    ((columns (mat_mult m (f (choose (remove (choose s') s'))))) =
     (columns m)))) /\
  (~ (mem (choose (remove (choose s') s')) s) -> (mem (choose s') s) ->
   ((columns m) = (columns (mat_mult m (f (choose s')))))).
Proof.
