...
Axiom qft_coeff_spec :
  ...
  ((qft_coeff x y n) =
   (ang_exp (int_to_ang ((bv_to_int x) * (bv_to_int (bv_inversion y)))%Z n))).
...
Axiom int_qft_coeff_def :
  ...
  ((int_qft_coeff i j n) =
   (ang_exp
    (real_to_ang
     (infix_asdt
      (infix_asdt (i_to_c (int.EuclideanDivision.mod1 i (power_ 2%Z n)))
       (i_to_c
        (int_bit_inversion (int.EuclideanDivision.mod1 j (power_ 2%Z n)) n)))
      (pow_inv_2 n))))).
...
Axiom int_qft_coeff_inv :
  forall (i:Z) (j:Z) (n:Z), (0%Z < n)%Z ->
  ((int_qft_rev_coeff i j n) = (infix_sldt c_one (int_qft_coeff i j n))).

Axiom int_qft_rev_coeff_inv :
  forall (i:Z) (j:Z) (n:Z), (0%Z < n)%Z ->
  ((int_qft_coeff i j n) = (infix_sldt c_one (int_qft_rev_coeff i j n))).
...
Axiom H2 : (i < (power_ 2%Z n))%Z.

Parameter j: Z.

Axiom H3 : (0%Z <= j)%Z.

Axiom H4 : (j < (power_ 2%Z n))%Z.

Parameter x: bitvec.

Axiom H5 : mem x (n_bvs n).

(* Why3 goal *)
Theorem G : ((length x) = (width (qft_rev n))).
Proof.
