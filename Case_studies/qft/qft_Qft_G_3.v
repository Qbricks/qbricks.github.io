...
Axiom teq_def :
  forall (x:complex) (y:complex), (teq x y) <-> (infix_eqeq x y).
...
Axiom r_to_c_inf :
  forall (i:R) (j:R),
  (infix_lseqdt (r_to_c i) (r_to_c j)) <-> (infix_lseqas i j).
...
Axiom binary_def :
  forall (t:Z -> Z),
  (binary t) <-> forall (k:Z), (0%Z <= (t k))%Z /\ ((t k) < 2%Z)%Z.
...
Axiom cpower_modulus :
  ...
  (((n > 0%Z)%Z -> (infix_lsdt (modulus x) c_one) ->
    infix_lsdt (modulus (cpower x n)) c_one) /\
   (((n > 0%Z)%Z -> ((modulus x) = c_one) ->
     ((modulus (cpower x n)) = c_one)) /\
    (((n > 0%Z)%Z -> (infix_gtdt (modulus x) c_one) ->
      infix_gtdt (modulus (cpower x n)) c_one) /\
     (((n > 0%Z)%Z -> (infix_lsdt (modulus (cpower x n)) c_one) ->
       infix_lsdt (modulus x) c_one) /\
      (((n > 0%Z)%Z -> ((modulus (cpower x n)) = c_one) ->
        ((modulus x) = c_one)) /\
       ((n > 0%Z)%Z -> (infix_gtdt (modulus (cpower x n)) c_one) ->
...
Axiom div_two_spec :
  forall (d:angle),
  ((ang_meas (div_two d)) = (infix_sldt (ang_meas d) ttwo)) /\
...
Axiom inv_auto_spec :
  ...
  forall (f:a -> a) (s:set a), (p_injective f s) ->
  (forall (e1:a), (mem e1 s) -> mem (f e1) s) ->
  (forall (e1:a), (mem e1 s) -> mem ((inv_auto f s) e1) s) /\
  ((forall (e1:a), (mem e1 s) -> ((f ((inv_auto f s) e1)) = e1)) /\
...
Axiom mapz_spec :
  ...
  ((0%Z < (cardinal (mapz n)))%Z /\
   ((forall (e1:bitvec), (mem e1 (mapz n)) -> (((getbv e1) 0%Z) = 0%Z)) /\
    ((forall (e1:bitvec), (mem e1 (mapz n)) ->
      exists bv:bitvec,
      (mem bv (n_bvs (n - 1%Z)%Z)) /\ (e1 = (concat_l bv 0%Z))) /\
     ((forall (e1:bitvec), (mem e1 (mapz n)) -> ((length e1) = n)) /\
      forall (e1:bitvec), ((length e1) = n) -> (((getbv e1) 0%Z) = 0%Z) ->
      mem e1 (mapz n))))).
...
Axiom mapo_spec :
  ...
  ((0%Z < (cardinal (mapo n)))%Z /\
   ((forall (e1:bitvec), (mem e1 (mapo n)) -> (((getbv e1) 0%Z) = 1%Z)) /\
    ((forall (e1:bitvec), (mem e1 (mapo n)) ->
      exists bv:bitvec,
      (mem bv (n_bvs (n - 1%Z)%Z)) /\ (e1 = (concat_l bv 1%Z))) /\
     ((forall (e1:bitvec), (mem e1 (mapo n)) -> ((length e1) = n)) /\
      forall (e1:bitvec), ((length e1) = n) -> (((getbv e1) 0%Z) = 1%Z) ->
      mem e1 (mapo n))))).
...
Axiom n_bvsz_spec :
  forall (us:unit),
  (forall (x:bitvec), ((length x) = 0%Z) -> mem x (n_bvsz us)) /\
  ((forall (x:bitvec), (mem x (n_bvsz us)) ->
...
Axiom n_bvso_spec :
  forall (us:unit),
  (forall (x:bitvec), ((length x) = 1%Z) -> mem x (n_bvso us)) /\
  ((forall (x:bitvec), (mem x (n_bvso us)) -> ((length x) = 1%Z)) /\
   (((n_bvso us) =
     (add (make_bv (fun (us1:Z) => 0%Z) 1%Z)
      (to_set (make_bv (fun (us1:Z) => 1%Z) 1%Z)))) /\
    forall (f:bitvec -> matrix complex), (constant_size (n_bvso us) f) ->
    ((mat_sum (n_bvso us) f) =
...
Axiom add_ket_l_spec :
  ...
  (((ket_length (add_ket_l m n l)) = l) /\
   ((forall (i:Z),
     ((get_ket (add_ket_l m n l) i) =
      (infix_pldt (get_ket m i) (get_ket n i)))) /\
   ...
   (map
    ((fun (y0:Z -> Z -> matrix complex) (y1:(Z* Z)%type) => (couple y0 y1))
     ((fun (y0:matrix complex) (y1:Z) (y2:Z) => (basis_projection y0 y1 y2))
      m))
    (to_indexes m))) /\
  ((forall (e1:matrix complex), (mem e1 (basis_projections m)) ->
    ((rows e1) = (rows m))) /\
   ((forall (e1:matrix complex), (mem e1 (basis_projections m)) ->
     ((columns e1) = (columns m))) /\
    (constant_size (basis_projections m)
     (fun (y0:matrix complex) => (p_id y0))))).
...
Axiom min_filter_spec :
  ...
  ((mem (min_filter s p) s) /\
   ((forall (e1:Z), (mem e1 s) -> ((p e1) = true) ->
     ((min_filter s p) <= e1)%Z) /\
    forall (e1:Z), (mem e1 s) -> (e1 < (min_filter s p))%Z ->
    ~ ((p e1) = true))).
...
Axiom max_filter_spec :
  ...
  ((mem (max_filter s p) s) /\
   ((forall (e1:Z), (mem e1 s) -> ((p e1) = true) ->
     (e1 <= (max_filter s p))%Z) /\
    forall (e1:Z), (mem e1 s) -> ((max_filter s p) < e1)%Z ->
    ~ ((p e1) = true))).
...
Axiom sem_def :
  ...
  (sem c x y) <-> ((is_a_ket_l x (width c)) /\ (infix_eqeq y (path_sem c x))).
...
Axiom path_sem_comp_ :
  forall (d:circuit) (e1:circuit) (x:matrix complex),
  (is_a_ket_l x (width d)) -> ((width d) = (width e1)) ->
  ((path_sem (sequence d e1) x) = (path_sem e1 (path_sem d x))).
...
Axiom qbit_zero_spec :
  forall (us:unit),
  ((get_ket (qbit_zero us) 0%Z) = c_one) /\
  (((get_ket (qbit_zero us) 1%Z) = c_zero) /\
   ((is_a_ket_l (qbit_zero us) 1%Z) /\
    (((rows (qbit_zero us)) = 2%Z) /\
     (((columns (qbit_zero us)) = 1%Z) /\
      (((get (qbit_zero us) 0%Z 0%Z) = c_one) /\
       (((get (qbit_zero us) 1%Z 0%Z) = c_zero) /\
        (is_a_ket_basis_elt (qbit_zero us)))))))).
...
Axiom qbit_one_spec :
  forall (us:unit),
  ((get_ket (qbit_one us) 0%Z) = c_zero) /\
  (((get_ket (qbit_one us) 1%Z) = c_one) /\
   ((is_a_ket_l (qbit_one us) 1%Z) /\
    (((rows (qbit_one us)) = 2%Z) /\
     (((columns (qbit_one us)) = 1%Z) /\
      (((get (qbit_one us) 0%Z 0%Z) = c_zero) /\
       (((get (qbit_one us) 1%Z 0%Z) = c_one) /\
        (is_a_ket_basis_elt (qbit_one us)))))))).
  ...
  ((is_a_ket_l (qbit_plus us) 1%Z) /\
   (((rows (qbit_plus us)) = 2%Z) /\
    (((columns (qbit_plus us)) = 1%Z) /\
     (((get_ket (qbit_plus us) 0%Z) = (pow_inv_sqrt_2 1%Z)) /\
      ((get_ket (qbit_plus us) 1%Z) = (pow_inv_sqrt_2 1%Z)))))).
  ...
  ((is_a_ket_l (qbit_minus us) 1%Z) /\
   (((rows (qbit_minus us)) = 2%Z) /\
    (((columns (qbit_minus us)) = 1%Z) /\
     (((get_ket (qbit_minus us) 0%Z) = (pow_inv_sqrt_2 1%Z)) /\
...
Axiom bv_check_zero_def :
  forall (x:bitvec),
  (((length x) = 0%Z) -> ((bv_check_zero x) = ang_minus_one)) /\
  (~ ((length x) = 0%Z) ->
   (((length x) = 1%Z) ->
...
Axiom fc_def27 :
  ...
  ((i = k) -> (((fc27 k x y) i) = ((getbv y) 0%Z))) /\
  (~ (i = k) -> (((fc27 k x y) i) = ((getbv x) i))).
       ...
       ((forall (x:Z -> Z) (y:Z -> Z), forall (m:Z), (m >= 1%Z)%Z ->
        ...
        (correct_path_sum_bv (place_hadamard_bv k n) 1%Z
         ...
         (fun (x:bitvec) (y:bitvec) =>
          (int_to_ang
           ((((getbv x) k) * ((getbv y) 0%Z))%Z * (power_ 2%Z (n - 1%Z)%Z))%Z
           n))))))))).
    ...
    ((forall (i:Z), ((0%Z <= i)%Z /\ (i < n)%Z) ->
     ...
     ((forall (i:Z), ((0%Z <= i)%Z /\ (i < n)%Z) ->
      ...
      ((path_sem (permutation_circuit n f) (bv_to_ket x)) =
       (bv_to_ket (make_bv (fun (i:Z) => ((getbv x) (f i))) n))))))).
...
Axiom cont_last_qbit_kron_def :
  forall (c:circuit), ((cont_last_qbit_kron c) = (cont_last_qbit c)).

Axiom cont_last_qbit_kron_spec :
  forall (c:circuit),
  ((size (cont_last_qbit_kron c)) <= ((size c) * cont_size)%Z)%Z /\
  (((ancillas (cont_last_qbit_kron c)) = (ancillas c)) /\
   (((range (cont_last_qbit_kron c)) = (range c)) /\
    (((width (cont_last_qbit_kron c)) = ((width c) + 1%Z)%Z) /\
     ((forall (x:matrix complex) (y:matrix complex), (sem c x y) ->
...
Axiom fc_def30 :
  forall (t:Z) (x:bitvec) (y:bitvec) (i:Z),
  ((i = t) -> (((fc30 t x y) i) = ((getbv y) 0%Z))) /\
  (~ (i = t) -> (((fc30 t x y) i) = ((getbv x) i))).
...
Axiom get_path_sem_rev_qft_ket :
  ...
  ((get_ket (path_sem (qft_rev_be n) (ket n i)) j) =
   (infix_asdt (pow_inv_sqrt_2 n)
  ...
  (((ancillas (qft_rev n)) = 0%Z) /\
   (((width (qft_rev n)) = n) /\
    ((forall (x:bitvec), ((length x) = n) ->
     ...
     ((0%Z <= i)%Z /\ (i < (power_ 2%Z n))%Z) ->
     ((get_ket (path_sem (qft_rev n) (bv_to_ket x)) i) =
      (infix_asdt (pow_inv_sqrt_2 n)
  ...
  (((ancillas (qft_rev_ n)) = 0%Z) /\
   (((width (qft_rev_ n)) = n) /\
    ((forall (x:bitvec), ((length x) = n) ->
     ...
     ((forall (x:bitvec), forall (i:Z), ((length x) = n) ->
       ((0%Z <= i)%Z /\ (i < (power_ 2%Z n))%Z) ->
       ((get_ket (path_sem (qft_rev_ n) (bv_to_ket x)) i) =
        (infix_asdt (pow_inv_sqrt_2 n)
...
Parameter complex_as_a_vector: complex -> matrix complex.
...
Axiom complex_as_a_vector_def :
  forall (x:complex),
  ((complex_as_a_vector x) = (make_f 2%Z 1%Z (result24 x))).
...
Axiom cos_half_pi : ((cos (infix_sldt pi ttwo)) = c_zero).

Axiom cos_quarter_pi :
  ((cos (infix_sldt pi (i_to_c 4%Z))) = (infix_sldt squarert_two ttwo)).

Axiom cos_pi : ((cos pi) = (prefix_mndt c_one)).

Axiom sin_zero : ((sin c_zero) = c_zero).

Axiom sin_half_pi : ((sin (infix_sldt pi ttwo)) = c_one).

Axiom sin_ang_exp :
  forall (x:complex), (real_ x) ->
  ((sin (infix_asdt (infix_asdt ttwo pi) x)) =
   (t_im_part (ang_exp (real_to_ang x)))).

Axiom cos_ang_exp :
  forall (x:complex), (real_ x) ->
  ((cos (infix_asdt (infix_asdt ttwo pi) x)) =
   (t_real_part (ang_exp (real_to_ang x)))).

Axiom sin_ang_exp_ :
  forall (x:complex), (real_ x) ->
  ((sin x) =
   (t_im_part (ang_exp (real_to_ang (infix_sldt x (infix_asdt ttwo pi)))))).

Axiom cos_ang_exp_ :
  forall (x:complex), (real_ x) ->
  ((cos x) =
   (t_real_part (ang_exp (real_to_ang (infix_sldt x (infix_asdt ttwo pi)))))).
...
Axiom cos_two_x :
  forall (x:complex), (real_ x) ->
  ((cos (infix_asdt ttwo x)) =
   (infix_mndt (cpower (cos x) 2%Z) (cpower (sin x) 2%Z))).

Axiom sin_two_x :
  forall (x:complex) (x':complex), (real_ x) -> (real_ x') ->
  ((sin (infix_asdt ttwo x)) =
   (infix_asdt (infix_asdt ttwo (sin x)) (cos x))).
...
Axiom symetries_vert :
  forall (x:complex), (real_ x) ->
  ((sin (infix_pldt (infix_sldt pi ttwo) x)) =
   (sin (infix_mndt (infix_sldt pi ttwo) x))) /\
  ((cos (infix_pldt (infix_sldt pi ttwo) x)) =
   (prefix_mndt (cos (infix_mndt (infix_sldt pi ttwo) x)))).
...
Axiom sin_mod_two_pi :
  forall (x:complex), (real_ x) ->
  ((sin (infix_asdt (infix_asdt ttwo pi) x)) =
   (sin (infix_asdt (infix_asdt ttwo pi) (dec_part x)))).

Axiom cos_mod_two_pi :
  forall (x:complex), (real_ x) ->
  ((cos (infix_asdt (infix_asdt ttwo pi) x)) =
   (cos (infix_asdt (infix_asdt ttwo pi) (dec_part x)))).
...
Axiom growing_sine :
  ...
  ((infix_lseqdt c_zero a) /\
   ((infix_lsdt a b) /\ (infix_lseqdt b (infix_sldt c_one ttwo)))) ->
...
Axiom bounded_by_sine :
  ...
  ((infix_lseqdt c_zero (modulus x)) /\
   (infix_lseqdt (modulus x) (infix_sldt c_one ttwo))) ->
  infix_lseqdt (modulus (infix_asdt ttwo x))
  (modulus (sin (infix_asdt pi x))).
...
Axiom bounded_by_sine_one :
  ...
  infix_lseqdt
  (modulus
   (infix_asdt (infix_asdt ttwo (infix_mndt x c_one))
    (i_to_c (power_ 2%Z k))))
...
Axiom place_rev_qft_def :
  forall (n:Z) (k:Z) (size_reg:Z), (0%Z < n)%Z -> (0%Z < k)%Z ->
  ((k + n)%Z < size_reg)%Z ->
  ((place_rev_qft n k size_reg) = (place (qft_rev n) k size_reg)).
...
Axiom H2 : ((k + n)%Z < size_reg)%Z.

Axiom H3 : ((width c) = size_reg).

Axiom H4 : ((width (place_qft n k size_reg)) = size_reg).

Axiom H5 :
  ((size (place_qft n k size_reg)) <= (polysquare n cont_size 0%Z 0%Z))%Z.

Axiom H6 : ((ancillas (place_qft n k size_reg)) = 0%Z).
...
Axiom H8 : ((width (place_qft n k size_reg)) = size_reg).

Parameter result25: circuit.

Axiom result_def25 : (result25 = (sequence (place_qft n k size_reg) c)).

Axiom H9 :
  ((circ_to_pps result25) =
   (pps_seq (circ_to_pps (place_qft n k size_reg)) (circ_to_pps c))).

Axiom H10 :
  ((ancillas result25) =
   (ZArith.BinInt.Z.max (ancillas (place_qft n k size_reg)) (ancillas c))).

Axiom H11 :
  ((size result25) = ((size (place_qft n k size_reg)) + (size c))%Z).

Axiom H12 : ((width result25) = (width (place_qft n k size_reg))).

Axiom H13 :
  ((range result25) = ((range (place_qft n k size_reg)) + (range c))%Z).

Axiom H14 :
  forall (x:Z -> Z) (y:Z -> Z),
  ((ang_ind result25 x y) =
   (ang_add (ang_ind (place_qft n k size_reg) x y)
    (ang_ind c
     ((((fun (y0:circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
         (basis_ket y0 y1 y2 y3))
       ...
       x)
      y)
     (fun (k1:Z) => (y (k1 + (range (place_qft n k size_reg)))%Z))))).

Axiom H15 :
  forall (x:Z -> Z) (y:Z -> Z), forall (i:Z),
  ((basis_ket result25 x y i) =
   (basis_ket c
    ((((fun (y0:circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
        (basis_ket y0 y1 y2 y3))
       (place_qft n k size_reg))
      x)
     y)
    (fun (k1:Z) => (y (k1 + (range (place_qft n k size_reg)))%Z)) i)).

Axiom H16 :
  ((size result25) <= ((polysquare n cont_size 0%Z 0%Z) + (size c))%Z)%Z.

Axiom H17 : ((ancillas result25) = (ancillas c)).

Axiom H18 : ((width result25) = size_reg).

Parameter y: matrix complex.

Parameter z: matrix complex.

Parameter i: Z.

Axiom H19 : (0%Z <= i)%Z.

Axiom H20 : (i < (power_ 2%Z n))%Z.

Axiom H21 : is_a_ket_l y k.
...
Axiom Hinst :
  (sem (place_qft n k size_reg) (kronecker y (kronecker (ket n i) z))
   (kronecker y (kronecker (apply_qft n i) z))) ->
  (sem c (kronecker y (kronecker (apply_qft n i) z))
   (kronecker y (kronecker (apply_qft n (f i)) z))) ->
  ((width (place_qft n k size_reg)) = (width c)) ->
  sem (sequence (place_qft n k size_reg) c)
  (kronecker y (kronecker (ket n i) z))
  (kronecker y (kronecker (apply_qft n (f i)) z)).

(* Why3 goal *)
Theorem G : is_a_ket_l z ((size_reg - k)%Z - n)%Z.
Proof.
