...
Axiom result_def6 :
  forall (f:Z -> Z) (i:Z),
  (((0%Z <= (f i))%Z /\ ((f i) < 2%Z)%Z) -> (((result6 f) i) = (f i))) /\
  (~ ((0%Z <= (f i))%Z /\ ((f i) < 2%Z)%Z) -> (((result6 f) i) = 0%Z)).
...
Axiom k_bound :
  ...
  (infix_lseqdt
   (infix_asdt (infix_asdt (i_to_c k) (i_to_c (multi_order a bound)))
    (pow_inv_2 (2%Z * n)%Z))
   c_one) /\
  ((infix_lseqdt (infix_asdt (i_to_c k) (i_to_c (multi_order a bound)))
    (i_to_c (power_ 2%Z (2%Z * n)%Z))) /\
   ((infix_lseqdt
     (infix_asdt (i_to_c (k * (multi_order a bound))%Z)
      (pow_inv_2 (2%Z * n)%Z))
     c_one) /\
    (infix_lseqdt (i_to_c (k * (multi_order a bound))%Z)
     (i_to_c (power_ 2%Z (2%Z * n)%Z))))).
...
Axiom best_appr__def :
  ...
  (((power_ 2%Z (n - 1%Z)%Z) < bound)%Z /\ (bound < (power_ 2%Z n))%Z) ->
  (a < bound)%Z -> (co_prime a bound) ->
  ((0%Z <= k)%Z /\ (k < (multi_order a bound))%Z) ->
...
Axiom best_appr__spec :
  ...
  ((infix_lseqdt
   ...
   forall (b:Z), (0%Z < b)%Z ->
   (((b - 1%Z)%Z * (multi_order a bound))%Z <= (power_ 2%Z (2%Z * n)%Z))%Z ->
   infix_lseqdt
   (modulus
    (infix_mndt (i_to_c k)
     (infix_asdt (pow_inv_2 (2%Z * n)%Z)
...
Axiom H11 : (1%Z <= (multi_order a bound))%Z.

Axiom H12 : ((multi_order a bound) < bound)%Z.

Axiom H13 :
  ((int.EuclideanDivision.mod1 (power_ a (multi_order a bound)) bound) = 1%Z).

Axiom H14 : real_ (i_to_c (multi_order a bound)).

Axiom H15 : real_ (i_to_c k).
...
Axiom H22 :
  ((infix_asdt (pow_inv_2 (2%Z * n)%Z)
    (i_to_c ((-(best_appr_ k a bound n))%Z * (multi_order a bound))%Z))
   = c_zero).
  ...
  ((ind_sum
    (fun (y1:Z) =>
     (cpower
      ...
      y1))
    0%Z b)
   = (infix_asdt c_one (i_to_c b))).

(* Why3 goal *)
Theorem VC_best_appr_bound :
  infix_lseqdt
  (infix_asdt (cpower (i_to_c b) 2%Z)
   (infix_sldt (i_to_c 4%Z) (cpower pi 2%Z)))
  (cpower (modulus (infix_asdt c_one (i_to_c b))) 2%Z).
Proof.
