...
Axiom modular_inverse_spec :
  ...
  ((int.EuclideanDivision.mod1 (pick * (modular_inverse pick bound))%Z bound)
   = 1%Z).
...
Axiom modular_multiplier_spec :
  ...
  (((size (modular_multiplier a bound n)) <=
    (polysquare (n + 1%Z)%Z (12%Z * cont_size)%Z
     (cont_size * ((2%Z * (adder_gate_bound n))%Z + 12%Z)%Z)%Z 0%Z))%Z /\
   (((ancillas (modular_multiplier a bound n)) = (n + 1%Z)%Z) /\
...
Axiom multiplier_gate_bound_spec :
  ...
  ((1%Z < bound)%Z /\ (bound < (power_ 2%Z n))%Z) -> (a < bound)%Z ->
  (co_prime a bound) ->
...
Axiom control_eigen_seq_real_def :
  ...
  ((control_eigen_seq_real fcirc y fk fc37 ft bound s n) =
   (control_eigen_seq fcirc y (result21 fk) fc37 ft bound s n)).
...
Axiom apply_black_box_def :
  ...
  (n = (k + (width circ))%Z) -> (real_ theta) -> (1%Z <= k)%Z ->
...
Parameter phase_estimation_run:
  circuit -> Z -> Z -> (matrix t) -> t -> matrix t.

Axiom phase_estimation_run_def :
  forall (circ:circuit) (k:Z) (n:Z) (y:matrix t) (theta:t),
  (n = (k + (width circ))%Z) -> (real_ theta) -> (1%Z <= k)%Z ->
  (eigen circ y (real_to_ang theta)) ->
  ((phase_estimation_run circ k n y theta) =
   (pat_sem (qft_rev k) (discard_black_box circ k n y theta))).

Axiom phase_estimation_run_spec :
  forall (circ:circuit) (k:Z) (n:Z) (y:matrix t) (theta:t),
  (n = (k + (width circ))%Z) -> (real_ theta) -> (1%Z <= k)%Z ->
  (eigen circ y (real_to_ang theta)) ->
  (is_a_ket_l (phase_estimation_run circ k n y theta) k) /\
  (((phase_estimation_run circ k n y theta) =
    (infix_asdtdt (pow_inv_sqrt_2 k)
     (ket_sum_l (n_bvs k)
      (fun (y1:bitvec) =>
...
Axiom phase_estimation_spec :
  ...
  (n = (k + (width circ))%Z) -> (real_ theta) -> (1%Z <= k)%Z ->
  (eigen circ y (real_to_ang theta)) ->
  (sem (phase_estimation circ k n y theta) (kronecker (ket k 0%Z) y)
   (kronecker (phase_estimation_run circ k n y theta) y)) /\
  (((pat_sem (phase_estimation circ k n y theta) (kronecker (ket k 0%Z) y)) =
    (kronecker (phase_estimation_run circ k n y theta) y)) /\
   ((discard (phase_estimation_run circ k n y theta) y
     (pat_sem (phase_estimation circ k n y theta) (kronecker (ket k 0%Z) y))
     k n)
    = (phase_estimation_run circ k n y theta))).
...
Axiom pe_measure_def :
  ...
  ((pe_measure circ k i n y theta) =
   (proba_measure (phase_estimation_run circ k n y theta) i k)).
...
Parameter result32: t -> Z -> Z -> bool.

Axiom result_def32 :
  forall (theta:t) (k:Z) (x:Z),
  (((result32 theta k) x) = true) <->
  (infix_lseqdt
...
Axiom best_appr_pre_def :
  ...
  ((best_appr_pre theta k) =
   (choose_filter (to_fset 0%Z ((power_ 2%Z k) + 1%Z)%Z) (result32 theta k))).
...
Axiom best_appr_def :
  ...
  ((best_appr theta k) =
   (int_bit_inversion
    (int.EuclideanDivision.mod1 (best_appr_pre theta k) (power_ 2%Z k)) k)).
...
Axiom best_appr_spec :
  ...
  ((mem (best_appr theta k) (to_fset 0%Z ((power_ 2%Z k) + 1%Z)%Z)) /\
   ((real_to_ang
     (infix_mndt theta
      (infix_asdt (i_to_t (int_bit_inversion (best_appr theta k) k))
       (pow_inv_2 k))))
    =
    (real_to_ang
     (infix_mndt theta
...
Axiom delta_pre_spec :
  forall (theta:t) (k:Z), (0%Z < k)%Z -> (real_ theta) ->
  ((infix_lseqdt tzero theta) /\ (infix_lsdt theta tone)) ->
  ((real_to_ang (delta_pre theta k)) =
   (real_to_ang
    (infix_mndt theta
     (infix_asdt (i_to_t (best_appr_pre theta k)) (pow_inv_2 k))))) /\
  ((real_ (delta_pre theta k)) /\
   ((((best_appr_pre theta k) < (power_ 2%Z k))%Z ->
     infix_lseqdt (modulus (delta_pre theta k)) (pow_inv_2 (k + 1%Z)%Z)) /\
    (((best_appr_pre theta k) = (power_ 2%Z k)) ->
     infix_lseqdt (modulus (infix_mndt (delta_pre theta k) tone))
     (pow_inv_2 (k + 1%Z)%Z)))).
...
Axiom delta_def :
  ...
  ((infix_lseqdt tzero theta) /\ (infix_lsdt theta tone)) ->
  (((best_appr_pre theta k) = (power_ 2%Z k)) ->
   ((delta theta k) = (infix_mndt (delta_pre theta k) tone))) /\
  (~ ((best_appr_pre theta k) = (power_ 2%Z k)) ->
   ((delta theta k) = (delta_pre theta k))).
...
Axiom best_appr_measure_def :
  forall (circ:circuit) (k:Z) (n:Z) (y:matrix t) (theta:t), (0%Z < k)%Z ->
  (n = (k + (width circ))%Z) -> (real_ theta) ->
  ((infix_lseqdt tzero theta) /\ (infix_lsdt theta tone)) -> (1%Z <= k)%Z ->
  (eigen circ y (real_to_ang theta)) ->
  ((best_appr_measure circ k n y theta) =
   (pe_measure circ k (best_appr theta k) n y theta)).
   ...
   (infix_asdt (pow_inv_2 (2%Z * k)%Z)
    (cpower
     (modulus
      (sum (n_bvs k)
       (fun (y1:bitvec) =>
        (cpower (ang_exp (real_to_ang (delta theta k))) (bv_to_int y1)))))
     2%Z))) /\
  (((delta theta k) = tzero) ->
   ((best_appr_measure circ k n y theta) = tone)).

Parameter modular_expo: Z -> Z -> Z -> circuit.
...
Axiom modular_expo_spec :
  ...
  ((width (modular_expo a bound n)) = (3%Z * n)%Z) /\
  (((ancillas (modular_expo a bound n)) = (n + 1%Z)%Z) /\
   (((size (modular_expo a bound n)) <=
     (((cont_size * (multiplier_gate_bound n))%Z * 2%Z)%Z * n)%Z)%Z /\
    forall (x:bitvec), ((length x) = (2%Z * n)%Z) ->
    ((pat_sem (modular_expo a bound n) (kronecker (bv_to_ket x) (ket n 1%Z)))
     =
     (kronecker (bv_to_ket x)
...
Axiom shor_circ_pre_spec :
  ...
  (((ancillas (shor_circ_pre a bound n)) = (n + 1%Z)%Z) /\
...
Axiom shor_ket_def :
  ...
  ((shor_ket a bound n i j) =
   (kronecker
    (infix_asdtdt
     (int_qft_rev_coeff
      (int.EuclideanDivision.mod1 i (power_ 2%Z (2%Z * n)%Z))
      (int.EuclideanDivision.mod1 j (power_ 2%Z (2%Z * n)%Z)) (2%Z * n)%Z)
     (ket (2%Z * n)%Z j))
...
Axiom H7 :
  ((size (qft_rev (2%Z * n)%Z)) <=
   (polysquare (2%Z * n)%Z (2%Z * cont_size)%Z 0%Z 0%Z))%Z.

Axiom H8 : ((ancillas (qft_rev (2%Z * n)%Z)) = 0%Z).

Axiom H9 : ((ancillas (qft_rev (2%Z * n)%Z)) = 0%Z).

Axiom H10 : ((width (qft_rev (2%Z * n)%Z)) = (2%Z * n)%Z).

Axiom H11 : cont_zh (qft_rev (2%Z * n)%Z).

Axiom H12 : ((zh_range (qft_rev (2%Z * n)%Z)) = (2%Z * n)%Z).

Axiom H13 :
  forall (x:bitvec) (y:bitvec), ((length x) = (2%Z * n)%Z) ->
  ((length y) = (2%Z * n)%Z) ->
  ((zh_ang_coeff_neg (qft_rev (2%Z * n)%Z) x y) =
   (ind_isum
    (fun (l:Z) =>
     (ind_isum
      (fun (k:Z) =>
       ((((value x) k) * ((value y) l))%Z *
        (power_ 2%Z ((((2%Z * n)%Z - k)%Z - 1%Z)%Z + l)%Z))%Z)
      l (2%Z * n)%Z))
    0%Z (2%Z * n)%Z)).
...
Axiom H16 :
  forall (x:bitvec) (y:bitvec), ((length x) = (2%Z * n)%Z) ->
  ((length y) = (2%Z * n)%Z) -> forall (i:Z),
  ((0%Z <= i)%Z /\ (i < (2%Z * n)%Z)%Z) ->
  ((zh_ket_i (qft_rev (2%Z * n)%Z) x y i) = ((value y) i)).

Axiom H17 :
  forall (x:matrix t) (y:matrix t),
  (is_a_ket_l x (width (qft_rev (2%Z * n)%Z))) ->
  (is_a_ket_l y ((3%Z * n)%Z - (width (qft_rev (2%Z * n)%Z)))%Z) ->
  ((pat_sem (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z) (kronecker x y)) =
   (kronecker (pat_sem (qft_rev (2%Z * n)%Z) x) y)).

Axiom H18 :
  forall (f:bitvec), (mem f (n_bvs (2%Z * n)%Z)) ->
  ((length f) = (2%Z * n)%Z).

Axiom H19 :
  forall (f:bitvec),
  (mem f (n_bvs (2%Z * n)%Z)) <-> ((length f) = (2%Z * n)%Z).

Axiom H20 : (0%Z < (cardinal (n_bvs (2%Z * n)%Z)))%Z.

Parameter result33: bitvec -> bitvec -> matrix t.

Axiom result_def33 :
  forall (i:bitvec) (j:bitvec),
  ((((length i) = (length j)) /\ ((length i) = (2%Z * n)%Z)) ->
   (((result33 i) j) =
    (kronecker (infix_asdtdt (qft_rev_coeff i j (2%Z * n)%Z) (bv_to_ket j))
     (ket n (int.EuclideanDivision.mod1 (power_ a (bv_to_int i)) bound))))) /\
  (~ (((length i) = (length j)) /\ ((length i) = (2%Z * n)%Z)) ->
   (((result33 i) j) = (ket (3%Z * n)%Z 0%Z))).

Axiom H21 :
  is_a_ket
  (ket_sum_l (n_bvs (2%Z * n)%Z)
   (fun (i:bitvec) =>
    (ket_sum_l (n_bvs (2%Z * n)%Z) (result33 i) (3%Z * n)%Z))
   (3%Z * n)%Z).

Parameter result34: bitvec -> bitvec -> matrix t.

Parameter result35: bitvec -> bitvec -> matrix t.

Parameter result36: bitvec -> bitvec -> matrix t.
...
Axiom H22 :
  ...
  (valid_index
   (ket_sum_l (n_bvs (2%Z * n)%Z)
    (fun (i1:bitvec) =>
     (ket_sum_l (n_bvs (2%Z * n)%Z) (result34 i1) (3%Z * n)%Z))
    (3%Z * n)%Z)
   i 0%Z) ->
  ((get
    (ket_sum_l (n_bvs (2%Z * n)%Z)
     (fun (i1:bitvec) =>
      (ket_sum_l (n_bvs (2%Z * n)%Z) (result35 i1) (3%Z * n)%Z))
     (3%Z * n)%Z)
    i 0%Z)
   =
   (sum (n_bvs (2%Z * n)%Z)
    (fun (e:bitvec) =>
     (get
      ((fun (i1:bitvec) =>
        (ket_sum_l (n_bvs (2%Z * n)%Z) (result36 i1) (3%Z * n)%Z))
       e)
      i 0%Z)))).

Parameter result37: bitvec -> bitvec -> matrix t.
  ...
  ((((length i) = (length j)) /\ ((length i) = (2%Z * n)%Z)) ->
   (((result37 i) j) =
    (kronecker (infix_asdtdt (qft_rev_coeff i j (2%Z * n)%Z) (bv_to_ket j))
     (ket n (int.EuclideanDivision.mod1 (power_ a (bv_to_int i)) bound))))) /\
  (~ (((length i) = (length j)) /\ ((length i) = (2%Z * n)%Z)) ->
   (((result37 i) j) = (ket (3%Z * n)%Z 0%Z))).

Axiom H23 :
  is_a_ket_l
  (ket_sum_l (n_bvs (2%Z * n)%Z)
   (fun (i:bitvec) =>
    (ket_sum_l (n_bvs (2%Z * n)%Z) (result37 i) (3%Z * n)%Z))
   (3%Z * n)%Z)
  (3%Z * n)%Z.

Parameter result38: bitvec -> bitvec -> matrix t.
  ...
  ((((length i) = (length j)) /\ ((length i) = (2%Z * n)%Z)) ->
   (((result38 i) j) =
    (kronecker (infix_asdtdt (qft_rev_coeff i j (2%Z * n)%Z) (bv_to_ket j))
     (ket n (int.EuclideanDivision.mod1 (power_ a (bv_to_int i)) bound))))) /\
  (~ (((length i) = (length j)) /\ ((length i) = (2%Z * n)%Z)) ->
   (((result38 i) j) = (ket (3%Z * n)%Z 0%Z))).

Axiom H24 :
  ((ket_length
    (ket_sum_l (n_bvs (2%Z * n)%Z)
     (fun (i:bitvec) =>
      (ket_sum_l (n_bvs (2%Z * n)%Z) (result38 i) (3%Z * n)%Z))
     (3%Z * n)%Z))
   = (3%Z * n)%Z).

Axiom H25 : real_ (pow_inv_sqrt_2 (2%Z * n)%Z).

Axiom H26 :
  ((pow_inv_sqrt_2 (2%Z * n)%Z) =
   (infix_sldt tone (square_rt (i_to_t (power 2%Z (2%Z * n)%Z))))).

Parameter result39: bitvec -> bitvec -> matrix t.

Parameter result40: bitvec -> bitvec -> matrix t.
...
Axiom H27 :
  ((columns
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (ket_sum_l (n_bvs (2%Z * n)%Z)
      (fun (i:bitvec) =>
       (ket_sum_l (n_bvs (2%Z * n)%Z) (result39 i) (3%Z * n)%Z))
      (3%Z * n)%Z)))
   =
   (columns
    (ket_sum_l (n_bvs (2%Z * n)%Z)
     (fun (i:bitvec) =>
      (ket_sum_l (n_bvs (2%Z * n)%Z) (result40 i) (3%Z * n)%Z))
     (3%Z * n)%Z))).

Parameter result41: bitvec -> bitvec -> matrix t.

Parameter result42: bitvec -> bitvec -> matrix t.
...
Axiom H28 :
  ((rows
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (ket_sum_l (n_bvs (2%Z * n)%Z)
      (fun (i:bitvec) =>
       (ket_sum_l (n_bvs (2%Z * n)%Z) (result41 i) (3%Z * n)%Z))
      (3%Z * n)%Z)))
   =
   (rows
    (ket_sum_l (n_bvs (2%Z * n)%Z)
     (fun (i:bitvec) =>
      (ket_sum_l (n_bvs (2%Z * n)%Z) (result42 i) (3%Z * n)%Z))
     (3%Z * n)%Z))).

Parameter result43: bitvec -> bitvec -> matrix t.

Parameter result44: bitvec -> bitvec -> matrix t.

Parameter result45: bitvec -> bitvec -> matrix t.
...
Axiom H29 :
  ...
  (valid_index
   (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
    (ket_sum_l (n_bvs (2%Z * n)%Z)
     (fun (i1:bitvec) =>
      (ket_sum_l (n_bvs (2%Z * n)%Z) (result43 i1) (3%Z * n)%Z))
     (3%Z * n)%Z))
   i j) ->
  ((get
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (ket_sum_l (n_bvs (2%Z * n)%Z)
      (fun (i1:bitvec) =>
       (ket_sum_l (n_bvs (2%Z * n)%Z) (result44 i1) (3%Z * n)%Z))
      (3%Z * n)%Z))
    i j)
   =
   (infix_asdt (pow_inv_sqrt_2 (2%Z * n)%Z)
    (get
     (ket_sum_l (n_bvs (2%Z * n)%Z)
      (fun (i1:bitvec) =>
       (ket_sum_l (n_bvs (2%Z * n)%Z) (result45 i1) (3%Z * n)%Z))
      (3%Z * n)%Z)
     i j))).

Parameter result46: bitvec -> bitvec -> matrix t.

Parameter result47: bitvec -> bitvec -> matrix t.
...
Axiom H30 :
  ...
  (valid_index
   (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
    (ket_sum_l (n_bvs (2%Z * n)%Z)
     (fun (i1:bitvec) =>
      (ket_sum_l (n_bvs (2%Z * n)%Z) (result46 i1) (3%Z * n)%Z))
     (3%Z * n)%Z))
   i j) <->
  (valid_index
   (ket_sum_l (n_bvs (2%Z * n)%Z)
    (fun (i1:bitvec) =>
     (ket_sum_l (n_bvs (2%Z * n)%Z) (result47 i1) (3%Z * n)%Z))
    (3%Z * n)%Z)
   i j).

Axiom H31 : real_ (pow_inv_sqrt_2 (2%Z * n)%Z).

Axiom H32 :
  ((pow_inv_sqrt_2 (2%Z * n)%Z) =
   (infix_sldt tone (square_rt (i_to_t (power 2%Z (2%Z * n)%Z))))).

Parameter result48: bitvec -> bitvec -> matrix t.

Parameter result49: bitvec -> bitvec -> matrix t.
  ...
  ((columns
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
      (ket_sum_l (n_bvs (2%Z * n)%Z)
       (fun (i:bitvec) =>
        (ket_sum_l (n_bvs (2%Z * n)%Z) (result48 i) (3%Z * n)%Z))
       (3%Z * n)%Z))))
   =
   (columns
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (ket_sum_l (n_bvs (2%Z * n)%Z)
      (fun (i:bitvec) =>
       (ket_sum_l (n_bvs (2%Z * n)%Z) (result49 i) (3%Z * n)%Z))
      (3%Z * n)%Z)))).

Parameter result50: bitvec -> bitvec -> matrix t.

Parameter result51: bitvec -> bitvec -> matrix t.
  ...
  ((rows
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
      (ket_sum_l (n_bvs (2%Z * n)%Z)
       (fun (i:bitvec) =>
        (ket_sum_l (n_bvs (2%Z * n)%Z) (result50 i) (3%Z * n)%Z))
       (3%Z * n)%Z))))
   =
   (rows
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (ket_sum_l (n_bvs (2%Z * n)%Z)
      (fun (i:bitvec) =>
       (ket_sum_l (n_bvs (2%Z * n)%Z) (result51 i) (3%Z * n)%Z))
      (3%Z * n)%Z)))).

Parameter result52: bitvec -> bitvec -> matrix t.

Parameter result53: bitvec -> bitvec -> matrix t.

Parameter result54: bitvec -> bitvec -> matrix t.
  ...
  (valid_index
   (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (ket_sum_l (n_bvs (2%Z * n)%Z)
      (fun (i1:bitvec) =>
       (ket_sum_l (n_bvs (2%Z * n)%Z) (result52 i1) (3%Z * n)%Z))
      (3%Z * n)%Z)))
   i j) ->
  ((get
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
      (ket_sum_l (n_bvs (2%Z * n)%Z)
       (fun (i1:bitvec) =>
        (ket_sum_l (n_bvs (2%Z * n)%Z) (result53 i1) (3%Z * n)%Z))
       (3%Z * n)%Z)))
    i j)
   =
   (infix_asdt (pow_inv_sqrt_2 (2%Z * n)%Z)
    (get
     (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
      (ket_sum_l (n_bvs (2%Z * n)%Z)
       (fun (i1:bitvec) =>
        (ket_sum_l (n_bvs (2%Z * n)%Z) (result54 i1) (3%Z * n)%Z))
       (3%Z * n)%Z))
     i j))).

Parameter result55: bitvec -> bitvec -> matrix t.

Parameter result56: bitvec -> bitvec -> matrix t.
  ...
  (valid_index
   (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
    (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
     (ket_sum_l (n_bvs (2%Z * n)%Z)
      (fun (i1:bitvec) =>
       (ket_sum_l (n_bvs (2%Z * n)%Z) (result55 i1) (3%Z * n)%Z))
      (3%Z * n)%Z)))
   i j) <->
  (valid_index
   (infix_asdtdt (pow_inv_sqrt_2 (2%Z * n)%Z)
    (ket_sum_l (n_bvs (2%Z * n)%Z)
     (fun (i1:bitvec) =>
      (ket_sum_l (n_bvs (2%Z * n)%Z) (result56 i1) (3%Z * n)%Z))
     (3%Z * n)%Z))
   i j).
...
Axiom H39 : (0%Z < (cardinal (n_bvs (2%Z * n)%Z)))%Z.

Axiom H40 :
  is_a_ket
  (ket_sum_l (n_bvs (2%Z * n)%Z)
   (fun (i:bitvec) =>
    (kronecker (bv_to_ket i)
     (ket n (int.EuclideanDivision.mod1 (power_ a (bv_to_int i)) bound))))
   (3%Z * n)%Z).
...
Axiom H41 :
  ...
  (valid_index
   (ket_sum_l (n_bvs (2%Z * n)%Z)
    (fun (i1:bitvec) =>
     (kronecker (bv_to_ket i1)
      (ket n (int.EuclideanDivision.mod1 (power_ a (bv_to_int i1)) bound))))
    (3%Z * n)%Z)
   i 0%Z) ->
  ((get
    (ket_sum_l (n_bvs (2%Z * n)%Z)
...
Axiom H44 : real_ (pow_inv_sqrt_2 (2%Z * n)%Z).
...
Axiom H50 : is_a_ket (ket n 1%Z).

Axiom H51 : ((ket_length (ket n 1%Z)) = n).

Axiom H52 : ((columns (ket n 1%Z)) = 1%Z).

Axiom H53 : ((rows (ket n 1%Z)) = (power 2%Z n)).

Axiom H54 :
  ((0%Z <= 1%Z)%Z /\ (1%Z < (power 2%Z n))%Z) -> forall (j:Z),
  (valid_index (ket n 1%Z) j 0%Z) ->
  ((get (ket n 1%Z) j 0%Z) = (indic j 1%Z)).

Axiom H55 :
  forall (j:Z), (valid_index (ket n 1%Z) j 0%Z) -> ~ (1%Z = j) ->
  ((get (ket n 1%Z) j 0%Z) = tzero).

Axiom H56 :
  ((0%Z <= 1%Z)%Z /\ (1%Z < (power 2%Z n))%Z) ->
  ((get (ket n 1%Z) 1%Z 0%Z) = tone).

Axiom H57 : is_a_ket (ket (2%Z * n)%Z 0%Z).

Axiom H58 : ((ket_length (ket (2%Z * n)%Z 0%Z)) = (2%Z * n)%Z).

Axiom H59 : ((columns (ket (2%Z * n)%Z 0%Z)) = 1%Z).

Axiom H60 : ((rows (ket (2%Z * n)%Z 0%Z)) = (power 2%Z (2%Z * n)%Z)).

Axiom H61 :
  ((0%Z <= 0%Z)%Z /\ (0%Z < (power 2%Z (2%Z * n)%Z))%Z) -> forall (j:Z),
  (valid_index (ket (2%Z * n)%Z 0%Z) j 0%Z) ->
  ((get (ket (2%Z * n)%Z 0%Z) j 0%Z) = (indic j 0%Z)).

Axiom H62 :
  forall (j:Z), (valid_index (ket (2%Z * n)%Z 0%Z) j 0%Z) -> ~ (0%Z = j) ->
  ((get (ket (2%Z * n)%Z 0%Z) j 0%Z) = tzero).
...
Axiom H64 :
  ((rows (kronecker (ket (2%Z * n)%Z 0%Z) (ket n 1%Z))) =
   ((rows (ket (2%Z * n)%Z 0%Z)) * (rows (ket n 1%Z)))%Z).

Axiom H65 :
  ((columns (kronecker (ket (2%Z * n)%Z 0%Z) (ket n 1%Z))) =
   ((columns (ket (2%Z * n)%Z 0%Z)) * (columns (ket n 1%Z)))%Z).

Axiom H66 :
  forall (i:Z) (j:Z),
  (valid_index (kronecker (ket (2%Z * n)%Z 0%Z) (ket n 1%Z)) i j) ->
  ((get (kronecker (ket (2%Z * n)%Z 0%Z) (ket n 1%Z)) i j) =
   (infix_asdt
    (get (ket (2%Z * n)%Z 0%Z)
     (int.EuclideanDivision.div i (rows (ket n 1%Z)))
     (int.EuclideanDivision.div j (columns (ket n 1%Z))))
    (get (ket n 1%Z) (int.EuclideanDivision.mod1 i (rows (ket n 1%Z)))
     (int.EuclideanDivision.mod1 j (columns (ket n 1%Z)))))).

Axiom H67 :
  ((size (qft_rev (2%Z * n)%Z)) <=
   (polysquare (2%Z * n)%Z (2%Z * cont_size)%Z 0%Z 0%Z))%Z.

Axiom H68 : ((ancillas (qft_rev (2%Z * n)%Z)) = 0%Z).

Axiom H69 : ((ancillas (qft_rev (2%Z * n)%Z)) = 0%Z).

Axiom H70 : ((width (qft_rev (2%Z * n)%Z)) = (2%Z * n)%Z).

Axiom H71 : cont_zh (qft_rev (2%Z * n)%Z).

Axiom H72 : ((zh_range (qft_rev (2%Z * n)%Z)) = (2%Z * n)%Z).
...
Axiom H77 :
  ((ancillas (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z)) =
   (ancillas (qft_rev (2%Z * n)%Z))).

Axiom H78 :
  ((size (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z)) =
   (size (qft_rev (2%Z * n)%Z))).

Axiom H79 :
  ((range (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z)) =
   (range (qft_rev (2%Z * n)%Z))).

Axiom H80 :
  ((width (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z)) = (3%Z * n)%Z).

Axiom H81 :
  forall (x:bitvec) (y:bitvec), forall (i:Z), ((length x) = (3%Z * n)%Z) ->
  ((length y) = (range (qft_rev (2%Z * n)%Z))) ->
  ((0%Z <= i)%Z /\ (i < (3%Z * n)%Z)%Z) ->
  (((0%Z <= i)%Z /\ (i < (0%Z + (width (qft_rev (2%Z * n)%Z)))%Z)%Z) ->
   ((basis_ket_i (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z) x y i) =
    (basis_ket_i (qft_rev (2%Z * n)%Z)
     (htpart x 0%Z (width (qft_rev (2%Z * n)%Z))) y (i - 0%Z)%Z))) /\
  (~ ((0%Z <= i)%Z /\ (i < (0%Z + (width (qft_rev (2%Z * n)%Z)))%Z)%Z) ->
   ((basis_ket_i (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z) x y i) =
    ((value x) i))).
  ...
  (((0%Z <= i)%Z /\ (i < (0%Z + (width (qft_rev (2%Z * n)%Z)))%Z)%Z) ->
   (((fc42 x y) i) =
    (basis_ket_i (qft_rev (2%Z * n)%Z)
     (htpart x 0%Z (width (qft_rev (2%Z * n)%Z))) y (i - 0%Z)%Z))) /\
  (~ ((0%Z <= i)%Z /\ (i < (0%Z + (width (qft_rev (2%Z * n)%Z)))%Z)%Z) ->
   (((fc42 x y) i) = ((value x) i))).

Axiom H82 :
  forall (x:bitvec) (y:bitvec), ((length x) = (3%Z * n)%Z) ->
  ((length y) = (range (qft_rev (2%Z * n)%Z))) ->
  ((basis_ket (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z) x y) =
   (make_bv (fc42 x y) (3%Z * n)%Z)).

Axiom H83 :
  forall (x:bitvec) (y:bitvec), ((length x) = (3%Z * n)%Z) ->
  ((length y) = (ang_ind_bound (qft_rev (2%Z * n)%Z))) ->
  ((ang_ind (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z) x y) =
   (ang_ind (qft_rev (2%Z * n)%Z)
    (htpart x 0%Z (width (qft_rev (2%Z * n)%Z))) y)).

Axiom H84 : ((width (shor_circ_pre a bound n)) = (3%Z * n)%Z).

Axiom H85 : ((ancillas (shor_circ_pre a bound n)) = (n + 1%Z)%Z).
...
Axiom H88 :
  ((width (shor_circ_pre a bound n)) =
   (width (place (qft_rev (2%Z * n)%Z) 0%Z (3%Z * n)%Z))).
...
Axiom H90 : mem a1 (n_bvs (2%Z * n)%Z).

(* Why3 goal *)
Theorem G :
  (((2%Z * n)%Z +
    (ket_length
     (ket n (int.EuclideanDivision.mod1 (power_ a (bv_to_int a1)) bound))))%Z
   = (3%Z * n)%Z).
Proof.
