...
Axiom ang_sum_sum_bv_to_int_mult_gen :
  forall (n:Z), forall (x:bitvec) (y:bitvec), ((length x) = n) ->
  ((length y) = n) ->
  ((ang_sum
    (fun (target:Z) =>
     (ang_sum
...
Axiom place_qft_def :
  ...
  ((place_qft n k) = (place (qft n) 0%Z (n + k)%Z)).

Axiom place_qft_spec :
  forall (n:Z) (k:Z), (0%Z < n)%Z -> (0%Z < k)%Z ->
  ((width (place_qft n k)) = (n + k)%Z) /\
  (((size (place_qft n k)) <= (polysquare n cont_size 0%Z 0%Z))%Z /\
   (((ancillas (place_qft n k)) = 0%Z) /\
    ((forall (g:Z), forall (y:matrix complex),
      ((0%Z <= g)%Z /\ (g < (power_ 2%Z n))%Z) -> (is_a_ket_l y k) ->
      sem (place_qft n k) (kronecker (ket n g) y)
      (kronecker (apply_qft n g) y)) /\
     ((width (place_qft n k)) = (n + k)%Z)))).

Parameter place_rev_qft: Z -> Z -> circuit.

Axiom place_rev_qft_def :
  forall (n:Z) (k:Z), (0%Z < n)%Z -> (0%Z < k)%Z ->
  ((place_rev_qft n k) = (reverse (place_qft n k))).

Axiom place_rev_qft_spec :
  forall (n:Z) (k:Z), (0%Z < n)%Z -> (0%Z < k)%Z ->
  ((width (place_rev_qft n k)) = (n + k)%Z) /\
  (((size (place_rev_qft n k)) <= (polysquare n cont_size 0%Z 0%Z))%Z /\
   (((ancillas (place_rev_qft n k)) = 0%Z) /\
    forall (g:Z), forall (y:matrix complex),
    ((0%Z <= g)%Z /\ (g < (power_ 2%Z n))%Z) -> (is_a_ket_l y k) ->
    sem (place_rev_qft n k) (kronecker (apply_qft n g) y)
    (kronecker (ket n g) y))).

Parameter apply_from_qft: Z -> Z -> circuit -> circuit.

Axiom apply_from_qft_spec :
  forall (n:Z) (k:Z) (c:circuit), (0%Z < n)%Z -> (0%Z < k)%Z ->
  ((width c) = (n + k)%Z) ->
  ((size (apply_from_qft n k c)) <=
   ((polysquare n (cont_size * 2%Z)%Z 0%Z 0%Z) + (size c))%Z)%Z /\
  (((ancillas (apply_from_qft n k c)) = (ancillas c)) /\
   (((width (apply_from_qft n k c)) = (n + k)%Z) /\
    ...
    sem (apply_from_qft n k c) (kronecker (apply_qft n i) y)
    (kronecker (apply_qft n i) z))).

Parameter apply_in_qft: Z -> Z -> circuit -> circuit.

Axiom apply_in_qft_spec :
  forall (n:Z) (k:Z) (c:circuit), (0%Z < n)%Z -> (0%Z < k)%Z ->
  ((width c) = (n + k)%Z) ->
  ((size (apply_in_qft n k c)) <=
   ((polysquare n (cont_size * 2%Z)%Z 0%Z 0%Z) + (size c))%Z)%Z /\
  (((ancillas (apply_in_qft n k c)) = (ancillas c)) /\
   (((width (apply_in_qft n k c)) = (n + k)%Z) /\
    forall (y:matrix complex) (z:matrix complex), forall (i:Z),
    ((0%Z <= i)%Z /\ (i < (power_ 2%Z n))%Z) -> (is_a_ket_l y k) ->
    (is_a_ket_l z k) ->
    (sem c (kronecker (apply_qft n i) y) (kronecker (apply_qft n i) z)) ->
    sem (apply_in_qft n k c) (kronecker (ket n i) y) (kronecker (ket n i) z))).
...
Parameter result29: circuit.

Parameter o: circuit.

Axiom H4 : (k = 0%Z).

Axiom H5 : (result29 = (cascade_adder_ 0%Z n (n - 1%Z)%Z y)).

Axiom H6 : ((ancillas result29) = 0%Z).
...
Axiom H7 :
  ...
  (fun (x:bitvec) =>
   (ang_sum
    (fun (k1:Z) =>
     (int_to_ang
      ((((value y) k1) * ((value x) 0%Z))%Z *
       (power_ 2%Z (((n - k1)%Z - 1%Z)%Z + ((n - 0%Z)%Z - 1%Z)%Z)%Z))%Z
      n))
    ((n - 0%Z)%Z - 1%Z)%Z ((n - 1%Z)%Z + 1%Z)%Z)).

Axiom H8 : ((size result29) = (((n - 1%Z)%Z - (n - 0%Z)%Z)%Z + 2%Z)%Z).

Axiom H9 : ((width result29) = n).

Axiom H10 : ((ancillas result29) = 0%Z).

Parameter x: bitvec.

Axiom H11 : ((length x) = (width result29)).

(* Why3 goal *)
Theorem G :
  ((path_sem result29 (bv_to_ket x)) =
   (infix_asdtdt
    (ang_exp
     ((fun (x1:bitvec) =>
       (ang_sum
        (fun (target:Z) =>
         (ang_sum
          (fun (k1:Z) =>
           (int_to_ang
            ((((value y) k1) * ((value x1) target))%Z *
             (power_ 2%Z (((n - k1)%Z - 1%Z)%Z + ((n - target)%Z - 1%Z)%Z)%Z))%Z
            n))
          ((n - target)%Z - 1%Z)%Z n))
        0%Z (k + 1%Z)%Z))
      x))
