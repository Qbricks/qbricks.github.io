    ...
    (((width (place_xx c n)) = n) /\
     ((forall (x:bitvec), ((length x) = 1%Z) ->
      ...
      ((forall (x:bitvec), ((length x) = n) ->
       ...
       forall (x:bitvec), forall (i:Z), ((length x) = n) ->
       ((0%Z <= i)%Z /\ (i < n)%Z) -> ~ (i = c) ->
       ((flat_ket_i (place_xx c n) x c) = ((value x) i))))))).
...
Parameter fc15: (Z -> t) -> Z -> Z -> t.

Axiom fc_def15 :
  forall (f:Z -> t) (i:Z) (j:Z),
  ((i = j) -> ((((fc15 f) i) j) = (f i))) /\
  (~ (i = j) -> ((((fc15 f) i) j) = tzero)).

(* Why3 assumption *)
Definition is_diagonal (m:matrix t) : Prop :=
  exists f:Z -> t, (m = (make_f (rows m) (rows m) (fc15 f))).
...
Parameter fc16: Z -> Z -> t.

Axiom fc_def16 :
  forall (i:Z) (j:Z),
  ((j = i) -> (((fc16 i) j) = tone)) /\ (~ (j = i) -> (((fc16 i) j) = tzero)).
        ...
        (((projection_basis_l m i l) = (ketbra l i i)) /\
         ((forall (i1:Z) (j:Z),
          ...
          ((forall (y:matrix t), (is_a_ket_l y l) ->
           ...
           ((projection_basis_l m i l) = (diag_mat1 (fc16 i) (power 2%Z l)))))))))))).
...
Parameter fc17: Z -> Z -> t.

Axiom fc_def17 :
  forall (i:Z) (j:Z),
  ((j = i) -> (((fc17 i) j) = tone)) /\ (~ (j = i) -> (((fc17 i) j) = tzero)).
       ...
       (((columns (ket_projection_basis l i)) = (power 2%Z l)) /\
        ((forall (y:matrix t), (is_a_ket_l y l) ->
          ((mat_mult (ket_projection_basis l i) y) =
           (infix_asdtdt (get y i 0%Z) (ket l i)))) /\
         ((ket_projection_basis l i) = (diag_mat1 (fc17 i) (power 2%Z l)))))))))).

(* Why3 assumption *)
Definition projection_bv (x:bitvec) : matrix t :=
  ket_projection_basis (length x) (bv_to_int x).

Parameter fc18: bitvec -> Z -> t.

Axiom fc_def18 :
  forall (x:bitvec) (j:Z),
  ((j = (bv_to_int x)) -> (((fc18 x) j) = tone)) /\
  (~ (j = (bv_to_int x)) -> (((fc18 x) j) = tzero)).
      ...
      (((rows (projection_bv x)) = (power 2%Z (length x))) /\
       (((columns (projection_bv x)) = (power 2%Z (length x))) /\
        ((forall (y:matrix t), (is_a_ket_l y (length x)) ->
         ...
         ((projection_bv x) = (diag_mat1 (fc18 x) (power 2%Z (length x))))))))))).
       ...
       ((forall (y:matrix t), (is_a_ket_l y (length x)) ->
         ((mat_mult (partial_projection_bv x n) y) =
          (ket_sum_l (n_bvs n)
           ...
           ((length x) + n)%Z))) /\
        ((partial_projection_bv x n) =
         (diag_mat1 (fc19 x n) (power 2%Z ((length x) + n)%Z))))))))).
...
Axiom had_zh_ket :
  forall (t1:Z) (n:Z), ((0%Z <= t1)%Z /\ (t1 < n)%Z) ->
  forall (x:bitvec) (y:bitvec), ((length x) = n) -> ((length y) = 1%Z) ->
  ((zh_ket (had_zh_ t1 n) x y) = (make_bv (fc20 t1 x y) n)).
      ...
      (((zh_range (had_zh t1 n)) = 1%Z) /\
       ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
        ...
        ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
         ...
         ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
           ((length y) = 1%Z) ->
           ((zh_ket (had_zh t1 n) x y) = (make_bv (fc21 t1 x y) n))) /\
          ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
            ((length y) = 1%Z) ->
      ...
      ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
        ...
        ((zh_ang (had_seq_zh g t1 n) x y) =
         (ang_add (int_to_ang (((value x) t1) * ((value y) 0%Z))%Z 1%Z)
          (zh_ang g (make_bv (fc22 t1 x y) n) (tpart y 1%Z))))) /\
       ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
         ((length y) = ((zh_range g) + 1%Z)%Z) ->
         ((zh_ang_coeff_neg (had_seq_zh g t1 n) x y) =
          (((((value x) t1) * ((value y) 0%Z))%Z *
            (power_ 2%Z (n - 1%Z)%Z))%Z
           + (zh_ang_coeff_neg g (make_bv (fc23 t1 x y) n) (tpart y 1%Z)))%Z)) /\
        ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
          ((length y) = ((zh_range g) + 1%Z)%Z) ->
         ...
         ((zh_ket_i (had_seq_zh g t1 n) x y i) =
          (zh_ket_i g (make_bv (fc25 t1 x y) n) (tpart y 1%Z) i))))))))).
     ...
     (((zh_range (had_seq_diag_zh g t1 n)) = 1%Z) /\
      ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
        ...
        ((zh_ang (had_seq_diag_zh g t1 n) x y) =
         (ang_add (int_to_ang (((value x) t1) * ((value y) 0%Z))%Z 1%Z)
          (zh_ang g (make_bv (fc26 t1 x y) n) (tpart y 1%Z))))) /\
       ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
         ((length y) = ((zh_range g) + 1%Z)%Z) ->
         ((zh_ang_coeff (had_seq_diag_zh g t1 n) x y) =
          (((((value x) t1) * ((value y) 0%Z))%Z *
            (power_ 2%Z (n - 1%Z)%Z))%Z
           + (zh_ang_coeff g (make_bv (fc27 t1 x y) n) (tpart y 1%Z)))%Z)) /\
        ((forall (x:bitvec) (y:bitvec), ((length x) = n) ->
          ((length y) = ((zh_range g) + 1%Z)%Z) ->
          ((zh_ang_coeff_neg (had_seq_diag_zh g t1 n) x y) =
           (((((value x) t1) * ((value y) 0%Z))%Z *
             (power_ 2%Z (n - 1%Z)%Z))%Z
            + (zh_ang_coeff_neg g (make_bv (fc28 t1 x y) n) (tpart y 1%Z)))%Z)) /\
         forall (x:bitvec) (y:bitvec), ((length x) = n) ->
         ((length y) = ((zh_range g) + 1%Z)%Z) -> forall (i:Z),
         ((0%Z <= i)%Z /\ (i < n)%Z) ->
         ((i = t1) ->
...
Axiom H4 : (0%Z <= (width circ))%Z.

Axiom H5 : (0%Z <= (width circ))%Z.

Axiom H6 :
  forall (i:Z), ((0%Z <= i)%Z /\ (i < ft)%Z) ->
  (0%Z <= ((fun (i1:Z) => i1) i))%Z /\ (((fun (i1:Z) => i1) i) < ft)%Z.

Axiom H7 : (0%Z < ft)%Z.

Axiom H8 : (ft <= ft)%Z.

Axiom H9 : (0%Z <= (width circ))%Z.

Axiom H10 : True.

Axiom H11 : (0%Z <= (ft + (width circ))%Z)%Z.
...
(* Why3 goal *)
Theorem VC_cascade_cont_pow :
  ((0%Z <= ((ft - i)%Z - 1%Z)%Z)%Z ->
   ((width (pow_pow_2 circ ((ft - i)%Z - 1%Z)%Z y (real_to_ang theta))) =
    (width circ))) /\
  (~ (0%Z <= ((ft - i)%Z - 1%Z)%Z)%Z ->
   ((width (pow_pow_2 circ 0%Z y (real_to_ang theta))) = (width circ))).
Proof.
