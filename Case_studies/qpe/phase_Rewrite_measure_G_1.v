...
Parameter result33: t -> t -> Z -> Z -> t.

Axiom result_def33 :
  forall (a:t) (b:t) (x:Z) (us:Z),
  ((x = 0%Z) -> ((((result33 a b) x) us) = a)) /\
  (~ (x = 0%Z) -> ((((result33 a b) x) us) = b)).

(* Why3 assumption *)
Definition to_binary_vector (a:t) (b:t) : matrix t :=
  make_f 2%Z 1%Z (result33 a b).
...
Parameter result34: Z -> Z -> t.

Parameter result35: Z -> Z -> t.

Axiom result_def34 :
  forall (x:Z) (us:Z),
  ((x = 0%Z) -> (((result34 x) us) = tone)) /\
  (~ (x = 0%Z) -> (((result34 x) us) = tzero)).

Axiom result_def35 :
  forall (x:Z) (us:Z),
  ((x = 1%Z) -> (((result35 x) us) = im)) /\
  (~ (x = 1%Z) -> (((result35 x) us) = tzero)).

Axiom complex_plan_def :
  ((ox complex_plan) = (make_f 2%Z 1%Z result34)) /\
  (((oy complex_plan) = (make_f 2%Z 1%Z result35)) /\
   ((dim complex_plan) = 1%Z)).

Parameter result36: t -> Z -> Z -> t.

Axiom result_def36 :
  forall (x:t) (i:Z) (us:Z),
  ((i = 0%Z) -> ((((result36 x) i) us) = (t_real_part x))) /\
  (~ (i = 0%Z) -> ((((result36 x) i) us) = (t_im_part x))).

(* Why3 assumption *)
Definition complex_as_a_vector (x:t) : matrix t :=
  make_f 2%Z 1%Z (result36 x).
...
Axiom geom_sum_rewrite :
  ...
  ((infix_lsdt tzero (modulus psi)) /\ (infix_lseqdt (modulus psi) tone)) ->
  ((cpower
    (modulus
...
Axiom H2 : infix_lsdt tzero (modulus psi).

Axiom H3 :
  infix_lseqdt (modulus psi)
  (infix_sldt tone (infix_asdt (infix_pldt tone tone) (i_to_t b))).

Axiom H4 : real_ (i_to_t b).

Axiom H5 : infix_lseqdt tzero (modulus psi).

Axiom H6 : infix_lseqdt tzero (modulus psi).

Axiom H7 : infix_lseqdt (modulus psi) tone.
...
Axiom H9 : infix_lseqdt tzero (modulus psi).

Axiom H10 :
  infix_lseqdt (modulus (sin (infix_asdt pi (modulus psi))))
  (modulus (infix_asdt pi (modulus psi))).

Axiom H11 : infix_lseqdt tzero (modulus (infix_asdt pi psi)).

Axiom H12 :
  ((sin (infix_asdt pi psi)) =
   (v_sine complex_plan
    (complex_as_a_vector (exp (infix_asdt im (infix_asdt pi psi)))))).

Axiom H13 : infix_lseqdt tzero (modulus (sin (infix_asdt pi psi))).

Axiom H14 :
  infix_lseqdt (cpower (modulus (sin (infix_asdt pi psi))) 2%Z)
  (cpower (modulus (infix_asdt pi psi)) 2%Z).

Axiom H15 : infix_lseqdt tzero (modulus (infix_asdt pi psi)).
...
Axiom H17 : infix_lseqdt tzero (modulus (sin (infix_asdt pi psi))).

Axiom H18 : real_ (i_to_t b).

Axiom H19 :
  ((sin (infix_asdt (infix_asdt pi psi) (i_to_t b))) =
   (v_sine complex_plan
    (complex_as_a_vector
     (exp (infix_asdt im (infix_asdt (infix_asdt pi psi) (i_to_t b))))))).

Axiom H20 :
  infix_lseqdt tzero
  (modulus (sin (infix_asdt (infix_asdt pi psi) (i_to_t b)))).
...
Axiom H22 :
  infix_lseqdt
  (infix_sldt
   (cpower (modulus (sin (infix_asdt pi (infix_asdt psi (i_to_t b))))) 2%Z)
   (cpower (modulus (infix_asdt pi psi)) 2%Z))
  (cpower
   (modulus
    (ind_sum (fun (y:Z) => (cpower (ang_exp (real_to_ang psi)) y)) 0%Z b))
...
Axiom H23 : real_ (i_to_t b).

Axiom H24 : real_ (infix_asdt psi (i_to_t b)).

(* Why3 goal *)
Theorem G : ((modulus (i_to_t b)) = (i_to_t b)).
Proof.
