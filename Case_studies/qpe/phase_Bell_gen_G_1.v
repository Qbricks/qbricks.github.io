    ...
    ((a_phase_part_ (repeat_ c k) bvx bvy) =
     (ind_product
      (fun (k1:Z) =>
       (a_phase_part_ c
        (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvx)
         ((k1 - 1%Z)%Z * (a_depth_ c))%Z)
        (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvy)
         ((k1 - 1%Z)%Z * (a_sum_range_ c))%Z)))
      1%Z (k + 1%Z)%Z)))).
     ...
     ((a_phase_part (repeat c k) bvx bvy) =
      (d_sum_
       (fun (k1:Z) =>
        (a_phase_part c
         (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvx)
          ((k1 - 1%Z)%Z * (a_depth c))%Z)
         (((fun (y0:Z -> Z) (y1:Z) (y2:Z) => (shift y0 y1 y2)) bvy)
          ((k1 - 1%Z)%Z * (a_sum_range c))%Z)))
       1%Z (k + 1%Z)%Z))))).
...
Axiom id_cont_kron :
  ...
  ((((value1 (ket_to_bv x)) c) = 1%Z) ->
   semantics (id_cont circ c ft n) x (kronecker x z)).
...
Parameter result20: a_circuit.

Parameter o: a_circuit.

Axiom H3 : ~ (k = 0%Z).

Axiom H4 : ((a_depth (param_cnot 0%Z k n)) = n).

Axiom H5 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
  ((0%Z <= i)%Z /\ (i < (a_depth (param_cnot 0%Z k n)))%Z) ->
  ((i = k) ->
   ((a_ket_part (param_cnot 0%Z k n) bvx bvy i) =
    (int.EuclideanDivision.mod1 ((bvx 0%Z) + (bvx k))%Z 2%Z))) /\
  (~ (i = k) -> ((a_ket_part (param_cnot 0%Z k n) bvx bvy i) = (bvx i))).

Axiom H6 :
  forall (bvx:Z -> Z) (bvy:Z -> Z),
  ((a_phase_part (param_cnot 0%Z k n) bvx bvy) = zero_d_).

Axiom H7 : ((a_depth o) = n).

Axiom H8 : ((a_sum_range o) = 1%Z).

Axiom H9 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
  ((0%Z <= i)%Z /\ (i < (k - 1%Z)%Z)%Z) ->
  ((a_ket_part o bvx bvy 0%Z) = (bvy i)).

Axiom H10 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
  (((k - 1%Z)%Z <= i)%Z /\ (i < n)%Z) ->
  ((a_ket_part o bvx bvy 0%Z) = (bvx i)).

Axiom H11 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), (binary bvx) -> (binary bvy) ->
  ((a_phase_part o bvx bvy) = zero_d_).

Axiom H12 : (result20 = (a_sequence o (param_cnot 0%Z k n))).

Axiom H13 :
  ((a_sum_range result20) =
   ((a_sum_range o) + (a_sum_range (param_cnot 0%Z k n)))%Z).

Axiom H14 : ((a_depth result20) = (a_depth o)).

Axiom H15 :
  forall (bvx:Z -> Z) (bvy:Z -> Z), forall (i:Z),
  ((0%Z <= i)%Z /\ (i < (a_depth result20))%Z) ->
  ((a_ket_part result20 bvx bvy i) =
   (a_ket_part (param_cnot 0%Z k n)
    ((((fun (y0:a_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
        (a_ket_part y0 y1 y2 y3))
       o)
      bvx)
...
Axiom H16 :
  forall (bvx:Z -> Z) (bvy:Z -> Z),
  ((a_phase_part result20 bvx bvy) =
   (add_ (a_phase_part o bvx bvy)
    (a_phase_part (param_cnot 0%Z k n)
     ((((fun (y0:a_circuit) (y1:Z -> Z) (y2:Z -> Z) (y3:Z) =>
         (a_ket_part y0 y1 y2 y3))
        o)
       bvx)
...
Axiom H17 : ((a_depth result20) = n).

Axiom H18 : ((a_sum_range result20) = 1%Z).
