...
Axiom H2 : infix_lseqdt tzero theta.

Axiom H3 : infix_lsdt theta tone.

Axiom H4 : real_ (pow_inv_2 k).

Axiom H5 : infix_gtdt (pow_inv_2 k) tzero.

Axiom H6 : ((pow_inv_2 k) = (pow_inv_sqrt_2 (2%Z * k)%Z)).

Axiom H7 :
  ((pow_inv_2 k) = (infix_asdt (pow_inv_sqrt_2 k) (pow_inv_sqrt_2 k))).

Axiom H8 : (0%Z <= (best_appr theta k))%Z.

Axiom H9 : ((best_appr theta k) < (power_ 2%Z k))%Z.

Axiom H10 : mem (best_appr theta k) (to_fset 0%Z ((power_ 2%Z k) + 1%Z)%Z).
...
(* Why3 goal *)
Theorem G : ((power 2%Z k) = (power_ 2%Z k)).
Proof.
