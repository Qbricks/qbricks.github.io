...
Axiom int_qft_rev_coeff_no_mod :
  ...
  ((int_qft_rev_coeff i j n) =
   (ang_exp
    (real_to_ang
     (infix_asdt
      (infix_asdt (prefix_mndt (i_to_t i)) (i_to_t (int_bit_inversion j n)))
      (pow_inv_2 n))))).
  ...
  ((best_appr_ k a bound n) =
   (int.EuclideanDivision.mod1
    (int_bit_inversion_ext
     (best_appr (infix_sldt (i_to_t k) (i_to_t (multi_order a bound)))
      (2%Z * n)%Z)
     (2%Z * n)%Z)
    (power_ 2%Z (2%Z * n)%Z))).
       ...
       (i_to_t
        (int_bit_inversion_ext
         (best_appr (infix_sldt (i_to_t k) (i_to_t (multi_order a bound)))
          (2%Z * n)%Z)
         (2%Z * n)%Z)))
      (infix_sldt (i_to_t k) (i_to_t (multi_order a bound)))))
...
Axiom best_appr_bound :
  forall (k:Z) (a:Z) (bound:Z) (n:Z) (b:Z), (0%Z < n)%Z ->
  ((0%Z <= a)%Z /\ (a < (power_ 2%Z n))%Z) ->
  (((power_ 2%Z (n - 1%Z)%Z) < bound)%Z /\ (bound < (power_ 2%Z n))%Z) ->
  (a < bound)%Z -> (co_prime a bound) ->
  ((0%Z <= k)%Z /\ (k < (multi_order a bound))%Z) -> (0%Z < b)%Z ->
  (((b - 1%Z)%Z * (multi_order a bound))%Z <= (power_ 2%Z (2%Z * n)%Z))%Z ->
  (infix_lseqdt
   (infix_asdt (cpower (i_to_t b) 2%Z)
    (infix_sldt (i_to_t 4%Z) (cpower pi 2%Z)))
      ...
      (fun (y:Z) =>
       (cpower
        (ang_exp
         (real_to_ang
          (infix_asdt (pow_inv_2 (2%Z * n)%Z)
           (i_to_t ((-(best_appr_ k a bound n))%Z * (multi_order a bound))%Z))))
        y))
      0%Z b))
    2%Z)) /\
  ((ind_sum
    (fun (y:Z) =>
     (ang_exp
      (real_to_ang
       (infix_asdt (pow_inv_2 (2%Z * n)%Z)
        (i_to_t
         (((-(best_appr_ k a bound n))%Z * (multi_order a bound))%Z * y)%Z)))))
    0%Z b)
   =
   (ind_sum
    (fun (y:Z) =>
...
Axiom separated_best_appr :
  ...
  (((power_ 2%Z (n - 1%Z)%Z) < bound)%Z /\ (bound < (power_ 2%Z n))%Z) ->
  (a < bound)%Z -> (co_prime a bound) ->
  ((0%Z <= k)%Z /\ ((k < k')%Z /\ (k' <= (multi_order a bound))%Z)) ->
  ((k' =
    (int.EuclideanDivision.div (power_ 2%Z (2%Z * n)%Z)
     (multi_order a bound))) ->
   (0%Z <
  ...
  forall (l:Z),
  ~ ((((int_bit_inversion (best_appr_ k a bound n) (2%Z * n)%Z) *
       (power_ 2%Z n))%Z
      + l)%Z
     =
     (((int_bit_inversion (best_appr_ k' a bound n) (2%Z * n)%Z) *
...
Axiom shor_meas_j_i :
  forall (a:Z) (bound:Z) (n:Z) (i:Z) (j:Z) (k:Z), (0%Z < n)%Z ->
  ((0%Z <= a)%Z /\ (a < (power_ 2%Z n))%Z) ->
  (((power_ 2%Z (n - 1%Z)%Z) < bound)%Z /\ (bound < (power_ 2%Z n))%Z) ->
  (a < bound)%Z -> (co_prime a bound) ->
  ((0%Z <= k)%Z /\ (k < (multi_order a bound))%Z) ->
  ((0%Z <= i)%Z /\ (i < (multi_order a bound))%Z) ->
  ((0%Z <= j)%Z /\ (j < (power_ 2%Z (2%Z * n)%Z))%Z) ->
  (j = (best_appr_ k a bound n)) ->
  ((i <
    (int.EuclideanDivision.mod1 (power_ 2%Z (2%Z * n)%Z)
     (multi_order a bound)))%Z ->
   infix_lseqdt
    ...
    (cpower
     (i_to_t
      (int.EuclideanDivision.div (power_ 2%Z (2%Z * n)%Z)
       (multi_order a bound)))
     2%Z)
    (infix_asdt (cpower (pow_inv_2 (2%Z * n)%Z) 2%Z)
     (infix_sldt (i_to_t 4%Z) (cpower pi 2%Z))))
   (proba_measure
    (pat_sem (shor_circ a bound n)
     (kronecker (ket (2%Z * n)%Z 0%Z) (ket n 1%Z)))
    (((int_bit_inversion j (2%Z * n)%Z) * (power_ 2%Z n))%Z +
     (int.EuclideanDivision.mod1 (power_ a i) bound))%Z
    (3%Z * n)%Z)) /\
  (((int.EuclideanDivision.mod1 (power_ 2%Z (2%Z * n)%Z)
     (multi_order a bound))
    <= i)%Z ->
...
Axiom H12 :
  (0%Z < (multi_order a bound))%Z ->
  ((cardinal (to_fset 0%Z (multi_order a bound))) =
   ((multi_order a bound) - 0%Z)%Z).

Axiom H13 :
  ((multi_order a bound) <= 0%Z)%Z ->
  is_empty (to_fset 0%Z (multi_order a bound)).

Axiom H14 :
  forall (k1:Z),
  (mem k1 (to_fset 0%Z (multi_order a bound))) <->
  ((0%Z <= k1)%Z /\ (k1 < (multi_order a bound))%Z).

Axiom H15 : is_a_ket (ket n 1%Z).

Axiom H16 : ((ket_length (ket n 1%Z)) = n).

Axiom H17 : ((columns (ket n 1%Z)) = 1%Z).

Axiom H18 : ((rows (ket n 1%Z)) = (power 2%Z n)).
...
Axiom H22 : is_a_ket (ket (2%Z * n)%Z 0%Z).

Axiom H23 : ((ket_length (ket (2%Z * n)%Z 0%Z)) = (2%Z * n)%Z).

Axiom H24 : ((columns (ket (2%Z * n)%Z 0%Z)) = 1%Z).

Axiom H25 : ((rows (ket (2%Z * n)%Z 0%Z)) = (power 2%Z (2%Z * n)%Z)).
...
Axiom H32 : ((width (shor_circ a bound n)) = (3%Z * n)%Z).

Axiom H33 : ((ancillas (shor_circ a bound n)) = (n + 2%Z)%Z).
...
Axiom H38 : mem i (to_fset 0%Z (multi_order a bound)).

(* Why3 goal *)
Theorem VC_shor_meas_i :
  ((best_appr_ i a bound n) < (power_ 2%Z (3%Z * n)%Z))%Z.
Proof.
